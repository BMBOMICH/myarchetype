...
export default observer(function IcebreakerGameScreen() {
  ...
  const initGame = useCallback(async () => {
    try {
      ...
      const bank = type === 'would_you_rather'
        ? WOULD_YOU_RATHER_QUESTIONS
        : THIS_OR_THAT_QUESTIONS;
      screen$.questions.set(
        secureShuffle([...(bank as Question[])]).slice(0, TOTAL_QUESTIONS),
      );
    } catch (error) {
  ...
  }, [chatId, matchId, gameType, router]);
  ...
  const handleAnswer = useCallback(async (answer: string) => {
    const session = screen$.gameSession.get();
    const current = screen$.myAnswer.get();
  ...
  // All useMemo calls at top level — never inside conditionals
  const progressFillStyle = useMemo(
    () => [s.progressFill, { width: `${((currentQuestion + 1) / TOTAL_QUESTIONS) * 100}%` as `${number}%` }],
    [currentQuestion],
  );

  const resultBannerStyle = useMemo(
    () => [s.resultBanner, lastResult?.matched ? s.resultMatch : s.resultNoMatch],
    [lastResult?.matched],
  );

  const optionAStyle = useMemo(
    () => [
      s.optionButton,
      myAnswer === 'a' && s.optionButtonSelected,
      waitingForMatch && myAnswer !== 'a' && s.optionButtonDisabled,
    ],
    [myAnswer, waitingForMatch],
  );

  const optionATextStyle = useMemo(
    () => [s.optionText, myAnswer === 'a' && s.optionTextSelected],
    [myAnswer],
  );

  const optionBStyle = useMemo(
    () => [
      s.optionButton,
      myAnswer === 'b' && s.optionButtonSelected,
      waitingForMatch && myAnswer !== 'b' && s.optionButtonDisabled,
    ],
    [myAnswer, waitingForMatch],
  );

  const optionBTextStyle = useMemo(
    () => [s.optionText, myAnswer === 'b' && s.optionTextSelected],
    [myAnswer],
  );

  const onPressA = useCallback(() => void handleAnswer('a'), [handleAnswer]);
  const onPressB = useCallback(() => void handleAnswer('b'), [handleAnswer]);
    ...
    <View style={s.container}>
      ...
      <View style={s.questionContainer}>
        <Text style={s.questionText}>
          {isWouldYouRather ? 'Would you rather...' : 'Which do you prefer?'}
        </Text>
      </View>
      ...
      {question && (
        <View style={s.optionsContainer}>
          <TouchableOpacity
            ...
            accessibilityRole="button"
          >
            <Text style={optionATextStyle}>{question.a}</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            accessibilityRole="button"
          >
            <Text style={optionBTextStyle}>{question.b}</Text>
          </TouchableOpacity>
        </View>
      )}
