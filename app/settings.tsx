...
export default function SettingsScreen() {
  ...
  const openModal  = useCallback((modal: keyof ModalState) => dispatchModals({ type: 'OPEN',  modal }), []);
  const closeModal = useCallback((modal: keyof ModalState) => dispatchModals({ type: 'CLOSE', modal }), []);
  ...
  const generateReferralCode = useCallback((uid: string) =>
    `MA${uid.substring(0, 6).toUpperCase()}`, []);
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      logger.log('[Settings] Account no longer valid:', err.code);
  ...
  const handleSubmitBugReport = useCallback(async () => {
    if (!bug.title.trim() || !bug.description.trim()) {
      Alert.alert(t.error, 'Please fill in all fields'); return;
    }
    if (!user) return;
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.section}>
        ...
        {notifItems.map((item) => (
          <View key={item.field} style={styles.settingRow}>
            <Text style={styles.settingLabel}>{item.label}</Text>
            <Switch
              value={item.value}
              onValueChange={(v) => setField(item.field, v)}
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.settingRow} onPress={() => openModal('bugReport')} accessibilityLabel="Report a bug" accessibilityRole="button">
          <Text style={styles.settingLabel}>{t.reportBug}</Text>
          <Text style={styles.arrow}>▶</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.settingRow} onPress={() => openModal('donation')} accessibilityLabel="Donate to support the app" accessibilityRole="button">
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.settingRow} onPress={() => router.push('/privacy')} accessibilityLabel="View privacy policy" accessibilityRole="button">
          <Text style={styles.settingLabel}>{t.privacyPolicy}</Text>
          <Text style={styles.arrow}>▶</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.settingRow} onPress={() => router.push('/terms')} accessibilityLabel="View terms of service" accessibilityRole="button">
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.settingRow} onPress={() => router.push('/edit-profile')} accessibilityLabel="Edit profile" accessibilityRole="button">
          <Text style={styles.settingLabel}>{t.editProfile}</Text>
          <Text style={styles.arrow}>▶</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.settingRow} onPress={() => router.push('/blocked-users')} accessibilityLabel="View blocked users" accessibilityRole="button">
      ...
      {/* ── LANGUAGE MODAL ── */}
      <Modal visible={modals.language} animationType="slide" transparent onRequestClose={() => closeModal('language')}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.languageList}>
              {SUPPORTED_LANGUAGES.map((lang) => (
                <TouchableOpacity
                  ...
                  accessibilityLabel={`Select ${languageNames[lang]}${language === lang ? ', currently selected' : ''}`}
                  accessibilityRole="button"
            ...
            <TouchableOpacity style={styles.modalClose} onPress={() => closeModal('language')} accessibilityLabel="Close language picker" accessibilityRole="button">
      ...
      {/* ── BUG REPORT MODAL ── */}
      <Modal visible={modals.bugReport} animationType="slide" transparent onRequestClose={() => closeModal('bugReport')}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              value={bug.title}
              onChangeText={(v) => dispatchBug({ type: 'SET_TITLE', value: v })}
              maxLength={100}
            ...
            <TextInput
              ...
              onChangeText={(v) => dispatchBug({ type: 'SET_DESC', value: v })}
              multiline
              numberOfLines={5}
              maxLength={500}
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity
                ...
                onPress={() => { closeModal('bugReport'); dispatchBug({ type: 'RESET' }); }}
      ...
      {/* ── DONATION MODAL ── */}
      <Modal visible={modals.donation} animationType="slide" transparent onRequestClose={() => closeModal('donation')}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.donationButtons}>
              {[
                ...
                <TouchableOpacity
                  ...
                  accessibilityLabel={`Donate via ${item.label}`}
            ...
            <TouchableOpacity style={styles.modalClose} onPress={() => closeModal('donation')} accessibilityLabel="Close donation modal" accessibilityRole="button">
              <Text style={styles.modalCloseText}>{t.cancel}</Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      {/* ── DELETE MODAL ── */}
      <Modal visible={modals.deleteAccount} animationType="slide" transparent onRequestClose={() => closeModal('deleteAccount')}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity
                style={styles.modalButtonCancel}
                onPress={() => { closeModal('deleteAccount'); setDeletePassword(''); setDeleteError(''); }}
