...
import { AccessibilityInfo, Alert, AppState, AppStateStatus, BackHandler, Platform, RefreshControl, ScrollView, StyleSheet, Text, TouchableOpacity, useColorScheme, useWindowDimensions, View } from 'react-native';
...
interface UserData {
  ...
  interestedIn?: string; [key: string]: unknown;
}

type AppRoute = '/login'|'/my-matches'|'/matches'|'/second-look'|'/personality-quiz'|'/edit-profile'|'/dating-stats'|'/stories'|'/date-checkin'|'/relationship-mode'|'/date-spot-reviews'|'/settings'|'/referral'|'/admin'|'/daily-question'|'/profile-views'|'/chat'|'/debug'|'/profile-setup';
...
interface HomeState {
  loading: boolean; refreshing: boolean; error: string | null; userData: UserData | null;
  loginStreak: number; longestStreak: number; profileStrength: ProfileStrengthResult | null;
  showDailyQuestion: boolean; isOnline: boolean; loggingOut: boolean;
  authReady: boolean; userId: string | null; userEmail: string | null;
}
...
const maskEmail = (e: string) => { const [l, d] = e.split('@'); if (!d) return e; if ((l ?? '').length <= 2) return `${l}@${d}`; return `${l![0]}${'*'.repeat(Math.min(l!.length - 2, 6))}${l![l!.length - 1]}@${d}`; };
...
function SkeletonBox({ width, height, radius = RADIUS.md, style, color }: SkeletonBoxProps) {
...
function HomeScreenSkeleton({ COL, insets }: { COL: ThemeColors; insets: { top: number; bottom: number } }) {
  return (
    <View style={[s.scroll, { backgroundColor: COL.bg }]} accessibilityLabel="Loading home screen">
      <View style={[s.scrollContent, { paddingTop: insets.top + SPACING.xl, paddingBottom: insets.bottom + SPACING.xxxxl }]}>
        ...
        <View style={[s.skeletonGrid, { marginTop: SPACING.xl }]}>
...
function OfflineBanner({ COL }: { COL: ThemeColors }) {
  ...
  if (isNative) {
    return (
      <Animated.View entering={FadeInUp.duration(300)} style={[s.offlineBanner, { backgroundColor: COL.danger }]} accessibilityRole="alert" accessibilityLabel="You are offline. Some features may not work." accessibilityLiveRegion="assertive">
        <Text style={[s.offlineBannerText, { color: COL.white }]} maxFontSizeMultiplier={MAX_FONT_SCALE}>📡 You're offline — some features may be unavailable</Text>
      </Animated.View>
    );
  }
  return (
    <View style={[s.offlineBanner, { backgroundColor: COL.danger }]} accessibilityRole="alert" accessibilityLabel="You are offline. Some features may not work." accessibilityLiveRegion="assertive">
      <Text style={[s.offlineBannerText, { color: COL.white }]} maxFontSizeMultiplier={MAX_FONT_SCALE}>📡 You're offline — some features may be unavailable</Text>
    </View>
  );
}

function ErrorScreen({ error, onRetry, COL }: { error: string; onRetry: () => void; COL: ThemeColors }) {
  return (
    <View style={[s.centered, { backgroundColor: COL.bg }]} accessibilityRole="alert">
      ...
      <TouchableOpacity style={[s.retryBtn, { backgroundColor: COL.primary }]} onPress={onRetry} activeOpacity={0.8} accessibilityRole="button" accessibilityLabel="Retry loading" accessibilityHint="Double tap to retry">
...
function ProfileHeader({ userPhoto, selfieVerified, isChampion, userName, loginStreak, longestStreak, personalityType, maskedEmail, COL, reducedMotion }: ProfileHeaderProps) {
  const greeting = getGreeting();
  const isNative = Platform.OS !== 'web';
  const streakA11y = loginStreak > 1 ? `${loginStreak} day login streak${loginStreak === longestStreak && loginStreak >= 7 ? ', personal best!' : ''}` : loginStreak === 1 ? 'Come back tomorrow to start a streak' : '';

  const content = (
    <View style={s.profileHeader} accessibilityRole="summary">
      <View style={s.photoWrap}>
        {userPhoto ? (
          <Image source={{ uri: userPhoto }} style={[s.photo, { borderColor: COL.primary }]} contentFit="cover" transition={200} accessibilityLabel={`${userName}'s profile photo`} accessibilityRole="image" />
        ) : (
          <View style={[s.photoPlaceholder, { backgroundColor: COL.card, borderColor: COL.primary }]} accessibilityLabel="No profile photo. Tap Edit Profile to add one." accessibilityRole="image">
            <Text style={[s.photoPlaceholderText, { color: COL.dim }]} accessibilityElementsHidden>?</Text>
          </View>
        )}
        {selfieVerified && (
          <View style={[s.verifiedBadge, { backgroundColor: COL.blue, borderColor: COL.bg }]} accessibilityLabel="Selfie verified" accessibilityRole="image">
            <Text style={[s.verifiedIcon, { color: COL.white }]} accessibilityElementsHidden>✓</Text>
          </View>
        )}
        {isChampion && (
          <View style={[s.championBadge, { backgroundColor: COL.gold, borderColor: COL.bg }]} accessibilityLabel="Community champion" accessibilityRole="image">
      ...
      <Text style={[s.userName, { color: COL.text }]} maxFontSizeMultiplier={MAX_FONT_SCALE} accessibilityRole="header" accessibilityLabel={`${greeting}, ${userName}`}>{userName}!</Text>
      {loginStreak > 1 && (
        <View style={[s.streakBadge, { backgroundColor: COL.orange }]} accessibilityRole="text" accessibilityLabel={streakA11y}>
          <Text style={[s.streakText, { color: COL.white }]} maxFontSizeMultiplier={MAX_FONT_SCALE} accessibilityElementsHidden>🔥 {loginStreak}-day streak!{loginStreak === longestStreak && loginStreak >= 7 ? ' (Personal best!)' : ''}</Text>
        </View>
      )}
      {loginStreak === 1 && (
        <View style={[s.streakBadge, { backgroundColor: COL.card, borderWidth: 1, borderColor: COL.orange }]} accessibilityRole="text" accessibilityLabel={streakA11y}>
          <Text style={[s.streakText, { color: COL.orange }]} maxFontSizeMultiplier={MAX_FONT_SCALE} accessibilityElementsHidden>🔥 Come back tomorrow to start a streak!</Text>
        </View>
      )}
      {isChampion && (
...
function StrengthCard({ profileStrength, onPress, COL, reducedMotion, index }: StrengthCardProps) {
  const isNative = Platform.OS !== 'web';
  const sColor = strengthColor(profileStrength.score, COL);
  const tips = (profileStrength.suggestions ?? []).slice(0, 2).join('. ');

  const content = (
    <TouchableOpacity style={[s.strengthCard, { backgroundColor: COL.card, borderColor: COL.primary }]} onPress={onPress} activeOpacity={0.8}
      accessibilityRole="button" accessibilityLabel={`Profile strength ${profileStrength.score}%. ${profileStrength.label}.${tips ? ` Tips: ${tips}.` : ''} Tap to edit profile.`}
      accessibilityHint="Double tap to open profile editor">
      <View style={s.strengthHeader}>
        <Text style={[s.strengthTitle, { color: COL.text }]} maxFontSizeMultiplier={MAX_FONT_SCALE} accessibilityElementsHidden>{strengthEmoji(profileStrength.score)} Profile Strength</Text>
      ...
      <View style={[s.strengthBarBg, { backgroundColor: COL.border }]} accessibilityRole="progressbar" accessibilityValue={{ min:0, max:100, now:profileStrength.score, text:`${profileStrength.score}%` }} accessibilityLabel={`Profile strength: ${profileStrength.score}%`}>
      ...
      {(profileStrength.suggestions?.length ?? 0) > 0 && profileStrength.score < 100 && (
        <View style={[s.suggestionsWrap, { borderTopColor: COL.border }]} importantForAccessibility="no">
          ...
          {(profileStrength.suggestions ?? []).slice(0, 2).map((tip, i) => <Text key={i} style={[s.suggestionText, { color: COL.sub }]} maxFontSizeMultiplier={MAX_FONT_SCALE}>• {tip}</Text>)}
          <Text style={[s.tapHint, { color: COL.primary }]} maxFontSizeMultiplier={MAX_FONT_SCALE}>Tap to edit profile →</Text>
        </View>
      )}
      {profileStrength.score >= 100 && <View style={s.perfectWrap} importantForAccessibility="no"><Text style={[s.perfectText, { color: COL.success }]} maxFontSizeMultiplier={MAX_FONT_SCALE}>🎉 Your profile is perfect!</Text></View>}
    </TouchableOpacity>
  );

  if (!reducedMotion && isNative) {
    return <Animated.View entering={FadeInDown.delay(index * 80).duration(400)} style={{ width: '100%' }}>{content}</Animated.View>;
  }
...
function PromptCard({ icon, title, subtitle, onPress, borderColor, accentColor, COL, reducedMotion, index, a11yLabel, a11yHint, rightContent }: PromptCardProps) {
  const isNative = Platform.OS !== 'web';
  const content = (
...
function EmptyMatchesCard({ COL, onPress, reducedMotion, index }: { COL: ThemeColors; onPress: () => void; reducedMotion: boolean; index: number }) {
  const isNative = Platform.OS !== 'web';
  const content = (
...
function NavigationGrid({ navItems, onNav, onLogout, loggingOut, COL, reducedMotion, screenWidth, startIndex }: NavigationGridProps) {
  const isNative = Platform.OS !== 'web';
  const isWide = screenWidth >= 600;
  const btnWidth: '48%' | '100%' = isWide ? '48%' : '100%';

  return (
    <View style={[s.buttonsWrap, isWide && s.buttonsWrapGrid]} accessibilityRole="menu" accessibilityLabel="Navigation menu">
      {navItems.map((item, i) => {
        const cleanLabel = item.label.replace(/^\S+\s/, '');
        const btn = (
          <TouchableOpacity
            style={[s.navBtn, { backgroundColor: item.color }, item.borderColor != null && { borderWidth: 2, borderColor: item.borderColor }]}
            onPress={() => onNav(item.route)} activeOpacity={0.8} accessibilityRole="menuitem"
        ...
        if (!reducedMotion && isNative) {
          return (
            <Animated.View key={item.key} entering={FadeInDown.delay((startIndex + i) * 60).duration(350)} style={{ width: btnWidth }}>
              {btn}
            </Animated.View>
          );
        }
        return <View key={item.key} style={{ width: btnWidth }}>{btn}</View>;
      })}
      {(() => {
        const logoutBtn = (
          <TouchableOpacity style={[s.logoutBtn, { borderColor: COL.danger }]} onPress={onLogout} activeOpacity={0.8} disabled={loggingOut}
            ...
            <Text style={[s.logoutBtnText, { color: COL.danger }]} maxFontSizeMultiplier={MAX_FONT_SCALE} accessibilityElementsHidden>{loggingOut ? '⏳ Logging out…' : '🚪 Log Out'}</Text>
          </TouchableOpacity>
        );
        if (!reducedMotion && isNative) {
          return (
            <Animated.View entering={FadeInDown.delay((startIndex + navItems.length) * 60).duration(350)} style={{ width: btnWidth }}>
              {logoutBtn}
            </Animated.View>
          );
        }
        return <View style={{ width: btnWidth }}>{logoutBtn}</View>;
      })()}
    </View>
  );
}
...
export default function HomeScreen() {
  ...
  const { loading, refreshing, error, userData, loginStreak, longestStreak, profileStrength, showDailyQuestion, isOnline, loggingOut, authReady, userId, userEmail } = state;
  ...
  const computedStrength = useMemo<ProfileStrengthResult | null>(() => userData ? calculateProfileStrength(userData) : null, [userData]);
  ...
  const trackStreak = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) dispatch({ type: 'SET_STREAK', payload: { current: result.currentStreak, longest: result.longestStreak } });
    } catch (err: unknown) { logger.error('trackStreak failed:', err); }
  }, [userId]);

  useEffect(() => { if (!userId || !authReady) return; void loadUserData(); void trackStreak(); }, [userId, authReady, loadUserData, trackStreak]);

  // Refresh
  const onRefresh = useCallback(async () => {
    dispatch({ type: 'SET_REFRESHING', payload: true });
    await Promise.all([loadUserData(), trackStreak()]);
    try { const answered = await hasAnsweredToday(); if (isMountedRef.current) dispatch({ type: 'SET_DAILY_QUESTION', payload: !answered }); } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      try { const answered = await hasAnsweredToday(); if (!cancelled && isMountedRef.current) dispatch({ type: 'SET_DAILY_QUESTION', payload: !answered }); }
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', (next: AppStateStatus) => { if (next === 'active' && isMountedRef.current) dispatch({ type: 'SET_ONLINE', payload: true }); });
    return () => sub.remove();
  }, []);
  ...
  useEffect(() => {
    if (!loading && userData) {
      AccessibilityInfo.announceForAccessibility(`Home screen loaded. ${getGreeting()}, ${userName}.${loginStreak > 1 ? ` ${loginStreak} day login streak.` : ''}${profileStrength ? ` Profile strength ${profileStrength.score} percent.` : ''}`);
    }
  ...
  return (
    <>
      ...
      <ScrollView
        ...
        <ProfileHeader userPhoto={userPhoto} selfieVerified={selfieVerified} isChampion={isChampion} userName={userName} loginStreak={loginStreak} longestStreak={longestStreak} personalityType={personalityType} maskedEmail={maskedEmail} COL={COL} reducedMotion={reducedMotion} />

        {profileStrength != null && <StrengthCard profileStrength={profileStrength} onPress={() => handleNav('/edit-profile')} COL={COL} reducedMotion={reducedMotion} index={animIndex++} />}

        {showDailyQuestion && <PromptCard icon="💭" title="Today's Question" subtitle="Answer to boost your profile!" onPress={() => handleNav('/daily-question')} borderColor={COL.purple} accentColor={COL.purple} COL={COL} reducedMotion={reducedMotion} index={animIndex++} a11yLabel="Today's daily question. Tap to answer and boost your profile." a11yHint="Double tap to open the daily question" />}

        {profileViews > 0 && <PromptCard icon="👀" title={`${profileViews}`} subtitle="people viewed your profile" onPress={() => handleNav('/profile-views')} borderColor={COL.orange} accentColor={COL.orange} COL={COL} reducedMotion={reducedMotion} index={animIndex++} a11yLabel={`${profileViews} people viewed your profile. Tap to see who.`} a11yHint="Double tap to see who viewed your profile" rightContent={<Text style={[s.promptArrow, { color: COL.orange }]} accessibilityElementsHidden>→</Text>} />}

        {matchCount === 0 && userData != null && <EmptyMatchesCard COL={COL} onPress={() => handleNav('/matches')} reducedMotion={reducedMotion} index={animIndex++} />}

        {userData != null && <ProfileCompletionCard userData={userData} showDetails />}

        <NavigationGrid navItems={navItems} onNav={handleNav} onLogout={handleLogout} loggingOut={loggingOut} COL={COL} reducedMotion={reducedMotion} screenWidth={screenWidth} startIndex={animIndex} />
      </ScrollView>
    </>
  );
}
...
const s = StyleSheet.create({
  ...
  skeletonGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', width: '100%', gap: SPACING.sm },
