    ...
    try {
      ...
      if (!theirLikeSnap.empty) {
        ...
        if (profile.pushToken) {
          ...
          const senderData = senderDoc.exists() ? (senderDoc.data() as { name?: string }) : null;
          const senderName = senderData?.name ?? 'Someone';
          await fetch('https://exp.host/--/api/v2/push/send', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ to: profile.pushToken, title: "It's a Match! 💕", body: `${senderName} likes you too!`, sound: 'default', data: { type: 'match' } }),
          });
        }
        ...
        if (profile.pushToken) {
          await fetch('https://exp.host/--/api/v2/push/send', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ to: profile.pushToken, title: 'Someone likes you! 💝', body: 'Open the app to see who!', sound: 'default', data: { type: 'like' } }),
          });
        }
  ...
  return (
    <ScrollView style={s.scrollContainer} contentContainerStyle={s.scrollContent}>
      ...
      <View style={s.card}>
            ...
            >
              {profile.photos.map((photoUrl, index) => (
                <Image
                  ...
                  accessibilityLabel={`${profile.name}'s photo ${index + 1} of ${photoCount}`}
                />
              ))}
            </ScrollView>
            {photoCount > 1 && (
        ...
        {profile.bio && (
          <Text style={s.bio} accessibilityLabel={`Bio: ${profile.bio}`}>"{profile.bio}"</Text>
        )}
      ...
      <View style={s.actions}>
        <TouchableOpacity
          style={s.removeButton} onPress={handleRemove} disabled={actionLoading}
        ...
        <TouchableOpacity
          style={s.skipAgainButton} onPress={handleSkipAgain} disabled={actionLoading}
        ...
        <TouchableOpacity
          style={[s.likeButton, actionLoading && s.likeButtonDisabled]}
...
const s = StyleSheet.create({
  ...
  photo:              { height: 350, borderRadius: 15, resizeMode: 'cover' },
