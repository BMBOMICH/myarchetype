...
const darkTokens = {
  ...
  warn: '#f0a050',
  success: '#5cb85c',
  successGlow: 'rgba(92,184,92,0.1)',
  textPrimary: '#f2f2ff',
...
const lightTokens = {
  ...
  warn: '#c47a00',
  success: '#3a8a3a',
  successGlow: 'rgba(58,138,58,0.08)',
  textPrimary: '#0d0d1a',
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[st.modalCard, { backgroundColor: C.card, borderColor: C.cardBorder, transform: [{ scale }] }]}>
        <Text style={[st.modalTitle, { color: C.textPrimary }]} allowFontScaling maxFontSizeMultiplier={1.2}>
          {config.title}
        </Text>
        <Text
...
const GradientButton = React.memo(({ onPress, disabled, loading, label, C }: {
  ...
  const colors = useMemo(
    () => (disabled ? [C.disabledBg, C.disabledBg] : [C.buttonGradStart, C.buttonGradEnd]) as [string, string],
    [disabled, C]
  );
  ...
  return (
    <Animated.View style={[st.buttonOuter, { shadowColor: C.accent, transform: [{ scale }] }]}>
      ...
      >
        <LinearGradient colors={colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[st.button, disabled && st.buttonDisabled]}>
          {loading ? (
            <View style={st.buttonContent}>
              <ActivityIndicator color={C.white} size="small" />
              <Text style={[st.buttonText, { marginLeft: 8 }]} allowFontScaling maxFontSizeMultiplier={1.2}>
                Creating Account…
              </Text>
            </View>
          ) : (
            <View style={st.buttonContent}>
              <Text style={[st.buttonText, disabled && { color: C.textMuted }]} allowFontScaling maxFontSizeMultiplier={1.2}>
                {label}
              </Text>
              <Ionicons
      ...
      {items.map((item) => {
        ...
        return (
          <View key={item.key} style={st.requirementRow}>
            <Ionicons name={met ? 'checkmark-circle' : 'close-circle'} size={16} color={met ? C.success : C.textMuted} style={{ marginRight: 8 }} />
            <Text style={[st.requirementText, { color: met ? C.success : C.textMuted }]} allowFontScaling maxFontSizeMultiplier={1.2}>
              {item.label}
            </Text>
          </View>
        );
...
const SuccessOverlay = React.memo(({ C }: { C: Tokens }) => {
  ...
  return (
    ...
    >
      <Animated.View
        style={[st.successCard, { backgroundColor: C.card, borderColor: C.cardBorder, transform: [{ scale }] }]}
      ...
      >
        ...
        <Text style={[st.successTitleText, { color: C.textPrimary }]} allowFontScaling maxFontSizeMultiplier={1.2}>
          Account Created!
        </Text>
    ...
    if (this.state.hasError) {
      return (
        <View style={[st.errorFallback, { backgroundColor: darkTokens.bg }]}>
          ...
          <Text style={[st.errorFallbackTitle, { color: darkTokens.textPrimary }]}>
            Something went wrong
          </Text>
          <TouchableOpacity
            onPress={() => this.setState({ hasError: false })}
            style={[st.retryButton, { borderColor: darkTokens.accent }]}
            accessibilityRole="button"
          >
...
export default function SignupScreen() {
  ...
  const confirmSuccessMsg =
    state.confirmPassword.length > 5 && passwordsMatch && !state.confirmPasswordError
      ? 'Passwords match'
      : '';
  const confirmError =
  ...
  const logoPaused =
    state.emailFocused || state.passwordFocused || state.confirmFocused || !appActive;
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <Text
                        style={[st.title, { color: C.textPrimary, fontSize: IS_SMALL ? 26 : 32 }]}
                      ...
                      <Text
                        style={[st.subtitle, { color: C.textSecondary, fontSize: IS_SMALL ? 14 : 16 }]}
                        allowFontScaling
                        maxFontSizeMultiplier={1.3}
                      >
                        Join MyArchetype today
                      </Text>
                    </Animated.View>
                    ...
                    >
                      ...
                      {IS_WEB && state.capsLockOn && (state.passwordFocused || state.confirmFocused) && (
                        <View style={[st.capsLockRow, { backgroundColor: C.errorGlow, borderColor: C.warn }]} accessibilityLiveRegion="polite">
                          ...
                          <Text style={[st.capsLockText, { color: C.warn }]} allowFontScaling maxFontSizeMultiplier={1.2}>
                            Caps Lock is on
                          </Text>
                        </View>
                      )}
                    ...
                    <Animated.View style={[st.footer, { opacity: footerFade }]}>
                      ...
                      <View style={st.separatorRow}>
                        <View style={[st.separatorLine, { backgroundColor: C.separator }]} />
                        <Text style={[st.separatorText, { color: C.textMuted }]}>or</Text>
                        <View style={[st.separatorLine, { backgroundColor: C.separator }]} />
                      </View>

                      <TouchableOpacity onPress={handleLogin} disabled={state.loading} activeOpacity={0.7} style={st.loginButton}>
              ...
              <Pressable style={st.fill} onPress={dismissKeyboard}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <PulseLogo C={C} paused={logoPaused} />
                      <Text style={[st.title, { color: C.textPrimary, fontSize: IS_SMALL ? 26 : 32 }]} accessibilityRole="header">
                        Create Account
                      </Text>
