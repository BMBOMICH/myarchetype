...
let prefersReducedMotion = IS_WEB && typeof window !== 'undefined'
...
const buildWebInputStyle = (C: Tokens): WebStyleProps => ({
  ...
  letterSpacing:0.2, color:C.textPrimary, direction: IS_RTL ? 'rtl' : 'ltr',
});
...
const AnimatedError = React.memo(function AnimatedError({
  message, inputId, C,
  ...
  const webProps: WebAriaProps = IS_WEB
    ...
    : {};
  return (
    <Animated.View style={[st.errorRow, animStyle]} accessibilityLiveRegion="assertive" {...webProps}>
      <Ionicons name="alert-circle" size={14} color={C.error} accessibilityElementsHidden importantForAccessibility="no" />
      <Text style={[st.errorText, { color: C.error }]} allowFontScaling maxFontSizeMultiplier={1.3}>
        {message}
      </Text>
    </Animated.View>
  );
...
const SuccessText = React.memo(function SuccessText({
  message, C,
  ...
  return (
    <View style={st.successRow}>
      ...
      <Text style={[st.successTextMsg, { color: C.success }]} allowFontScaling maxFontSizeMultiplier={1.3}>
        {message}
      </Text>
    </View>
  );
...
const BrandLogo = React.memo(function BrandLogo({
  ...
  return (
    <Animated.View style={[st.logoOuter, animStyle]} accessibilityElementsHidden importantForAccessibility="no">
...
const CustomModal = React.memo(function CustomModal({
  ...
  return (
    <Animated.View
      style={[st.modalOverlay, { backgroundColor: C.overlay }, overlayStyle]}
      {...webDialogProps}
      accessibilityViewIsModal
    >
      <Pressable
        style={st.modalBackdrop}
        onPress={() = accessibilityLabel="button"> void doClose()}
        accessibilityLabel="Close dialog"
      />
      <Animated.View style={[st.modalCard, { backgroundColor: C.card, borderColor: C.cardBorder }, cardStyle]}>
        ...
        <View style={st.modalButtons}>
          {config.buttons.map((btn, i) => (
            ...
            >
              <Text style={[
                st.modalButtonText,
                { color: btn.primary ? C.white : C.textSecondary, fontWeight: btn.primary ? '700' : '500' },
              ]}>
                {btn.label}
              </Text>
            </Pressable>
...
const GradientButton = React.memo(function GradientButton({
  ...
  return (
    <Animated.View style={[st.buttonOuter, { shadowColor: isDisabled ? 'transparent' : C.accent }, animStyle]}>
      <Pressable
      ...
      >
        {/* borderRadius extracted to style — no inline object */}
        <View pointerEvents={isDisabled ? 'none' : 'auto'} style={st.gradientInner}>
          <LinearGradient
            colors={colors}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
          ...
          >
            {loading ? (
              <View style={st.buttonContent}>
                ...
                <Text style={[st.buttonText, st.buttonLoadingText]}>Creating Account…</Text>
              </View>
            ) : (
              <View style={st.buttonContent}>
      ...
      <View style={[
        ...
        {onTogglePassword && (
          <Pressable
            ...
            hitSlop={12}
            style={[st.eyeButton, webCursorStyle]}
            accessibilityRole="button"
...
const StrengthBar = React.memo(function StrengthBar({
  password, C,
}: { password: string; C: Tokens }) {
  const widthPct = useSharedValue(0);
  const strength = useMemo(() => validators.strength(password, C), [password, C]);

  useEffect(() => {
    widthPct.value = withTiming(strength.percent, { duration: getAnimDuration(300) }, []);
  }, [strength.percent, widthPct]);

  const barStyle = useAnimatedStyle(() => ({
    width: `${widthPct.value}%` as `${number}%`,
  }));
  ...
  return (
    <View style={st.strengthContainer} accessibilityLabel={`Password strength: ${strength.level}`}>
      <View style={[st.strengthBarBg, { backgroundColor: C.inputBorder }]}>
        <Animated.View style={[st.strengthBar, { backgroundColor: strength.color }, barStyle]} />
      </View>
...
const PasswordRequirements = React.memo(function PasswordRequirements({
  password, C,
}: { password: string; C: Tokens }) {
  const opacity = useSharedValue(0);
  ...
  return (
    <Animated.View
      style={[st.requirementsContainer, { backgroundColor: C.requirementsBg, borderColor: C.cardBorder }, animStyle]}
        ...
        return (
          <View key={item.key} style={st.requirementRow}>
            <Ionicons
              ...
              style={st.requirementIcon}
            />
            <Text
              style={[st.requirementText, { color: met ? C.success : C.textMuted }, met && st.requirementMet]}
              allowFontScaling
              maxFontSizeMultiplier={1.2}
            >
              {item.label}
...
const LockoutBanner = React.memo(function LockoutBanner({
  seconds, C,
}: { seconds: number; C: Tokens }) {
  ...
  return (
    ...
    >
      ...
      <Text style={[st.lockoutText, { color: C.error }]} allowFontScaling maxFontSizeMultiplier={1.2}>
        Too many attempts — try again in <Text style={st.lockoutBold}>{seconds}s</Text>
      </Text>
    </View>
  );
});

const SuccessOverlay = React.memo(function SuccessOverlay({ C }: { C: Tokens }) {
  ...
  useEffect(() => {
    ...
    if (!IS_WEB) runOnJS(AccessibilityInfo.announceForAccessibility)('Account created successfully!');
  }, [overlayOpacity, cardScale, iconScale]);

  return (
    <Animated.View
      style={[st.successOverlay, { backgroundColor: C.overlay }, overlayStyle]}
      pointerEvents="box-only"
      accessibilityViewIsModal
      accessibilityLiveRegion="assertive"
    >
      <Animated.View
        style={[st.successCard, { backgroundColor: C.card, borderColor: C.cardBorder }, cardStyle]}
      ...
      >
        ...
        <Text style={[st.successSub, { color: C.textSecondary }]}>
          Check your email to verify your account.
        </Text>
        <ActivityIndicator size="small" color={C.accent} style={st.successSpinner} />
      </Animated.View>
    </Animated.View>
  );
});
...
export default function SignupScreen() {
  ...
  const slideAnim   = useSharedValue(prefersReducedMotion ? 0 : 30);
  const headerFade  = useSharedValue(0);
  const headerSlide = useSharedValue(prefersReducedMotion ? 0 : 16);
  const formFade    = useSharedValue(0);
  const formSlide   = useSharedValue(prefersReducedMotion ? 0 : 16);
    ...
    <SignupErrorBoundary>
      <View style={[st.rootBg, { backgroundColor: C.bg }]}>
        ...
        <SafeAreaView style={st.safe}>
          ...
          >
            <Pressable style={st.fill} onPress={dismissKeyboard} accessibilityLabel="button">
              <ScrollView
                ref={scrollRef}
                ...
                bounces={false}
              >
                <Animated.View style={[st.content, { maxWidth: 440 }, contentStyle]}>
                  <Pressable
                    onPress={handleLogin}
                    style={[st.backRow, webCursorStyle]}
                  ...
                  <Animated.View style={[st.headerContainer, headerStyle]}>
                    ...
                    <Text style={[st.subtitle, { color: C.textSecondary, fontSize: IS_SMALL ? 15 : 16 }]}>
                  ...
                  <Animated.View style={[
                    ...
                    <InputField
                      ...
                      onFocus={useCallback(() => dispatch({ type: 'SET_NAME_FOCUSED', payload: true , [])})}
                      onBlur={useCallback(() => dispatch({ type: 'SET_NAME_FOCUSED', payload: false , [])})}
                    ...
                    <InputField
                      ...
                      onFocus={useCallback(() => dispatch({ type: 'SET_DOB_FOCUSED', payload: true , [])})}
                      onBlur={useCallback(() => dispatch({ type: 'SET_DOB_FOCUSED', payload: false , [])})}
                    ...
                    <InputField
                      ...
                      onFocus={useCallback(() => dispatch({ type: 'SET_EMAIL_FOCUSED', payload: true , [])})}
                      onBlur={useCallback(() => dispatch({ type: 'SET_EMAIL_FOCUSED', payload: false , [])})}
                    ...
                    <InputField
                      ...
                      onBlur={useCallback(() => dispatch({ type: 'SET_PASSWORD_FOCUSED', payload: false , [])})}
                    ...
                    {state.breachedWarning && (
                      <View style={[st.lockoutBanner, { backgroundColor: C.errorGlow, borderColor: C.error, marginBottom: 12 }]}>
                        ...
                        <Text style={[st.lockoutText, { color: C.error }]}>
                    ...
                    <InputField
                      ...
                      onBlur={useCallback(() => dispatch({ type: 'SET_CONFIRM_FOCUSED', payload: false , [])})}
                    ...
                    {IS_WEB && state.capsLockOn && (state.passwordFocused || state.confirmFocused) && (
                      <View
                        style={[st.capsLockRow, { backgroundColor: C.errorGlow, borderColor: C.warn }]}
                        accessibilityLiveRegion="polite"
                      >
                  ...
                  <Animated.View style={[st.footer, footerStyle]}>
                    <Text style={[st.legalText, { color: C.textMuted }]}>
                      By creating an account, you agree to our{' '}
                      <Text style={[st.legalLink, { color: C.accentSoft }]} onPress={handleTerms} accessibilityRole="link">
                        Terms of Service
                      </Text>
                      {' and '}
                      <Text style={[st.legalLink, { color: C.accentSoft }]} onPress={handlePrivacy} accessibilityRole="link">
                        Privacy Policy
                      </Text>
                    </Text>
                    <Pressable
                      onPress={handleLogin}
                      disabled={state.loading}
                      style={({ pressed }) = accessibilityLabel="button"> [st.loginButton, { opacity: pressed ? 0.7 : 1 }, webCursorStyle]}
                    >
                      <Text style={[st.loginText, { color: C.textSecondary }]}>
            ...
            {state.loading && !showSuccess && (
              ...
              >
                <View
                  style={[st.loadingCard, { backgroundColor: C.card, borderColor: C.cardBorder }]}
                  accessibilityRole="alert"
                >
...
const st = StyleSheet.create(() => ({
  ...
  gradientInner:   { borderRadius: 16 },   // extracted from inline style={{ borderRadius: 16 }}
