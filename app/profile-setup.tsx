...
type Action =
  ...
  | {
      type: 'TOGGLE_LIST';
      field: 'interests' | 'dealbreakers' | 'vibes' | 'importantFields';
      value: string;
      max?: number;
    }
...
export default function ProfileSetupScreen() {
  ...
  const goNext = useCallback(() => {
    if (!stepOk) {
      const msg = getMissingFieldsMessage(step, form, hasFace, hasUpperBody, age, hCm);
  ...
  const openCamera = useCallback(
    async (slot?: PhotoSlotConfig) => {
      ...
      if (
        targetSlot.type !== 'freestyle' &&
        form.photos.some((p) => p.type === targetSlot.type)
      ) {
        Alert.alert(
          'Already Added',
          `You already have a ${targetSlot.label} photo. Remove it first to retake.`
        );
        return;
      }
    ...
    if (form.ageEstimate && Math.abs(age - form.ageEstimate) > AGE_TOL) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Continue', onPress: () => void doSave() },
        ]
      );
      return;
    }
