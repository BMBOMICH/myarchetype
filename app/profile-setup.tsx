...
// ─── Web-safe alert (Alert.alert is broken inside Modal on mobile web) ───
function showAlert(
  title: string,
  message?: string,
  ...
  if (!IS_WEB) {
    Alert.alert(title, message, buttons as any, _options);
    return;
  }
  ...
  const confirmed = (globalThis as any).confirm(msg);
  if (confirmed) {
...
function getZodiac(m: number, d: number): ZodiacResult {
  for (const z of ZODIAC_DATA) { if ((m === z.s[0] && d >= z.s[1]) || (m === z.e[0] && d <= z.e[1])) return { sign: z.sign, emoji: z.emoji }; }
...
function checkBlocked(text: string): string | null {
  for (const r of BLOCKED_RE) { if (r.test(text)) { if (r.source.includes('@') || r.source.includes('\\d')) return 'Contact information is not allowed.'; if (r.source.includes('snap')) return 'Social media handles are not allowed.'; return 'This contains inappropriate language.'; } }
  return null;
}
function getMissingFieldsMessage(step: number, form: FormState, hasFace: boolean, hasUpperBody: boolean, age: number | null, hCm: number): string {
  switch (step) {
    ...
    case 3: return 'Select your body type and preference'; case 4: return 'Select religion, lifestyle and relationship goal'; case 5: return 'Pick at least 3 interests'; case 8: return 'Accept the Terms of Service to continue'; default: return 'Complete required fields';
  }
}

const INIT: FormState = { photos: [], name: '', bdayMonth: '', bdayDay: '', bdayYear: '', gender: '', interestedIn: '', pronouns: '', heightCm: '', heightFt: '', heightIn: '', heightUnit: 'cm', bodyType: '', lookingForBody: '', religion: '', lifestyle: '', relationship: '', education: '', occupation: '', smoking: '', drinking: '', children: '', pets: '', diet: '', politics: '', interests: [], loveLang: '', commStyle: '', firstDate: '', vibes: [], ageMin: '18', ageMax: '50', distKm: '50', heightPrefMinCm: '', heightPrefMaxCm: '', dealbreakers: [], importantFields: [], bio: '', prompts: [], locCity: '', locData: null, ageEstimate: null, blurUntilMatch: false, incognito: false, verifiedOnly: false, termsAccepted: false };
...
const WebVideoPreview = React.memo(function WebVideoPreview({ facing, onReady }: { streamReady?: boolean; facing: 'front' | 'back'; onReady: (el: any) => void }) {
  if (!IS_WEB) return null;
  return (
    <View style={{ flex: 1 }} pointerEvents="none">
      {/* @ts-ignore web only */}
      <video ref={(node: any) => { if (node) onReady(node); }} autoPlay playsInline muted
        style={{ width: '100%', height: '100%', objectFit: 'cover' as any, display: 'block', transform: facing === 'front' ? 'scaleX(-1)' : 'none', pointerEvents: 'none', touchAction: 'none' } as any} />
    </View>
  );
});
...
export default function ProfileSetupScreen() {
  ...
  useEffect(() => { const u = onAuthStateChanged(auth, (u) => { if (u) { setUserId(u.uid); setUserEmail(u.email); } else router.replace('/login' as any); }); return u; }, [router]);
  ...
  useEffect(() => { return () => { if (streamRef.current) { streamRef.current.getTracks().forEach((t: any) => t.stop()); streamRef.current = null; } if (countdownRef.current) clearInterval(countdownRef.current); if (readyPollRef.current) clearInterval(readyPollRef.current); }; }, []);
  ...
  useEffect(() => { if (!draftKey || !stepKey) return; (async () => { try { const r = profileStorage.getString(draftKey) ?? null; if (r) { const p = JSON.parse(r); delete p.photos; if (isMountedRef.current) dispatch({ type: 'LOAD', state: p }); } if (isMountedRef.current) setStep(1); } catch { } })(); }, [draftKey, stepKey]);

  const prevFormRef = useRef(form);
  useEffect(() => { if (prevFormRef.current !== form) { isDirtyRef.current = true; prevFormRef.current = form; } }, [form]);
  useEffect(() => { if (!draftKey || !stepKey) return; const t = setTimeout(() => { if (!isDirtyRef.current) return; isDirtyRef.current = false; const { photos: _, ...rest } = form; try { profileStorage.set(draftKey, JSON.stringify(rest)); } catch { } try { profileStorage.set(stepKey, String(step)); } catch { } }, 2000); return () => clearTimeout(t); }, [form, step, draftKey, stepKey]);
  ...
  useEffect(() => { if (!IS_WEB && !permission?.granted && permission?.canAskAgain !== false) showAlert('Camera Required', 'This app uses your camera to take profile photos.', [{ text: 'Not Now', style: 'cancel' }, { text: 'Grant Access', onPress: () => requestPermission() }]); }, [permission?.granted, permission?.canAskAgain, requestPermission]);
  ...
  const animate = useCallback((dir: 'fwd' | 'back') => { const tv = dir === 'fwd' ? -screenWidth : screenWidth; Animated.parallel([Animated.timing(fadeAnim, { toValue: 0, duration: 140, useNativeDriver: true }), Animated.timing(slideAnim, { toValue: tv, duration: 140, useNativeDriver: true })]).start(() => { slideAnim.setValue(dir === 'fwd' ? screenWidth : -screenWidth); Animated.parallel([Animated.timing(fadeAnim, { toValue: 1, duration: 200, useNativeDriver: true }), Animated.timing(slideAnim, { toValue: 0, duration: 200, useNativeDriver: true })]).start(); }); }, [fadeAnim, slideAnim, screenWidth]);
  ...
  const goNext = useCallback(() => {
    ...
    haptic(); successHaptic(); animate('fwd'); setStep(s => s + 1); requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: 0, animated: false }));
  ...
  const goBack = useCallback(() => {
    ...
    haptic(); animate('back'); setStep(s => s - 1); requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: 0, animated: false }));
  }, [step, haptic, animate, router]);

  const stopWebStream = useCallback(() => { if (readyPollRef.current) { clearInterval(readyPollRef.current); readyPollRef.current = null; } if (streamRef.current) { streamRef.current.getTracks().forEach((t: any) => t.stop()); streamRef.current = null; } if (isMountedRef.current) setCamReady(false); }, []);

  const attachStreamToVideo = useCallback(() => {
    const video = webVideoElRef.current, stream = streamRef.current;
    if (!video || !stream) return;
    if (video.srcObject === stream) { if (video.readyState >= 2 && isMountedRef.current) setCamReady(true); return; }
    ...
    stream.getTracks().forEach((t: any) => { t.onended = () => { if (isMountedRef.current) { setCamReady(false); setCamErr('Camera disconnected.'); } }; });
  ...
  const handleVideoRef = useCallback((el: any) => { if (!el) { if (webVideoElRef.current) { webVideoElRef.current.onloadedmetadata = null; webVideoElRef.current.oncanplay = null; webVideoElRef.current.onerror = null; webVideoElRef.current.srcObject = null; } webVideoElRef.current = null; return; } webVideoElRef.current = el; attachStreamToVideo(); }, [attachStreamToVideo]);

  const startWebStream = useCallback(async (facing: 'front' | 'back') => {
    try { stopWebStream(); if (!IS_WEB) return; const nav = navigator as any; if (!nav.mediaDevices?.getUserMedia) { if (isMountedRef.current) setCamErr('Camera not supported.'); return; }
      const devs = await nav.mediaDevices.enumerateDevices(); const vd = devs.filter((d: any) => d.kind === 'videoinput');
      ...
      if (!isMountedRef.current) { s.getTracks().forEach((t: any) => t.stop()); return; }
      ...
      if (n === 'NotAllowedError') msg = 'Camera access blocked. Allow it in browser settings.';
      else if (n === 'NotFoundError') msg = 'No camera found.';
      else if (n === 'NotReadableError') msg = 'Camera in use by another app.';
      else if (n === 'OverconstrainedError') { try { const s = await (navigator as any).mediaDevices.getUserMedia({ video: true, audio: false }); if (!isMountedRef.current) { s.getTracks().forEach((t: any) => t.stop()); return; } streamRef.current = s; attachStreamToVideo(); return; } catch { msg = 'Could not start camera.'; } }
      if (isMountedRef.current) setCamErr(msg);
    }
  }, [stopWebStream, attachStreamToVideo]);

  const closeCam = useCallback(() => { stopWebStream(); webVideoElRef.current = null; if (countdownRef.current) { clearInterval(countdownRef.current); countdownRef.current = null; } if (isMountedRef.current) { setCamOpen(false); setCamErr(null); setCamSlot(null); setCountdown(null); setTimerEnabled(false); setCapturing(false); capturingRef.current = false; } }, [stopWebStream]);

  const openCamera = useCallback(async (slot?: PhotoSlotConfig) => {
    const ts = slot ?? nextSlot; if (!ts) { showAlert('Maximum Photos', `You can add up to ${MAX_PHOTOS} photos.`); return; }
    ...
    if (!IS_WEB && !permission?.granted) { const r = await requestPermission(); if (!r.granted) { showAlert('Camera Required', 'Enable camera in device settings.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Open Settings', onPress: () => Linking.openSettings() }]); return; } }
    ...
    if (IS_WEB) setTimeout(() => { if (isMountedRef.current) startWebStream(ts.cameraSide); }, 500);
  ...
  const doCapture = useCallback(async () => {
    ...
    try {
      if (IS_WEB) {
        ...
        const doc2 = (globalThis as any).document; if (!doc2) { showAlert('Browser Error', 'Cannot access document.'); return; }
        const vw = v.videoWidth, vh = v.videoHeight; if (!vw || !vh) { showAlert('Camera Not Ready', 'Video dimensions not available.'); return; }
    ...
    } catch (err) { logger.error('doCapture failed:', err); showAlert('Error', 'Something went wrong capturing the photo.'); }
    finally { capturingRef.current = false; if (isMountedRef.current) setCapturing(false); }
  ...
  const handleCapture = useCallback(() => {
    ...
    if (timerEnabled && camSlot?.timerAvailable) {
      setCountdown(TIMER_SECONDS); let c = TIMER_SECONDS;
      countdownRef.current = setInterval(() => { c--; if (c <= 0) { if (countdownRef.current) clearInterval(countdownRef.current); countdownRef.current = null; if (isMountedRef.current) setCountdown(null); void doCapture(); } else if (isMountedRef.current) setCountdown(c); }, 1000);
    } else void doCapture();
  }, [countdown, timerEnabled, camSlot, doCapture]);

  const removePhoto = useCallback((i: number) => { const p = form.photos[i]; if (!p) return; showAlert('Remove Photo', p.type === 'face' || p.type === 'upper_body' ? `Removing ${getPhotoLabel(p.type)} will make Step 1 incomplete.` : 'Remove this photo?', [{ text: 'Cancel', style: 'cancel' }, { text: 'Remove', onPress: () => { dispatch({ type: 'REMOVE_PHOTO', index: i }); haptic(); } }]); }, [form.photos, haptic]);
  ...
  const getLoc = useCallback(async () => { showAlert('Location Access', 'Only your city is shown to other users.', [{ text: 'Not Now', style: 'cancel' }, { text: 'Enable', onPress: async () => { if (isMountedRef.current) setGettingLoc(true); try { const loc = await requestLocationPermission(); if (loc) { const d = loc.city ? `${loc.city}, ${loc.country}` : 'Location found'; if (isMountedRef.current) { set('locCity', d); set('locData', loc); } await saveUserLocation(loc); showAlert('📍 Location Set', d); } else showAlert('Location Error', 'Enable location in settings.'); } catch { showAlert('Location Error', 'Something went wrong.'); } finally { if (isMountedRef.current) setGettingLoc(false); } }}]); }, [set]);

  const switchHeightUnit = useCallback(() => { const { newFt, newIn, newCm } = convertHeightForUnitSwitch(form.heightUnit, form.heightCm, form.heightFt, form.heightIn); dispatch({ type: 'SET', field: 'heightUnit', value: form.heightUnit === 'cm' ? 'ft' : 'cm' }); dispatch({ type: 'SET', field: 'heightFt', value: newFt }); dispatch({ type: 'SET', field: 'heightIn', value: newIn }); dispatch({ type: 'SET', field: 'heightCm', value: newCm }); }, [form.heightUnit, form.heightCm, form.heightFt, form.heightIn]);

  const doSave = useCallback(async () => {
    if (!userId || !birthday || !age) return; if (isMountedRef.current) setLoading(true);
    try {
      const e2 = await ensureMyE2EEIdentity(); if (!e2.success || !e2.publicKey) throw new Error(e2.error ?? 'Encryption identity failed');
      ...
      dispatch({ type: 'RESET' });
      showAlert('🎉 Profile Created!', 'Next: discover your personality type!', [{ text: 'Continue', onPress: () => router.replace('/personality-quiz' as any) }]);
    } catch (err: any) { logger.error('doSave failed:', err?.message); showAlert('Save Error', `Could not save: ${err?.message || 'Unknown error'}`); }
  ...
  const handleSave = useCallback(async () => {
    ...
    <TouchableOpacity key={value} style={[st.chip, { borderColor: selected ? C.accent : C.inputBorder, backgroundColor: selected ? C.accentGlow : C.input }, disabled && st.chipOff]} onPress={() => { haptic(); onPress(); }} disabled={disabled || loading || uploading} activeOpacity={0.7}>
  ...
  const renderOpt = useCallback((opt: OptionItem, sel: string, onSel: (v: string) => void) => (
    <TouchableOpacity key={opt.value} style={[st.optRow, { backgroundColor: C.input, borderColor: sel === opt.value ? C.accent : C.inputBorder }]} onPress={() => { haptic(); onSel(opt.value); }} disabled={loading || uploading} activeOpacity={0.7}>
  ...
  const renderStep1 = useCallback(() => (
    <View>
      ...
      {uploading && (<View style={[st.loadRow, { backgroundColor: C.input }]}><ActivityIndicator size="small" color={C.accent} /><View style={{ flex: 1, marginLeft: SPACING.md }}><Text style={[st.loadRowText, { color: C.accent }]}>Uploading & verifying… {uploadProgress}%</Text><View style={[st.uploadBarBg, { backgroundColor: C.inputBorder }]}><View style={[st.uploadBarFill, { width: `${uploadProgress}%` as any, backgroundColor: C.accent }]} /></View></View></View>)}
      <View style={st.photoGrid}>
        ...
        {nextSlot && (<TouchableOpacity style={[st.addBtn, { borderColor: C.accent, backgroundColor: C.accentGlow }, (uploading || loading) && st.addBtnOff]} onPress={() => void openCamera()} disabled={uploading || loading} activeOpacity={0.7}><Text style={st.addBtnIcon}>{nextSlot.icon}</Text><Text style={[st.addBtnLabel, { color: C.accent }]}>{nextSlot.label}</Text>{nextSlot.required && <Text style={[st.addBtnReq, { color: C.warning }]}>Required</Text>}</TouchableOpacity>)}
      </View>
      {!hasFullBody && hasFace && hasUpperBody && <TouchableOpacity style={[st.tipBox, { backgroundColor: C.accentGlow, borderColor: C.accent }]} onPress={() => void openCamera(PHOTO_SLOTS[2])} activeOpacity={0.7}><Text style={[st.tipText, { color: C.accent }]}>💡 Add a full-body photo for 40% more matches!</Text></TouchableOpacity>}
  ...
  const renderStep2 = useCallback(() => (
    <View>
      ...
      <View style={st.fg}>
        ...
        <TextInput style={[st.input, { backgroundColor: C.input, color: C.text, borderColor: C.inputBorder }, form.name.length > 0 && !validateName(form.name).valid && { borderColor: C.danger }, validateName(form.name).valid && { borderColor: C.success }]} placeholder="Sarah" placeholderTextColor={C.muted} value={form.name} onChangeText={t => set('name', t.replace(/[^a-zA-Z\s\-']/g, ''))} onBlur={() => { if (form.name) set('name', formatName(form.name)); }} editable={!loading} maxLength={MAX_NAME} autoCapitalize="words" autoCorrect={false} />
  ...
  const renderStep3 = useCallback(() => (<View><Text style={[st.title, { color: C.accent }]}>💪 Body & Appearance</Text><BodyTypeSelector label="Your Body Type *" selectedType={form.bodyType as any} onSelect={v => set('bodyType', v)} disabled={loading} /><View style={st.spacer} /><BodyTypeSelector label="Body Type Preference *" selectedType={form.lookingForBody as any} onSelect={v => set('lookingForBody', v)} disabled={loading} showLookingFor /></View>), [C, form.bodyType, form.lookingForBody, loading, set]);
  ...
  const renderStep4 = useCallback(() => (
    <View>
      ...
      <View style={st.fg}><Text style={[st.label, { color: C.text }]}>Occupation</Text><TextInput style={[st.input, { backgroundColor: C.input, color: C.text, borderColor: C.inputBorder }]} placeholder="Software Engineer…" placeholderTextColor={C.muted} value={form.occupation} onChangeText={t => set('occupation', t)} editable={!loading} maxLength={50} autoCapitalize="words" /><Text style={[st.charCt, { color: form.occupation.length >= 45 ? C.warning : C.muted }]}>{form.occupation.length}/50</Text></View>
  ...
  const renderStep5 = useCallback(() => (
    <View>
      ...
      <View style={st.fg}><Text style={[st.label, { color: C.text }]}>Your Vibes</Text><Text style={[st.hint, { color: C.muted }]}>Pick up to 3</Text><View style={st.vibeGrid}>{VIBE_EMOJIS.map((e, idx) => { const sel = form.vibes.includes(e); const mx = !sel && form.vibes.length >= 3; return (<TouchableOpacity key={`v_${idx}`} style={[st.vibeItem, { backgroundColor: C.input, borderColor: sel ? C.accent : C.inputBorder }, sel && { backgroundColor: C.accentGlow }, mx && st.chipOff]} onPress={() => { haptic(); dispatch({ type: 'TOGGLE_LIST', field: 'vibes', value: e, max: 3 }); }} disabled={mx} activeOpacity={0.7}><Text style={st.vibeEmoji}>{e}</Text></TouchableOpacity>); })}</View></View>
    </View>
    ...
    <View>
      ...
      <View style={[st.privacyCard, { backgroundColor: C.card, borderColor: C.cardBorder }]}>
        ...
        {([{ key: 'blurUntilMatch' as const, label: '🔵 Blur photos until match', desc: 'Photos blur until you match.', val: form.blurUntilMatch }, { key: 'incognito' as const, label: '👻 Incognito mode', desc: 'Only people you like first see you.', val: form.incognito }, { key: 'verifiedOnly' as const, label: '✅ Verified users only', desc: 'Only verified users discover you.', val: form.verifiedOnly }] as const).map(pi => (
          <View key={pi.key} style={[st.privRow, { borderBottomColor: C.inputBorder }]}><View style={st.privInfo}><Text style={[st.privLabel, { color: C.text }]}>{pi.label}</Text><Text style={[st.privDesc, { color: C.muted }]}>{pi.desc}</Text></View><Switch value={pi.val} onValueChange={v => set(pi.key, v)} trackColor={{ false: C.inputBorder, true: C.accent }} thumbColor={pi.val ? C.success : C.dim} /></View>
        ))}
      </View>
      ...
      <View style={[st.preview, { backgroundColor: C.card, borderColor: C.cardBorder }]}>
        {form.photos.length > 0 && <View><ScrollView horizontal showsHorizontalScrollIndicator={false} style={st.previewPhotoScroll}>{form.photos.map((p, i) => <Image key={`pv_${i}`} source={{ uri: p.uri }} style={[st.previewThumb, i === 0 && st.previewThumbMain, form.blurUntilMatch && { opacity: 0.15 }]} contentFit="cover" transition={150} />)}</ScrollView>{form.blurUntilMatch && <View style={st.blurOverlay}><Text style={[st.blurText, { color: C.accent }]}>🔒 Blurred until match</Text></View>}</View>}
  ...
  const renderCurrent = useCallback(() => {
    switch (step) { case 1: return renderStep1(); case 2: return renderStep2(); case 3: return renderStep3(); case 4: return renderStep4(); case 5: return renderStep5(); case 6: return renderStep6(); case 7: return renderStep7(); case 8: return renderStep8(); default: return null; }
    ...
    <KeyboardAvoidingView style={[st.root, { backgroundColor: C.bg }]} behavior={IS_IOS ? 'padding' : 'height'}>
      ...
      <View style={[st.stepDots, { backgroundColor: C.card }]}>{STEP_NAMES.map((name, i) => <View key={name} style={[st.stepDot, { backgroundColor: C.inputBorder }, i + 1 < step && { backgroundColor: C.success }, i + 1 === step && { backgroundColor: C.accent, transform: [{ scale: 1.3 }] }]} />)}</View>
      <View style={[st.progBg, { backgroundColor: C.inputBorder }]}><Animated.View style={[st.progFill, { backgroundColor: C.accent, width: progAnim.interpolate({ inputRange: [0, 1], outputRange: ['0%', '100%'] }) }]} /></View>
      {IS_WEB ? (
        <View style={{ flex: 1 }}><ScrollView ref={scrollRef} style={st.sv} contentContainerStyle={st.svContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Animated.View style={{ opacity: fadeAnim, transform: [{ translateX: slideAnim }] }}>{renderCurrent()}</Animated.View><View style={{ height: SPACING.xl }} /></ScrollView></View>
      ) : (
        <Pressable style={{ flex: 1 }} onPress={() => Keyboard.dismiss()}><ScrollView ref={scrollRef} style={st.sv} contentContainerStyle={st.svContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Animated.View style={{ opacity: fadeAnim, transform: [{ translateX: slideAnim }] }}>{renderCurrent()}</Animated.View><View style={{ height: SPACING.xl }} /></ScrollView></Pressable>
      )}
      <View style={[st.botBar, { backgroundColor: C.card, borderTopColor: C.cardBorder }]}>
        {step < TOTAL_STEPS ? (stepOk ? (
          <TouchableOpacity style={st.nextBtnWrap} onPress={goNext} activeOpacity={0.85}><LinearGradient colors={[C.buttonGradStart, C.buttonGradEnd] as [string, string]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={st.nextBtn}><Text style={[st.nextBtnText, { color: C.white }]}>Next → {STEP_NAMES[step]}</Text></LinearGradient></TouchableOpacity>
        ) : (
          ...
          <TouchableOpacity style={st.nextBtnWrap} onPress={() => void handleSave()} activeOpacity={0.85}><LinearGradient colors={[C.success, '#3aaa50'] as [string, string]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={st.nextBtn}><Text style={[st.nextBtnText, { color: C.white }]}>✓ Create Profile</Text></LinearGradient></TouchableOpacity>
        ) : (
      ...
      <Modal visible={camOpen} animationType="slide" onRequestClose={closeCam} statusBarTranslucent>
        <View style={[st.camModal, { backgroundColor: C.bg }]}>
          ...
          <View style={[st.camHead, { backgroundColor: C.card, borderBottomColor: C.cardBorder }]}>
            <TouchableOpacity onPress={closeCam} activeOpacity={0.7} disabled={capturing} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Text style={[st.camCancel, { color: C.danger }]}>✕ Cancel</Text></TouchableOpacity>
            ...
            <TouchableOpacity onPress={() => { if (capturing || countdown !== null) return; handleCapture(); }} activeOpacity={0.7} style={{ backgroundColor: C.accent, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20 }}>
              <Text style={{ color: C.white, fontWeight: '700', fontSize: 14 }}>{capturing ? '⏳ Wait...' : '📸 Capture'}</Text>
            </TouchableOpacity>
          </View>
          <View style={st.camContent} pointerEvents="box-none">
            {IS_WEB ? (
              <View style={[st.camBox, { borderColor: C.accent }]}>
                {camErr ? (
                  <View style={st.camErrWrap}><Text style={st.camErrIcon}>📷</Text><Text style={[st.camErrText, { color: C.danger }]}>{camErr}</Text>
                    <TouchableOpacity style={[st.retryBtn, { backgroundColor: C.accent }]} onPress={() => { if (isMountedRef.current) { setCamErr(null); setCamReady(false); } if (camSlot) startWebStream(camSlot.cameraSide); }} activeOpacity={0.7}><Text style={[st.retryBtnText, { color: C.white }]}>Try Again</Text></TouchableOpacity></View>
                ) : (
                  <>
                    {!camReady && <View style={[StyleSheet.absoluteFillObject, st.camLoadWrap]} pointerEvents="none"><ActivityIndicator size="large" color={C.accent} /><Text style={[st.camLoadText, { color: C.muted }]}>Starting camera…</Text></View>}
                    <View style={StyleSheet.absoluteFillObject} pointerEvents="none"><WebVideoPreview streamReady={!!streamRef.current} facing={camFacing} onReady={handleVideoRef} /></View>
                    {camReady && camSlot && <View style={StyleSheet.absoluteFillObject} pointerEvents="none"><CameraGuide type={camSlot.type} C={C} /></View>}
          ...
          <View style={[st.camControls, { backgroundColor: C.card, borderTopColor: C.cardBorder }]}>
            {camSlot?.timerAvailable && <TouchableOpacity style={[st.timerBtn, { backgroundColor: C.input, borderColor: timerEnabled ? C.accent : C.inputBorder }, timerEnabled && { backgroundColor: C.accentGlow }, capturing && st.btnOff]} onPress={() => { setTimerEnabled(v => !v); haptic(); }} disabled={capturing} activeOpacity={0.7}><Text style={[st.timerBtnText, { color: timerEnabled ? C.accent : C.text }]}>{timerEnabled ? `⏱ ${TIMER_SECONDS}s ON` : '⏱ Timer'}</Text></TouchableOpacity>}
            <View style={st.camBtnRow}>
              <TouchableOpacity style={[st.flipBtn, { backgroundColor: C.input, borderColor: C.inputBorder }, capturing && st.btnOff]} onPress={flipCamera} disabled={capturing} activeOpacity={0.7}><Text style={st.flipBtnText}>🔄</Text></TouchableOpacity>
              <TouchableOpacity style={[st.captureBtn, { borderColor: C.accent }, (capturing || countdown !== null) && st.captureBtnOff]} onPress={handleCapture} activeOpacity={0.8}>
      ...
      <Modal visible={promptPicker !== null} animationType="slide" transparent onRequestClose={() => setPromptPicker(null)}>
        <View style={st.pickerOverlay}>
          <View style={[st.pickerContent, { backgroundColor: C.card, borderColor: C.cardBorder }]}>
            ...
            <FlatList data={PROMPT_QUESTIONS} keyExtractor={(_, i) => `pq_${i}`} showsVerticalScrollIndicator={false} renderItem={({ item: q }) => { const used = promptPicker !== null && promptPicker < form.prompts.length && form.prompts.some((p, i) => p.q === q && i !== promptPicker); return (<TouchableOpacity style={[st.pickerItem, { borderBottomColor: C.inputBorder }, used && st.pickerItemOff]} onPress={() => { if (used || promptPicker === null || promptPicker >= form.prompts.length) return; dispatch({ type: 'SET_PROMPT', index: promptPicker, q, a: form.prompts[promptPicker]?.a ?? '' }); setPromptPicker(null); }} disabled={used} activeOpacity={0.7}><Text style={[st.pickerItemText, { color: used ? C.muted : C.text }]}>{q}</Text>{used && <Text style={[st.pickerUsed, { color: C.muted }]}>Already used</Text>}</TouchableOpacity>); }} />
            <TouchableOpacity style={st.pickerCancel} onPress={() => setPromptPicker(null)} activeOpacity={0.7}><Text style={[st.pickerCancelText, { color: C.danger }]}>Cancel</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
    </KeyboardAvoidingView>
...
const st = StyleSheet.create({
  ...
  chipOn: {}, chipOff: { opacity: 0.3 }, chipIcon: { fontSize: FONT.lg }, chipText: { fontSize: FONT.base }, chipCheck: { fontSize: FONT.base, fontWeight: 'bold' },
  ...
  loadRow: { flexDirection: 'row', alignItems: 'center', padding: SPACING.md + 2, borderRadius: RADIUS.md, marginBottom: SPACING.md + 2, gap: SPACING.sm }, loadRowText: { fontSize: FONT.base },
  uploadBarBg: { height: 4, borderRadius: RADIUS.sm, overflow: 'hidden', marginTop: SPACING.xs + 2, flex: 1 }, uploadBarFill: { height: '100%', borderRadius: RADIUS.sm },
  ...
  moveBtn: { borderRadius: RADIUS.md, width: 22, height: 22, justifyContent: 'center', alignItems: 'center' }, moveBtnText: { fontSize: FONT.md, fontWeight: 'bold' },
  ...
  previewPhotoScroll: { height: 180 }, previewThumb: { width: 140, height: 180, marginRight: SPACING.xxs + 2 }, previewThumbMain: { width: 180 },
  blurOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(26,26,46,0.75)', justifyContent: 'center', alignItems: 'center' }, blurText: { fontSize: FONT.xl, fontWeight: 'bold' },
  ...
  countdownOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 80, justifyContent: 'center', alignItems: 'center', zIndex: 20 } as any,
