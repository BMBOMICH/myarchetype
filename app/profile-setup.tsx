...
function showAlert(title: string, message?: string, buttons?: AlertButton[], _options?: Record<string, unknown>) {
  ...
  if (!buttons || buttons.length <= 1) {
    (globalThis as Record<string, unknown>)['alert']?.(msg);
    buttons?.[0]?.onPress?.();
    return;
  }
  const confirmed = (globalThis as Record<string, unknown>)['confirm']?.(msg);
  if (confirmed) {
    const btn = buttons.find((b) => b.style !== 'cancel') ?? buttons[buttons.length - 1];
    btn?.onPress?.();
  } else {
...
function convertHeightForUnitSwitch(cu: HeightUnit, hc: string, hf: string, hi: string) {
  if (cu === 'cm') {
    const cm = parseInt(hc) || 0;
    if (cm >= MIN_H) { const ti = cm / 2.54; return { newFt: String(Math.floor(ti / 12)), newIn: String(Math.round(ti % 12)), newCm: hc }; }
  } else {
    const cm = ftToCm(parseInt(hf) || 0, parseInt(hi) || 0);
    if (cm >= MIN_H) return { newFt: hf, newIn: hi, newCm: String(cm) };
  }
...
const WebVideoPreview = React.memo(function WebVideoPreview({
  facing, onReady,
}: { streamReady?: boolean; facing: 'front' | 'back'; onReady: (el: WebVideoElement) => void }) {
...
export default function ProfileSetupScreen() {
  ...
  const startWebStream = useCallback(async (facing: 'front' | 'back') => {
    try {
      ...
      const s    = await nav.mediaDevices.getUserMedia({
        video: { facingMode: vd.length > 1 ? (facing === 'front' ? 'user' : 'environment') : undefined, width: { ideal: 1280 }, height: { ideal: 960 } },
        audio: false,
      });
    ...
    try {
      ...
      if (type === 'face') {
        ...
        if (faceResult.faceCount !== -1) {
          ...
          if (!faceResult.ok) {
            showAlert('Face Check Failed', faceResult.reason + '\n\nTips:\n• Look directly at the camera\n• Good lighting\n• Remove sunglasses');
            return false;
          }
        }
      }
      ...
      const imageWidth    = upload.width  ?? 0;
      const imageHeight   = upload.height ?? 0;
      ...
      if (type === 'face') {
        try {
          const ar: AgeEstimationResult | null = await estimateAgeFromPhoto(photoUrl);
          if (ar?.estimatedAge && ar.confidence >= 0.1) { if (isMountedRef.current) set('ageEstimate', ar.estimatedAge); }
        } catch { /* non-critical */ }
      }
      ...
      if (type === 'full_body') {
        ...
        if (detectedFaces.length > 0 && imageWidth > 0 && imageHeight > 0) {
          isFullBody = Math.max(...detectedFaces.map(getFaceHeight)) / imageHeight < 0.25;
        }
        if (!isFullBody) {
          try {
            if (detectedFaces.length === 0) { const body = await detectFullBodyPhoto(photoUrl); isFullBody = body.isFullBody; }
          } catch { /* non-critical */ }
          if (!isFullBody) {
  ...
  const doCapture = useCallback(async () => {
    ...
    try {
      if (IS_WEB) {
        ...
        const doc2 = g['document'] as { createElement: (tag: string) => WebCanvasElement } | undefined;
        ...
        {form.photos.map((p, i) => (
          <View key={`ph_${i}`} style={st.photoSlot}>
            ...
            <View style={st.moveRow}>
              {i > 0 && <TouchableOpacity style={[st.moveBtn, { backgroundColor: 'rgba(0,0,0,0.7)' }]} onPress={() => movePhoto(i, i - 1)} accessibilityLabel="Move photo left"  accessibilityRole="button"><Text style={[st.moveBtnText, { color: C.white }]}>←</Text></TouchableOpacity>}
              {i < form.photos.length - 1 && <TouchableOpacity style={[st.moveBtn, { backgroundColor: 'rgba(0,0,0,0.7)' }]} onPress={() => movePhoto(i, i + 1)} accessibilityLabel="Move photo right" accessibilityRole="button"><Text style={[st.moveBtnText, { color: C.white }]}>→</Text></TouchableOpacity>}
            </View>
            <TouchableOpacity style={[st.rmBtn, { backgroundColor: C.danger, borderColor: C.card }]} onPress={() => removePhoto(i)} disabled={uploading || loading} accessibilityLabel={`Remove ${getPhotoLabel(p.type)} photo`} accessibilityRole="button"><Text style={[st.rmBtnText, { color: C.white }]}>×</Text></TouchableOpacity>
          </View>
        ))}
  ...
  const renderStep2 = useCallback(() => (
    <View>
      ...
      <View style={st.fg}>
        ...
        <TextInput
          style={[st.input, { backgroundColor: C.input, color: C.text, borderColor: C.inputBorder }, form.name.length > 0 && !validateName(form.name).valid && { borderColor: C.danger }, validateName(form.name).valid && { borderColor: C.success }]}
  ...
  const renderStep3 = useCallback(() => (
    <View>
      ...
      <BodyTypeSelector label="Your Body Type *"       selectedType={form.bodyType       as BodyType} onSelect={(v) => set('bodyType',       v)} disabled={loading} />
      <View style={st.spacer} />
      <BodyTypeSelector label="Body Type Preference *" selectedType={form.lookingForBody as BodyType} onSelect={(v) => set('lookingForBody', v)} disabled={loading} showLookingFor />
    </View>
  ...
  const renderStep4 = useCallback(() => (
    <View>
      ...
      <TextInput
        ...
        placeholder="Software Engineer…" placeholderTextColor={C.muted} value={form.occupation}
        onChangeText={(t) => { const blocked = checkBlocked(t); if (blocked) { showAlert('Not Allowed', blocked); return; } set('occupation', t); }}
        editable={!loading} maxLength={50} autoCapitalize="words"
        accessibilityLabel="Occupation" accessibilityHint="Enter your job or profession" />
      ...
      <View style={st.fg}>
        ...
        {form.prompts.map((p, i) => (
          <View key={`pr_${i}`} style={[st.promptCard, { backgroundColor: C.input, borderColor: C.inputBorder }]}>
            <TouchableOpacity style={st.promptQ} onPress={() => setPromptPicker(i)} activeOpacity={0.7} accessibilityLabel={p.q || 'Choose a prompt question'} accessibilityRole="button"><Text style={[st.promptQText, { color: C.accent }]}>{p.q || 'Tap to pick a question…'}</Text><Text style={[st.promptArr, { color: C.accent }]}>▼</Text></TouchableOpacity>
            ...
            <TouchableOpacity style={st.promptRm} onPress={() => dispatch({ type: 'DEL_PROMPT', index: i })} accessibilityLabel="Remove prompt" accessibilityRole="button"><Text style={[st.promptRmText, { color: C.danger }]}>✕ Remove</Text></TouchableOpacity>
          </View>
        ))}
        {form.prompts.length < 3 && <TouchableOpacity style={[st.addPrompt, { borderColor: C.accent }]} onPress={() => dispatch({ type: 'ADD_PROMPT' })} activeOpacity={0.7} accessibilityLabel="Add a profile prompt" accessibilityRole="button"><Text style={[st.addPromptText, { color: C.accent }]}>+ Add Prompt</Text></TouchableOpacity>}
      </View>
      <View style={st.fg}>
        <Text style={[st.label, { color: C.text }]}>📍 Location</Text>
        <TouchableOpacity style={[st.locBtn, { backgroundColor: C.input, borderColor: form.locCity !== '' ? C.success : C.inputBorder }, (gettingLoc || loading) && st.btnOff]} onPress={() => void getLoc()} disabled={gettingLoc || loading} activeOpacity={0.7} accessibilityLabel={form.locCity || 'Enable location'} accessibilityRole="button">
          {gettingLoc
        ...
        {form.photos.length > 0 && (
          <View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={st.previewPhotoScroll}>
              {form.photos.map((p, i) => <Image key={`pv_${i}`} source={{ uri: p.uri }} style={[st.previewThumb, i === 0 && st.previewThumbMain, form.blurUntilMatch && { opacity: 0.15 }]} contentFit="cover" transition={150} accessibilityLabel={`${getPhotoLabel(p.type)} preview`} />)}
            </ScrollView>
      ...
      <Modal visible={camOpen} animationType="slide" onRequestClose={closeCam} statusBarTranslucent>
        <View style={[st.camModal, { backgroundColor: C.bg }]}>
          ...
          <View style={[st.camHead, { backgroundColor: C.card, borderBottomColor: C.cardBorder }]}>
            ...
            <TouchableOpacity onPress={() => { if (capturing || countdown !== null) return; handleCapture(); }} activeOpacity={0.7} style={{ backgroundColor: C.accent, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20 }} disabled={capturing || countdown !== null} accessibilityLabel={capturing ? 'Processing photo' : 'Take photo'} accessibilityRole="button">
              ...
              <View style={[st.camBox, { borderColor: C.accent }]}>
                {camErr ? (
                  <View style={st.camErrWrap}>
                    ...
                    <TouchableOpacity style={[st.retryBtn, { backgroundColor: C.accent }]} onPress={() => { if (isMountedRef.current) { setCamErr(null); setCamReady(false); } if (camSlot) void startWebStream(camSlot.cameraSide); }} activeOpacity={0.7} accessibilityLabel="Retry camera" accessibilityRole="button"><Text style={[st.retryBtnText, { color: C.white }]}>Try Again</Text></TouchableOpacity>
                  </View>
                ) : (
                  <>
      ...
      <Modal visible={promptPicker !== null} animationType="slide" transparent onRequestClose={() => setPromptPicker(null)}>
        <View style={st.pickerOverlay}>
          <View style={[st.pickerContent, { backgroundColor: C.card, borderColor: C.cardBorder }]}>
            ...
            />
            <TouchableOpacity style={st.pickerCancel} onPress={() => setPromptPicker(null)} activeOpacity={0.7} accessibilityLabel="Cancel" accessibilityRole="button"><Text style={[st.pickerCancelText, { color: C.danger }]}>Cancel</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
