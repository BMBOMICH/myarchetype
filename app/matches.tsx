...
interface UserProfile {
  ...
  lastSeen?: { toMillis?: () => number }; isOnline?: boolean;
...
interface FilterChipProps { label: string; active: boolean; onPress: () => void; a11yLabel: string; }
const FilterChip = React.memo(function FilterChip({ label, active, onPress, a11yLabel }: FilterChipProps) {
  return (
    <TouchableOpacity style={[s.chip, active && s.chipOn]} onPress={onPress}
      accessibilityLabel={a11yLabel} accessibilityRole="button" accessibilityState={{ selected: active }}>
      <Text style={[s.chipTxt, active && s.chipTxtOn]}>{label}</Text>
    </TouchableOpacity>
  );
});

interface FilterToggleProps { label: string; value: boolean; onPress: () => void; a11yLabel: string; }
const FilterToggle = React.memo(function FilterToggle({ label, value, onPress, a11yLabel }: FilterToggleProps) {
  return (
    <TouchableOpacity style={s.toggleRow} onPress={onPress}
      accessibilityLabel={a11yLabel} accessibilityRole="switch" accessibilityState={{ checked: value }}>
...
export default function MatchesScreen() {
  ...
  const setF = useCallback((patch: Partial<Filters>) => setFilters(f => ({ ...f, ...patch })), []);
  ...
  const applyFilters = useCallback(() => {
    ...
    const result = src.filter(m => {
      ...
      if (cur?.location && u.location && maxDist < 9_999) {
        const d = calculateDistance(cur.location.latitude, cur.location.longitude, u.location.latitude, u.location.longitude);
  ...
  const handleReport = useCallback(() => {
    ...
    const user = auth.currentUser;
    if (!user || !them) return;
    Alert.prompt(`Report ${them.name}`, 'Why are you reporting this user?', async (reason) => {
      if (!reason) return;
      try {
  ...
  const handleBlock = useCallback(() => {
    ...
    const user = auth.currentUser;
    if (!user || !them) return;
    Alert.alert(`Block ${them.name}?`, "They won't be able to see your profile or contact you.", [
  ...
  if (filtered.length === 0) return (
    <View style={s.center}>
      ...
      {matches.length > 0 && (
        <TouchableOpacity style={s.resetBtn} onPress={resetFilters} accessibilityLabel="Reset all filters" accessibilityRole="button">
          <Text style={s.resetBtnText}>Reset Filters</Text>
        </TouchableOpacity>
      )}
      <TouchableOpacity style={s.btn} onPress={() => router.push('/home')} accessibilityLabel="Go back home" accessibilityRole="button">
  ...
  if (idx >= filtered.length) return (
    <View style={s.center}>
      ...
      {lastSkipped && (
        <TouchableOpacity style={s.undoBtn} onPress={handleUndo} accessibilityLabel="Undo last skip" accessibilityRole="button">
      ...
      <TouchableOpacity style={[s.btn, s.viewMatchesBtn]} onPress={() => router.push('/my-matches')} accessibilityLabel="View my matches" accessibilityRole="button">
...
const s = StyleSheet.create({
  ...
  viewMatchesBtn:   { backgroundColor: '#53a8b6', marginTop: 10 },
