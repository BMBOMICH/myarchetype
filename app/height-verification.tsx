...
export default function HeightVerificationScreen() {
  ...
  const capturePhoto = useCallback(async () => {
    try {
      ...
      if (isWeb) {
        ...
        if (!ctx) { Alert.alert('Error', 'Could not access the camera frame'); return; }
  ...
  const runAIEstimation = useCallback(async () => {
    if (!photoData) { Alert.alert('No Photo', 'Please take a photo first'); return; }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const heightData: HeightData = {
        value: heightNum, verificationMethod: method, verifiedAt: new Date().toISOString(),
      };
      ...
      Alert.alert('Success', 'Height verification saved!', [
        ...
        <View style={styles.tutorialContainer}>
          ...
          >
            {uploading
              ...
              : <Text style={styles.uploadButtonText}>{photo ? '📷 Retake Photo' : '📷 Take Measurement Photo'}</Text>
            }
          </TouchableOpacity>
          ...
          {photo && (
            <Image
              ...
              accessibilityLabel="Measurement proof photo preview"
            />
          )}
      ...
      {method === 'ai-estimated' && (
        <View style={styles.aiContainer}>
          ...
          <Text style={styles.aiInstructions}>
            Take a full-body photo standing straight. The AI will estimate your height using face-to-body proportions.
          </Text>
          ...
          <View style={styles.accuracyWarning}>
            ...
            <Text style={styles.accuracyWarningText}>
              AI estimation uses face-to-body proportions and is accurate to approximately ±10cm.
              For exact results, use Manual Measured instead.
            </Text>
          </View>
          ...
          >
            {uploading
              ? <ActivityIndicator size="small" color="#fff" />
              : <Text style={styles.uploadButtonText}>{photo ? '📷 Retake Photo' : '📷 Take Full-Body Photo'}</Text>
            }
          </TouchableOpacity>
          ...
          {photo && (
            <Image
              source={{ uri: photo }}
              style={styles.previewPhoto}
              accessibilityLabel="Full body photo preview for AI estimation"
            />
          )}
      ...
      {cameraOpen && (
        <View style={styles.cameraModal}>
          <Text style={styles.cameraTitle}>
            {captureTarget === 'ai' ? 'Full-Body Photo' : 'Measurement Photo'}
          </Text>
          <Text style={styles.cameraHint}>
            {captureTarget === 'ai' ? 'Stand 2-3m away, full body visible' : 'Show tape measure clearly'}
          </Text>
...
const styles = StyleSheet.create((theme) => ({
  ...
  title:                  { fontSize: 28, fontWeight: 'bold', color: theme.colors.text, marginTop: 20, marginBottom: 10, textAlign: 'center' },
  subtitle:               { fontSize: 14, color: theme.colors.textSecondary, marginBottom: 30, textAlign: 'center' },
  ...
  methodTitle:            { fontSize: 18, fontWeight: 'bold', color: theme.colors.text, textAlign: 'center', marginBottom: 5 },
  methodDesc:             { fontSize: 13, color: theme.colors.textSecondary, textAlign: 'center', marginBottom: 8 },
  ...
  input:                  { backgroundColor: '#16213e', color: theme.colors.text, padding: 15, borderRadius: 10, fontSize: 18, textAlign: 'center' },
  hint:                   { fontSize: 12, color: theme.colors.textSecondary, marginTop: 10, textAlign: 'center' },
  ...
  stepTitle:              { fontSize: 16, fontWeight: '600', color: theme.colors.text, flex: 1 },
  stepDesc:               { fontSize: 14, color: theme.colors.textSecondary, lineHeight: 20, paddingLeft: 42 },
  ...
  aiInstructions:         { fontSize: 14, color: theme.colors.textSecondary, textAlign: 'center', marginBottom: 20, lineHeight: 20 },
  aiTips:                 { backgroundColor: '#16213e', borderRadius: 10, padding: 15, marginBottom: 15 },
  aiTipTitle:             { fontSize: 14, fontWeight: '600', color: theme.colors.text, marginBottom: 10 },
  aiTip:                  { fontSize: 13, color: theme.colors.textSecondary, marginBottom: 6 },
  ...
  aiResultTitle:          { fontSize: 14, color: theme.colors.textSecondary, marginBottom: 10 },
  aiResultHeight:         { fontSize: 40, fontWeight: 'bold', color: '#53a8b6', marginBottom: 5 },
  aiResultConfidence:     { fontSize: 16, color: theme.colors.text, marginBottom: 10 },
  aiResultNote:           { fontSize: 12, color: theme.colors.textSecondary, textAlign: 'center', lineHeight: 18 },
  ...
  cameraModal:            { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: theme.colors.background, justifyContent: 'center', alignItems: 'center', padding: 20, zIndex: 999 },
  cameraTitle:            { fontSize: 22, fontWeight: 'bold', color: theme.colors.text, marginBottom: 8 },
  cameraHint:             { fontSize: 13, color: theme.colors.textSecondary, marginBottom: 20, fontStyle: 'italic' },
  ...
  loadingTextCam:         { color: theme.colors.textSecondary, marginTop: 10 },
