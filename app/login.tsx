...
const validators = {
  ...
  password: (v: string): string => {
    ...
    if (v.length > MAX_PASSWORD_LENGTH) return `Password must be under ${MAX_PASSWORD_LENGTH} characters`;
...
const InputField = React.memo(({
  ...
  const animatedBorderColor = borderAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [hasError ? C.error : C.inputBorder, hasError ? C.error : C.accent],
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[s.successText, { color: C.textPrimary }]} allowFontScaling maxFontSizeMultiplier={1.2}>
          Welcome back!
        </Text>
...
if (!e2eeResult.success) {
  console.warn('E2EE identity sync skipped during login:', e2eeResult.error);
      ...
      showAppAlert('Login Failed', getErrorMessage(error.code) || error?.message || 'Login failed');
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  }, [
    state.email,
  ...
  const handleResendVerification = useCallback(async () => {
    ...
    if (!email) {
      showAppAlert('Email Required', 'Please enter your email address first.');
      return;
    }
    if (!password) {
      showAppAlert('Password Required', 'Please enter your password to verify identity.');
      return;
    }
