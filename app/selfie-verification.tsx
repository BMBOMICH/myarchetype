...
import {
  ActivityIndicator, Alert, InteractionManager, Platform,
...
interface NSFWResult      { safe: boolean; reason?: string; shouldBlur?: boolean; }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'SET_POSES':          return { ...state, poses:         action.payload };
    case 'SET_POSE_INDEX':     return { ...state, poseIndex:     action.payload };
    case 'ADD_PHOTO':          return { ...state, photos: [...state.photos, action.payload.uri], timestamps: [...state.timestamps, action.payload.timestamp] };
    case 'SET_COUNTDOWN':      return { ...state, countdown:     action.payload };
    case 'SET_ERROR':          return { ...state, error:         action.payload };
    case 'SET_ATTEMPTS':       return { ...state, attempts:      action.payload };
    case 'SET_WEB_READY':      return { ...state, webReady:      action.payload };
    case 'SET_CAM_ERROR':      return { ...state, camError:      action.payload };
    case 'SET_STATUS':         return { ...state, statusText:    action.payload };
    case 'SET_COOLDOWN_END':   return { ...state, cooldownEnd:   action.payload };
    case 'SET_NSFW_READY':     return { ...state, nsfwReady:     action.payload };
    case 'SET_FACE_READY':     return { ...state, faceReady:     action.payload };
    case 'SET_LOADING_MODELS': return { ...state, loadingModels: action.payload };
    case 'RESET_FOR_RETRY':    return {
      ...state,
      step: 'camera', error: '', photos: [], timestamps: [],
      poseIndex: 0, poses: action.payload, camError: null,
    };
...
async function verifyPhotoNSFWServer(imageUri: string): Promise<NSFWResult> {
  ...
  return res.json() as Promise<NSFWResult>;
}
...
const ShieldItem = React.memo(({ text }: { text: string }) => (
  <Text style={st.shieldItem}>• {text}</Text>
));

const Thumb = React.memo(({ uri, index }: { uri: string; index: number }) => (
  <TurboImage
    source={{ uri }}
    style={st.thumb}
...
const CamErrorView = React.memo(({ error, onRetry }: { error: string; onRetry: () => void }) => (
  <View style={st.camErrBox}>
    ...
    <TouchableOpacity style={st.retryBtn} onPress={onRetry} accessibilityRole="button" accessibilityLabel="button">
...
export default function SelfieVerificationScreen() {
  ...
  const nsfwMutation = useMutation({
    mutationFn: verifyPhotoNSFWServer,
  });
  ...
  useEffect(() => {
    if (!IS_WEB) { dispatch({ type: 'SET_LOADING_MODELS', payload: false }, []); return; }
    let mounted = true;
    Promise.all([preloadSafetyModel(), loadFaceVerification()]).then(([nsfw, face]) => {
      if (!mounted) return;
      dispatch({ type: 'SET_NSFW_READY',     payload: nsfw });
      dispatch({ type: 'SET_FACE_READY',     payload: face });
  ...
  const stopCamera = useCallback(() => {
    ...
    dispatch({ type: 'SET_WEB_READY', payload: false });
  }, []);
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      if (Date.now() >= state.cooldownEnd!) {
        dispatch({ type: 'SET_COOLDOWN_END', payload: null }, []);
        dispatch({ type: 'SET_ATTEMPTS',     payload: 0 });
      }
    }, 1000);
  ...
  const cooldownStr = useMemo(() => {
    if (!state.cooldownEnd) return '';
    const s = Math.max(0, Math.ceil((state.cooldownEnd - Date.now()) / 1000));
  ...
  const startWebCamera = useCallback(async () => {
    ...
    const nav = IS_WEB && typeof navigator !== 'undefined' ? navigator : null;
    if (!nav?.mediaDevices?.getUserMedia) {
      dispatch({ type: 'SET_CAM_ERROR', payload: 'Camera not supported.' });
      return;
    }
    try {
      ...
      for (const d of devices as MediaDeviceInfo[]) {
        if (d.kind === 'videoinput' && isVirtualCam(d.label)) {
          dispatch({ type: 'SET_CAM_ERROR', payload: `Virtual camera detected: "${d.label}". Use your real device camera.` });
    ...
    try {
      ...
      if (track && isVirtualCam(track.label)) {
        stream.getTracks().forEach(t => t.stop());
        dispatch({ type: 'SET_CAM_ERROR', payload: 'Virtual camera not allowed.' });
        return;
      }
      streamRef.current = stream;
      setTimeout(() => {
        const doc   = IS_WEB && typeof document !== 'undefined' ? document : null;
        const video = doc?.getElementById('selfie-cam') as HTMLVideoElement | null;
        if (!video) return;
        video.srcObject = stream;
        video.onloadedmetadata = () => {
          void video.play().catch(() => {});
          dispatch({ type: 'SET_WEB_READY', payload: true });
        };
      }, 150);
    } catch (e) {
      const msg = e instanceof DOMException
        ...
        : 'Camera error.';
      dispatch({ type: 'SET_CAM_ERROR', payload: msg });
    }
  }, []);

  useEffect(() => {
    if (state.step === 'camera' && IS_WEB) void startWebCamera();
  }, [state.step, startWebCamera]);

  const captureWeb = useCallback((): string | null => {
    const doc   = IS_WEB && typeof document !== 'undefined' ? document : null;
    const video = doc?.getElementById('selfie-cam') as HTMLVideoElement | null;
  ...
  const fail = useCallback(async (reason: string) => {
    ...
    dispatch({ type: 'SET_ATTEMPTS', payload: next });
    dispatch({ type: 'SET_ERROR',    payload: reason });
    if (next >= MAX_ATTEMPTS) {
      dispatch({ type: 'SET_COOLDOWN_END', payload: Date.now() + COOLDOWN_MS });
    }
    await logAudit('safety.content_flagged', {
      type: 'selfie_verification_failed', reason, attempt: next, userId: user?.uid,
    });
    dispatch({ type: 'SET_STEP', payload: 'failed' });
  }, [state.attempts, user?.uid]);

  const verify = useCallback(async (pics: string[], times: number[]) => {
    try {
      if (!user) throw new Error('Not logged in');

      dispatch({ type: 'SET_STATUS', payload: 'Checking capture integrity…' });
      const timing = checkTimings(times);
      if (!timing.ok) { await fail(timing.reason); return; }

      for (let i = 0; i < pics.length; i++) {
        dispatch({ type: 'SET_STATUS', payload: `Scanning photo ${i + 1} (client)…` });
        const clientNSFW = await checkImageSafety(pics[i]!, 'profile');
        if (!clientNSFW.safe) { await fail(clientNSFW.reason); return; }

        dispatch({ type: 'SET_STATUS', payload: `Scanning photo ${i + 1} (server)…` });
        const serverNSFW = await nsfwMutation.mutateAsync(pics[i]!);
      ...
      if (isFaceVerificationReady()) {
        for (let i = 0; i < pics.length; i++) {
          dispatch({ type: 'SET_STATUS', payload: `Detecting face in photo ${i + 1}…` });
        ...
        if (descriptors.length >= 2) {
          dispatch({ type: 'SET_STATUS', payload: 'Checking selfie consistency…' });
          const cons = checkSelfieConsistency(descriptors);
          if (!cons.consistent) { await fail(cons.reason); return; }
        }
        if (pics[0] && IS_WEB) {
          dispatch({ type: 'SET_STATUS', payload: 'Estimating age…' });
          const age = await estimateAgeFromPhoto(pics[0]);
          if (age && age.estimatedAge < 16 && age.confidence > 70) {
            await fail('Age verification failed. Must be 18+.');
            return;
          }
        }
        if (pics[0]) {
          dispatch({ type: 'SET_STATUS', payload: 'Checking eligibility…' });
          const banned = await checkAgainstBannedFaces(pics[0]);
          if (banned.isBanned) { await fail('Account registration not allowed.'); return; }
        }
        if (pics[0]) {
          dispatch({ type: 'SET_STATUS', payload: 'Checking identity…' });
          const celeb = await checkCelebrityImpersonation(pics[0]);
          if (celeb.isCelebrity && celeb.confidence > 85) {
            await fail('Photo matches a public figure. Use your own photo.');
            return;
          }
        }
      }
      ...
      for (let i = 0; i < pics.length; i++) {
        dispatch({ type: 'SET_STATUS', payload: `Uploading ${i + 1}/${pics.length}…` });
        uploads.push(await uploadMutation.mutateAsync(pics[i]!));
      }
      ...
      for (const u of uploads) {
        const d = checkDimensions(u.width, u.height);
        if (!d.ok) { await fail(d.reason); return; }
      }
      for (const u of uploads) {
        const e = checkExifForAI(u.metadata);
        if (e.likelyAI) { await fail(e.reason ?? 'AI-generated image detected.'); return; }
      }

      for (const u of uploads) {
        dispatch({ type: 'SET_STATUS', payload: 'Server NSFW check on upload…' });
        const sv = await nsfwMutation.mutateAsync(u.url);
        if (!sv.safe) { await fail(sv.reason ?? 'Uploaded photo failed server safety check.'); return; }
      }

      dispatch({ type: 'SET_STATUS', payload: 'Checking profile photo…' });
      ...
      if (isFaceVerificationReady() && descriptors.length > 0) {
        dispatch({ type: 'SET_STATUS', payload: 'Comparing selfie to profile…' });
      ...
      await logAudit('user.verify_selfie', {
        matchConf, poseCount: state.poses.length, usedLocal, serverNSFW: !!SERVER_URL,
      });
  ...
  }, [user, state.poses, fail, router, nsfwMutation, uploadMutation]);

  const startVerification = useCallback(async () => {
    if (state.cooldownEnd && Date.now() < state.cooldownEnd) return;
    const newPoses = secureShuffle(ALL_POSES).slice(0, NUM_POSES);
    if (!IS_WEB && !permission?.granted) {
      const res = await requestPermission();
      if (!res.granted) { dispatch({ type: 'SET_ERROR', payload: 'Camera permission required.' }); return; }
    }
    dispatch({ type: 'RESET_FOR_RETRY', payload: newPoses });
  }, [state.cooldownEnd, permission, requestPermission]);

  const capture = useCallback(async () => {
    let uri: string | null = null;
    if (IS_WEB) {
      if (!state.webReady) return;
      uri = captureWeb();
    } else {
    ...
    dispatch({ type: 'ADD_PHOTO', payload: { uri, timestamp: Date.now() } });
    const nextPoseIndex = state.poseIndex + 1;

    if (nextPoseIndex < state.poses.length) {
      dispatch({ type: 'SET_POSE_INDEX', payload: nextPoseIndex });
      return;
    }

    const nextPhotos = [...state.photos, uri];
    const nextTimes  = [...state.timestamps, Date.now()];

    stopCamera();
    dispatch({ type: 'SET_STEP', payload: 'verifying' });

    const task = InteractionManager.runAfterInteractions(() => {
      void verify(nextPhotos, nextTimes);
    });
    return () => task.cancel();
  }, [state.webReady, state.poseIndex, state.poses.length, state.photos, state.timestamps, captureWeb, stopCamera, verify]);

  const autoCapture = useCallback(() => {
    if (IS_WEB && !state.webReady) return;
    dispatch({ type: 'SET_COUNTDOWN', payload: 3 });
    const timer = setInterval(() => {
      dispatch(prev => {
        const next = (prev as unknown as State).countdown;
        if (!next || next <= 1) {
          clearInterval(timer);
          void capture();
          return { type: 'SET_COUNTDOWN', payload: null } as Action;
        }
        return { type: 'SET_COUNTDOWN', payload: next - 1 } as Action;
      });
    }, 1000);
  }, [state.webReady, capture]);

  const currentPose      = state.poses[state.poseIndex];
  const isCooldownActive = !!state.cooldownEnd && Date.now() < state.cooldownEnd;
  const modelStatusStr   = `NSFW: ${state.nsfwReady ? '✅' : '⚠️'}  Face: ${state.faceReady ? '✅' : '⚠️'}  Server: ${SERVER_URL ? '✅' : '⚠️'}`;

  if (state.step === 'intro') return (
    <ScrollView contentContainerStyle={st.scrollContainer} keyboardShouldPersistTaps="handled">
      <Text style={st.title}>Verify Your Identity</Text>
      <View style={st.infoBox}>
        <Text style={st.infoTitle}>What happens:</Text>
        {[
          '1. Camera opens (live only — no gallery)',
          '2. Follow 3 random pose instructions',
          '3. Client + server AI scans for safety + face',
          '4. Face matched to profile photo',
        ].map(t => <Text key={t} style={st.infoItem}>{t}</Text>)}
      </View>
      ...
      {IS_WEB && !state.loadingModels && (
        <View style={st.modelStatusBox}>
          <Text style={st.modelStatusText}>{modelStatusStr}</Text>
        </View>
      )}
      {isCooldownActive
        ? <View style={st.cooldownBox}><Text style={st.cooldownText}>Try again in {cooldownStr}</Text></View>
        : (
          <TouchableOpacity style={st.startBtn} onPress={() = accessibilityLabel="button"> void startVerification()} accessibilityRole="button">
            <Text style={st.startBtnText}>Start Verification</Text>
          </TouchableOpacity>
        )
      }
      <TouchableOpacity style={st.backBtn} onPress={() = accessibilityLabel="button"> router.back()} accessibilityRole="button">
        <Text style={st.backBtnText}>Cancel</Text>
      </TouchableOpacity>
    </ScrollView>
  );

  if (state.step === 'camera') return (
    <View style={st.container}>
      <Text style={st.poseText}>{currentPose?.instruction}</Text>
      <Text style={st.stepText}>{`Step ${state.poseIndex + 1} / ${state.poses.length}`}</Text>
      <Text style={st.poseHint}>Live camera only — no gallery uploads accepted</Text>
      {IS_WEB ? (
        <View style={st.camBox}>
          {state.camError
            ? <CamErrorView error={state.camError} onRetry={() => void startWebCamera()} />
            : !state.webReady
              ? <View style={st.loadBox}><ActivityIndicator size="large" color="#53a8b6" /><Text style={st.loadText}>Starting camera…</Text></View>
              : null
          }
          <View style={state.webReady ? st.videoBox : st.hidden}>
            {React.createElement('video', {
              id: 'selfie-cam', autoPlay: true, playsInline: true, muted: true, style: WEB_VIDEO_STYLE,
            })}
            <View style={st.guideOverlay}>
              <Text style={st.guideIcon}>{currentPose?.icon}</Text>
            </View>
          </View>
          {state.webReady && (
      ...
      {state.countdown !== null
        ? <Text style={st.countdown}>{state.countdown}</Text>
        : (
          <View style={st.controls}>
            <TouchableOpacity
              style={[st.captureBtn, IS_WEB && !state.webReady && st.disabled]}
              onPress={() = accessibilityLabel="button"> void capture()}
              disabled={IS_WEB && !state.webReady}
      ...
      <TouchableOpacity
        style={st.backBtn}
        onPress={() = accessibilityLabel="button"> { stopCamera(); router.back(); }}
        accessibilityRole="button"
      >
        <Text style={st.backBtnText}>Cancel</Text>
      </TouchableOpacity>
  ...
  if (state.step === 'verifying') return (
    <View style={st.container}>
      ...
      <Text style={st.statusText}>{state.statusText}</Text>
      <View style={st.thumbRow}>
        {state.photos.map((p, i) => <Thumb key={i} uri={p} index={i} />)}
      </View>
  ...
  if (state.step === 'failed') return (
    <View style={st.container}>
      ...
      <Text style={st.failText}>{state.error}</Text>
      <Text style={st.attemptsText}>{`Attempt ${state.attempts} of ${MAX_ATTEMPTS}`}</Text>
      {isCooldownActive
        ? <View style={st.cooldownBox}><Text style={st.cooldownText}>Try again in {cooldownStr}</Text></View>
        : state.attempts < MAX_ATTEMPTS
          ? (
            <TouchableOpacity style={st.retryBtn} onPress={() = accessibilityLabel="button"> void startVerification()} accessibilityRole="button">
...
const st = StyleSheet.create((theme) => ({
  ...
  scrollContainer: { flexGrow: 1, backgroundColor: theme.colors.background, justifyContent: 'center', alignItems: 'center', padding: 20 },
  title:           { fontSize: 26, fontWeight: 'bold', color: theme.colors.text, marginBottom: 20 },
  ...
  modelStatusText: { color: theme.colors.textSecondary, fontSize: 12 },
  ...
  stepText:        { fontSize: 14, color: theme.colors.textSecondary, marginBottom: 5 },
  ...
  loadText:        { color: theme.colors.textSecondary, marginTop: 10 },
  ...
  successTitle:    { fontSize: 26, fontWeight: 'bold', color: theme.colors.text, marginBottom: 10 },
  successSub:      { fontSize: 15, color: theme.colors.textSecondary },
  ...
  failTitle:       { fontSize: 24, fontWeight: 'bold', color: theme.colors.text, marginBottom: 10 },
  failText:        { fontSize: 14, color: theme.colors.textSecondary, textAlign: 'center', marginBottom: 10, paddingHorizontal: 20, lineHeight: 22 },
