...
import {
    calculateDatingStats,
    DatingStats,
    getConversationRateLevel,
    getMatchRateLevel,
} from '../utils/datingStats';
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.overviewGrid}>
        <View style={styles.overviewCard}>
          <Text style={styles.overviewNumber}>{stats.totalMatches}</Text>
          <Text style={styles.overviewLabel}>Total Matches</Text>
        </View>

        <View style={styles.overviewCard}>
          <Text style={[styles.overviewNumber, { color: matchRateInfo.color }]}>
            {stats.matchRate}%
          </Text>
          <Text style={styles.overviewLabel}>Match Rate</Text>
        </View>

        <View style={styles.overviewCard}>
          <Text style={styles.overviewNumber}>{stats.profileViews}</Text>
          <Text style={styles.overviewLabel}>Profile Views</Text>
        </View>

        <View style={styles.overviewCard}>
          <Text style={styles.overviewNumber}>{stats.trustScore}</Text>
          <Text style={styles.overviewLabel}>Trust Score</Text>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.ratingCard, { borderColor: matchRateInfo.color }]}>
          ...
          <View style={styles.progressBar}>
            <View 
              style={[
                styles.progressFill, 
                { width: `${stats.matchRate}%`, backgroundColor: matchRateInfo.color }
              ]} 
            />
          </View>
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statItem}>
            <Text style={styles.statNumber}>{stats.likesSent}</Text>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statNumber}>{stats.expiredMatches}</Text>
        ...
        <View style={[styles.ratingCard, { borderColor: conversationRateInfo.color }]}>
          ...
          <View style={styles.progressBar}>
            <View 
              style={[
                styles.progressFill, 
                { width: `${stats.conversationRate}%`, backgroundColor: conversationRateInfo.color }
              ]} 
            />
          </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Profile Views</Text>
            <Text style={styles.infoValue}>{stats.profileViews}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Views per Day</Text>
            <Text style={styles.infoValue}>{stats.profileViewRate}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Swipes per Day</Text>
            <Text style={styles.infoValue}>{stats.averageSwipesPerDay}</Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              ...
              <Text style={[styles.infoValue, { color: '#f1c40f' }]}>
                {stats.averageRating} ⭐
              </Text>
            </View>
            <View style={styles.infoRow}>
            ...
            {stats.secondDateRate > 0 && (
              <View style={styles.infoRow}>
                ...
                <Text style={[styles.infoValue, { color: '#5cb85c' }]}>
                  {stats.secondDateRate}%
                </Text>
              </View>
            )}
          </View>
      ...
      <View style={styles.tipsCard}>
        ...
        {stats.matchRate < 15 && (
          <Text style={styles.tipText}>
            • Update your photos - use recent, clear, smiling pictures{'\n'}
            • Write a better bio - be specific and interesting{'\n'}
            • Complete your profile (verifications, icebreaker, etc.)
          </Text>
        )}
        
        {stats.conversationRate < 40 && (
          <Text style={styles.tipText}>
            • Send the first message - don't wait for them{'\n'}
            • Use opening lines feature for conversation starters{'\n'}
            • Ask questions about their interests
          </Text>
        )}
        
        {stats.profileViewRate < 5 && (
          <Text style={styles.tipText}>
            • Be more active - swipe daily to appear in more searches{'\n'}
            • Answer the daily question - boosts visibility{'\n'}
            • Update your profile regularly
          </Text>
        )}

        {stats.matchRate >= 30 && stats.conversationRate >= 60 && (
          <Text style={styles.tipText}>
            ✅ You're doing great! Keep being yourself and stay active.
          </Text>
        )}
      </View>
    </ScrollView>
