...
export default function VerificationBadge({
  selfieVerified, heightVerified, ageVerified, trustedUser, ratings,
  size = 'medium', showAll = false,
}: VerificationBadgeProps) {
  const badgeSize = size === 'small' ? 12 : size === 'large' ? 18 : 14;

  const trustLevel = (): 'trusted' | 'verified' | null => {
    if (trustedUser || (ratings && ratings.totalRatings >= 3 && ratings.averagePhotosMatch >= 4)) return 'trusted';
  ...
  if (showAll) {
    return (
      <View style={styles.allBadgesContainer}>
        {selfieVerified && <View style={[styles.badge, styles.verifiedBadge]}><Text style={[styles.badgeIcon, { fontSize: badgeSize }]}>✓</Text><Text style={styles.badgeText}>Identity</Text></View>}
        {heightVerified && <View style={[styles.badge, styles.heightBadge]}><Text style={[styles.badgeIcon, { fontSize: badgeSize }]}>📏</Text><Text style={styles.badgeText}>Height</Text></View>}
        {ageVerified    && <View style={[styles.badge, styles.ageBadge]}><Text style={[styles.badgeIcon, { fontSize: badgeSize }]}>🎂</Text><Text style={styles.badgeText}>Age</Text></View>}
        {trustedUser    && <View style={[styles.badge, styles.trustedBadge]}><Text style={[styles.badgeIcon, { fontSize: badgeSize }]}>⭐</Text><Text style={styles.badgeText}>Trusted</Text></View>}
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  combinedIcon:       { color: '#fff' },
  combinedText:       { color: '#fff', fontWeight: '600' },
});