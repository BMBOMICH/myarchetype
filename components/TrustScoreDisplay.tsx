...
export default function TrustScoreDisplay({
  ...
  const trustLevel = getUserTrustLevel(ratings);
  ...
  // SMALL: Just a badge (for matches.tsx cards)
  if (size === 'small') {
    return (
      <TouchableOpacity
  ...
  // MEDIUM: Badge + score (for my-matches.tsx cards)
  if (size === 'medium') {
    return (
      <TouchableOpacity style={styles.mediumContainer} onPress={() => setShowModal(true)}>
        <View style={styles.mediumRow}>
          ...
          {trustScore > 0 && (
            <Text style={styles.mediumScore}>{trustScore}%</Text>
          )}
        </View>
  ...
  // LARGE: Full view (for edit-profile.tsx)
  return (
    ...
    <View style={styles.detailContainer}>
      ...
      <View style={styles.verificationsSection}>
        ...
        <View style={styles.verificationRow}>
          <View style={[styles.verificationDot, selfieVerified && styles.verificationDotActive]} />
          <Text style={[styles.verificationText, selfieVerified && styles.verificationTextActive]}>
            Identity {selfieVerified ? '✓' : '✗'}
          </Text>
        </View>
        <View style={styles.verificationRow}>
          <View style={[styles.verificationDot, ageVerified && styles.verificationDotActive]} />
          <Text style={[styles.verificationText, ageVerified && styles.verificationTextActive]}>
            Age {ageVerified ? '✓' : '✗'}
          </Text>
        </View>
        <View style={styles.verificationRow}>
          <View style={[styles.verificationDot, heightVerified && styles.verificationDotActive]} />
          <Text style={[styles.verificationText, heightVerified && styles.verificationTextActive]}>
            Height {heightVerified ? '✓' : '✗'}
          </Text>
        </View>
      </View>
      ...
      {hasRatings ? (
        <View style={styles.ratingsSection}>
          ...
          <RatingBar label="Photos match reality" value={ratings.averagePhotosMatch} max={5} />
          <RatingBar label="Height accurate" value={ratings.heightAccuracyRate} max={100} isPercent />
          <RatingBar label="Body type accurate" value={ratings.bodyTypeAccuracyRate} max={100} isPercent />
          <RatingBar label="Age accurate" value={ratings.ageAccuracyRate} max={100} isPercent />
          <RatingBar label="Personality match" value={ratings.averagePersonalityMatch} max={5} />
          <RatingBar label="Overall experience" value={ratings.averageOverall} max={5} />
        </View>
      ) : (
        <View style={styles.noRatingsBox}>
          <Text style={styles.noRatingsText}>
            No community ratings yet. Ratings appear after dates.
          </Text>
        </View>
      )}
    </View>
...
function RatingBar({ label, value, max, isPercent = false }: any) {
...
function calculateDisplayScore(ratings: any): number {
