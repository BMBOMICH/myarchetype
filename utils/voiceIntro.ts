...
export interface VoiceIntro {
  url: string; duration: number; uploadedAt: string;
  transcription?: string; safetyChecked?: boolean; flagged?: boolean;
}

export interface VoiceAnalysisResult {
  passed: boolean; issues: string[]; warnings: string[];
  transcription?: string; likelyPreRecorded?: boolean; likelyCloned?: boolean;
}

interface TranscribeResponse {
  safe: boolean; transcription?: string; reason?: string; flaggedCategories?: string[];
}

interface GenderResponse {
  consistent?: boolean; detectedGender?: string; confidence?: number;
}

interface CloudinaryAutoResponse {
  secure_url?: string;
}
  ...
  try {
    ...
    if (transcription) {
      ...
      for (const pattern of NSFW_SPEECH_PATTERNS) {
        if (pattern.test(transcription)) return { safe: false, transcription, reason: 'Voice intro contains inappropriate content.', flaggedCategories: ['nsfw_speech'] };
      }
...
export async function checkVoiceGenderConsistency(audioUrl: string, profileGender: string): Promise<{ consistent: boolean; detectedGender?: string; confidence: number }> {
  try {
    const res = await fetch(`${SERVER_URL}/analyzeVoiceGender`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ audioUrl, profileGender }),
    });
...
export async function getVoiceIntro(): Promise<VoiceIntro | null> {
  ...
  try {
    ...
    return snap.exists() ? (snap.data()['voiceIntro'] as VoiceIntro) ?? null : null;
  } catch { return null; }
}
  ...
  try {
    ...
    await updateDoc(doc(db, 'users', user.uid), {
      voiceIntro: {
        ...
        uploadedAt: new Date().toISOString(), transcription: analysis.transcription,
        safetyChecked: true, flagged: false,
      },
    });
...
export async function deleteVoiceIntro(): Promise<{ success: boolean }> {
  ...
  try {
    await updateDoc(doc(db, 'users', user.uid), { voiceIntro: null });
    return { success: true };
