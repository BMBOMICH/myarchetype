...
export async function detectVoiceCloneHeuristic(audioUri: string): Promise<{ likelyCloned: boolean; signals: string[] }> {
  ...
  try {
    ...
    for (let i = 0; i < samples.length; i += CHUNK) {
      ...
      rms.push(Math.sqrt(chunk.reduce((s, x) => s + x * x, 0) / chunk.length));
    }
    if (rms.length > 5) {
...
export async function transcribeAndModerateAudio(audioUrl: string): Promise<NsfwSpeechResult> {
  try {
    const res = await fetch(`${SERVER_URL}/transcribeAndModerate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ audioUrl, model: 'whisper-large-v3', guardrail: 'duoguard', categories: ['sexual_content', 'violence', 'hate', 'self_harm', 'scam'] }),
    });
    if (!res.ok) return { safe: true };
    const data = await res.json() as TranscribeResponse;
    if (!data.safe) {
      const result: NsfwSpeechResult = { safe: false, reason: data.reason ?? 'Audio contains inappropriate content.' };
      if (data.transcription !== undefined) result.transcription = data.transcription;
      if (data.flaggedCategories !== undefined) result.flaggedCategories = data.flaggedCategories;
      return result;
    }
    ...
    if (transcription) {
      ...
      if (!textCheck.safe) {
        const result: NsfwSpeechResult = { safe: false, transcription, reason: textCheck.reason };
        if (textCheck.flaggedCategories !== undefined) result.flaggedCategories = textCheck.flaggedCategories;
        return result;
      }
      for (const pattern of NSFW_SPEECH_PATTERNS) {
        if (pattern.test(transcription)) return { safe: false, transcription, reason: 'Voice intro contains sexually explicit content.', flaggedCategories: ['nsfw_speech', 'sexual_content'] };
      }
      for (const pattern of THREAT_PATTERNS) {
        if (pattern.test(transcription)) return { safe: false, transcription, reason: 'Voice intro contains threatening content.', flaggedCategories: ['threats', 'violence'] };
      }
      for (const pattern of SCAM_PATTERNS) {
        if (pattern.test(transcription)) return { safe: false, transcription, reason: 'Voice intro contains suspicious solicitation.', flaggedCategories: ['scam', 'solicitation'] };
      }
    ...
    const result: NsfwSpeechResult = { safe: true };
...
export async function checkVoiceGenderConsistency(audioUrl: string, profileGender: string): Promise<{ consistent: boolean; detectedGender?: string; confidence: number }> {
  try {
    ...
    const result: { consistent: boolean; detectedGender?: string; confidence: number } = {
      consistent: data.consistent ?? true, confidence: data.confidence ?? 0,
    };
    if (data.detectedGender !== undefined) result.detectedGender = data.detectedGender;
    return result;
...
export async function analyzeVoiceIntro(audioUri: string, audioUrl: string, profileGender?: string): Promise<VoiceAnalysisResult> {
  ...
  const [cloneCheck, preCheck, transcript] = await Promise.all([
    detectVoiceCloneHeuristic(audioUri),
    detectPreRecordedAudio(audioUri),
    transcribeAndModerateAudio(audioUrl),
  ]);
  ...
  const result: VoiceAnalysisResult = {
    passed: issues.length === 0, issues, warnings,
    likelyPreRecorded: preCheck.likelyPreRecorded, likelyCloned: cloneCheck.likelyCloned,
  };
  if (transcript.transcription !== undefined) result.transcription = transcript.transcription;
  return result;
}
...
export async function getVoiceIntro(): Promise<VoiceIntro | null> {
  ...
  try {
    ...
    return snap.exists() ? (snap.data()['voiceIntro'] as VoiceIntro | undefined) ?? null : null;
  } catch { return null; }
}

export async function uploadVoiceIntro(
  audioUri: string, duration: number, profileGender?: string,
): Promise<{ success: boolean; url?: string; error?: string; warnings?: string[] }> {
  const user = auth.currentUser;
  ...
  try {
    ...
    const voiceIntroData: VoiceIntro = {
      url: upData.secure_url, duration: Math.round(duration),
      uploadedAt: new Date().toISOString(), safetyChecked: true, flagged: false,
    };
    if (analysis.transcription !== undefined) voiceIntroData.transcription = analysis.transcription;
    await updateDoc(doc(db, 'users', user.uid), { voiceIntro: voiceIntroData });
    const result: { success: boolean; url?: string; warnings?: string[] } = { success: true, url: upData.secure_url };
    if (analysis.warnings.length > 0) result.warnings = analysis.warnings;
    return result;
