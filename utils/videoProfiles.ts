...
export async function trackFaceInVideo(
  ...
  try {
    ...
    if (!ctx) return { tracked: false, frameCount: 0, facePresentFrames: 0, consistencyScore: 0, signals: ['No canvas context'] };
...
export async function validateProfileVideo(videoUri: string): Promise<VideoMetadataCheck> {
  ...
  try {
    ...
    if (docApi) {
      ...
      if (!faceTracking.tracked && faceTracking.frameCount > 0) warnings.push(`Face not consistently visible (${faceTracking.consistencyScore}% frames). Ensure your face is in frame.`);
    }
  } catch (err) { logger.warn('[videoProfiles] metadata check error:', err); }
...
export function isVideoOld(uploadedAt: string | null): boolean {
  ...
  return Math.floor((Date.now() - new Date(uploadedAt).getTime()) / (1_000 * 60 * 60 * 24)) > 180;
}
