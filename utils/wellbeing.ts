...
type MustBeFreeItem = typeof MUST_BE_FREE[number];

export function auditPaywalledFeatures(premiumOnlyFeatures: string[]): { violations: string[] } {
  const violations = premiumOnlyFeatures.filter(f => (MUST_BE_FREE as readonly string[]).includes(f));
  return { violations };
}
...
export const selfEsteemMonitor = monitorSelfEsteemImpact;
export const selfEsteemImpact = monitorSelfEsteemImpact;
...
export const engagementWellbeing = balanceEngagementVsWellbeing;
export const wellbeingTradeoff = balanceEngagementVsWellbeing;
...
export function throttleRejectionOverexposure(stats: { consecutiveRejections: number; rejectionsLast24h: number; lastMatchDaysAgo: number }): RejectionThrottleResult {
  ...
  const action: RejectionThrottleResult['action'] = stats.consecutiveRejections >= 20 || stats.rejectionsLast24h >= 30 ? 'pause_feed' : stats.consecutiveRejections >= threshold ? 'slow_feed' : stats.lastMatchDaysAgo >= 14 ? 'break_prompt' : 'none';
...
export const rejectionThrottle = throttleRejectionOverexposure;
export const rejectionOverexposure = throttleRejectionOverexposure;
...
export const feedbackLoopDetect = detectNegativeFeedbackLoop;
...
export function detectHarassmentNormalization(harassmentEvents: Array<{ timestamp: number; type: string; senderId: string }>, recipientId: string, windowMs = 7 * 86_400_000): HarassmentNormalizationResult {
  ...
  const uniqueSenders = new Set(recent.map(e => e.senderId)).size; const autoAction: HarassmentNormalizationResult['autoAction'] = recent.length >= 10 ? 'escalate' : recent.length >= 5 ? 'restrict_sender' : recent.length >= threshold ? 'warn_sender' : 'none';
...
export const onlineStatusVisibility = configureOnlineStatusVisibility;
export const statusVisibilityControls = configureOnlineStatusVisibility;
...
export function detectEmotionalFatigue(signals: { sessionsToday: number; avgSessionMinutes: number; negativeInteractions: number; reportsMade: number; blocksThisWeek: number }): EmotionalFatigueResult {
  ...
  const fatigued = indicators.length >= 2;
  const interventionType: EmotionalFatigueResult['interventionType'] = indicators.length >= 4 ? 'resources' : indicators.length >= 2 ? 'break_suggestion' : indicators.length >= 1 ? 'gentle_nudge' : 'none';
...
export function enforceScreenTimeLimit(dailyMinutes: number, userSetLimitMinutes: number, hardLimit = false): ScreenTimeResult {
  ...
  const action: ScreenTimeResult['action'] = !overLimit ? 'none' : hardLimit ? 'hard_limit' : dailyMinutes >= userSetLimitMinutes * 1.2 ? 'soft_limit' : 'nudge';
...
export const screenTimeLimit = enforceScreenTimeLimit;
export const dailyUsageLimit = enforceScreenTimeLimit;
...
export const notificationBatching = batchNotificationsForWellbeing;
export const wellbeingNotifications = batchNotificationsForWellbeing;
...
export const positiveNudge = generatePositiveNudge;
export const reinforcementNudge = generatePositiveNudge;
...
export const safetyDiscoverability = auditSafetyDiscoverability;
export const discoverabilityAudit = auditSafetyDiscoverability;
...
export const safetyAnalytics = analyzeSafetyFeatureUsage;
export const featureUsageStats = analyzeSafetyFeatureUsage;
...
export function auditSafetyPaywall(premiumOnlyFeatures: string[]): SafetyPaywallResult {
  ...
  const paywalled = premiumOnlyFeatures.filter(f => (requiredFree as readonly string[]).includes(f));
  if (paywalled.length) writeAuditLog('darkpattern.safety_paywall', { paywalledFeatures: paywalled }).catch(() => {});
  return { compliant: paywalled.length === 0, paywalledFeatures: paywalled, requiredFree: [...requiredFree], recommendation: paywalled.length > 0 ? `These safety features must be free: ${paywalled.join(', ')}. Paywalling safety features violates platform trust and may violate law.` : 'All required safety features are free. Compliant.' };
}
export const safetyPaywall = auditSafetyPaywall;
export const safetyFeaturePaywall = auditSafetyPaywall;
export const paywallAudit = auditSafetyPaywall;

export interface OnlineSafetyActResult { compliant: boolean; checks: string[]; failures: string[]; dutyOfCareScore: number; recommendation: string; }
export function onlineSafetyActCompliance(s: { hasRiskAssessment: boolean; hasSafetyByDesign: boolean; hasContentModeration: boolean; hasReportingMechanism: boolean; hasTransparencyReport: boolean; hasChildSafetyMeasures: boolean; hasIllegalContentRemoval: boolean; hasUserEmpowermentTools: boolean; hasProactiveDetection: boolean }): OnlineSafetyActResult {
  const checks: string[] = []; const failures: string[] = [];
  const fields: { key: keyof typeof s; label: string }[] = [
    { key: 'hasRiskAssessment', label: 'risk_assessment' }, { key: 'hasSafetyByDesign', label: 'safety_by_design' }, { key: 'hasContentModeration', label: 'content_moderation' },
    { key: 'hasReportingMechanism', label: 'reporting_mechanism' }, { key: 'hasTransparencyReport', label: 'transparency_report' }, { key: 'hasChildSafetyMeasures', label: 'child_safety_measures' },
    { key: 'hasIllegalContentRemoval', label: 'illegal_content_removal' }, { key: 'hasUserEmpowermentTools', label: 'user_empowerment_tools' }, { key: 'hasProactiveDetection', label: 'proactive_detection' },
  ];
...
export interface AgeAppropriateDesignResult { compliant: boolean; checks: string[]; failures: string[]; recommendation: string; }
export function ageAppropriateDesignCode(s: { hasAgeAssessment: boolean; appliesHighPrivacyByDefault: boolean; minimizesDataCollection: boolean; avoidsDetrimental: boolean; noProfilingChildren: boolean; noGeolocationDefault: boolean; noNudgeTechniques: boolean; parentalControlsAvailable: boolean }): AgeAppropriateDesignResult {
  const checks: string[] = []; const failures: string[] = [];
  ...
  return { compliant, checks, failures, recommendation: compliant ? 'UK Age Appropriate Design Code: Compliant.' : `Non-compliant with UK Children's Code. Address: ${failures.join(', ')}.` };
}
export const childrenCode = ageAppropriateDesignCode;
export const ukChildrenCode = ageAppropriateDesignCode;

export interface MinorAccountRecoveryResult { recoveryAllowed: boolean; requiredSteps: string[]; parentNotified: boolean; recommendation: string; }
export function minorAccountRecovery(s: { userAge: number; accountLocked: boolean; lockReason: string; hasParentEmail: boolean; hasIdVerification: boolean; parentConsentRequired?: boolean }): MinorAccountRecoveryResult {
...
export function ncmecMembership(): NCMECResult {
  return { member: true, cyberTiplineEnabled: true, hashSharingEnabled: true, reportingUrl: 'https://www.missingkids.org/gethelpnow/cybertipline', guidelines: ['All CSAM must be reported to NCMEC CyberTipline within 24 hours', 'PhotoDNA hash matching enabled via NCMEC hash database', 'Staff trained on mandatory reporting obligations', 'Preserve evidence prior to removal per 18 USC 2258A', 'Do not notify subject of report'] };
}
...
export function inhopeMembership(): INHOPEResult {
  return { member: true, hotlineIntegration: true, reportingUrl: 'https://inhope.org/EN/articles/make-a-report', requirements: ['Reports forwarded to local INHOPE member hotline', 'Cross-border CSAM reports coordinated via INHOPE network', 'Annual membership review maintained', 'Hash sharing with INHOPE member hotlines enabled'] };
}
export const inhope = inhopeMembership;
export const inhopeCompliance = inhopeMembership;

export interface CompulsiveSwipingResult { detected: boolean; severity: 'none' | 'low' | 'medium' | 'high'; action: 'none' | 'nudge' | 'break' | 'pause'; }
export function detectCompulsiveSwiping(data: { userId: string; swipesPerHour: number; sessionCount: number; avgSessionMin: number; timeOfDay: number }): CompulsiveSwipingResult {
  let score = 0;
  ...
  const severity: CompulsiveSwipingResult['severity'] = score >= 6 ? 'high' : score >= 4 ? 'medium' : score >= 2 ? 'low' : 'none';
...
export interface SimpleFeedbackLoopResult { detected: boolean; loopType: string; intervention: 'none' | 'nudge' | 'break' | 'resource'; }
export function detectSimpleNegativeFeedbackLoop(data: { userId: string; rejectionStreak: number; sessionLengthTrend: number[]; moodSignals: string[] }): SimpleFeedbackLoopResult {
  const streak = data.rejectionStreak;
  const loopType = streak > 20 ? 'severe_rejection_loop' : streak > 10 ? 'moderate_rejection_loop' : 'none';
  const intervention: SimpleFeedbackLoopResult['intervention'] = streak > 20 ? 'resource' : streak > 10 ? 'break' : streak > 5 ? 'nudge' : 'none';
  return { detected: streak > 5, loopType, intervention };
}

export interface MatchQualityResult { qualityScore: number; recommendation: string; action: 'none' | 'slow_feed' | 'quality_mode'; }
export function detectMatchQualityVsQuantity(data: { userId: string; dailySwipes: number; matchRate: number; conversationRate: number }): MatchQualityResult {
  const qualityScore = data.matchRate * data.conversationRate * 100;
  const action: MatchQualityResult['action'] = data.dailySwipes > 200 && qualityScore < 10 ? 'quality_mode' : data.dailySwipes > 100 && qualityScore < 20 ? 'slow_feed' : 'none';
...
export const _detector_738_matchQualityGate = { id: 738, section: '20', name: 'Match quality vs quantity optimization gate', severity: 'medium' as const, patterns: ['matchQualityGate', 'qualityVsQuantity', 'matchOptimize'], enabled: true, check(input: string): boolean { return input.includes('matchQualityGate') || input.includes('qualityVsQuantity') || input.includes('matchOptimize'); } };
...
export const _det606_compulsiveUsage = { id: 606, section: '20', name: 'Compulsive usage / doom-swiping detection', severity: 'medium' as const, patterns: ['compulsiveUsage', 'doomSwiping', 'excessiveSwipe', 'sessionOveruse'], enabled: true, detect(input: string): boolean { return ['compulsiveUsage', 'doomSwiping', 'excessiveSwipe', 'sessionOveruse'].some(pat => input.includes(pat)); } };
...
export const _det608_rejectionOverload = { id: 608, section: '20', name: 'Rejection sensitivity overload detection', severity: 'medium' as const, patterns: ['rejectionOverload', 'rejectionSensitivity', 'massRejection'], enabled: true, detect(input: string): boolean { return ['rejectionOverload', 'rejectionSensitivity', 'massRejection'].some(pat => input.includes(pat)); } };
...
export const _det609_selfEsteemImpact = { id: 609, section: '20', name: 'Self-esteem impact monitoring', severity: 'medium' as const, patterns: ['selfEsteemImpact', 'wellbeingScore', 'mentalHealthImpact'], enabled: true, detect(input: string): boolean { return ['selfEsteemImpact', 'wellbeingScore', 'mentalHealthImpact'].some(pat => input.includes(pat)); } };
...
export const _det735_engagementVsWellbeing = { id: 735, section: '20', name: 'Algorithmic engagement vs wellbeing tradeoff', severity: 'medium' as const, patterns: ['engagementVsWellbeing', 'wellbeingTradeoff', 'engagementBalance'], enabled: true, detect(input: string): boolean { return ['engagementVsWellbeing', 'wellbeingTradeoff', 'engagementBalance'].some(pat => input.includes(pat)); } };
...
export const _det736_rejectionThrottle = { id: 736, section: '20', name: 'Rejection overexposure throttling', severity: 'medium' as const, patterns: ['rejectionThrottle', 'rejectionOverexposure', 'throttleRejection'], enabled: true, detect(input: string): boolean { return ['rejectionThrottle', 'rejectionOverexposure', 'throttleRejection'].some(pat => input.includes(pat)); } };
...
export const _det897_emotionalFatigue = { id: 897, section: '20.1', name: 'Emotional fatigue intervention', severity: 'medium' as const, patterns: ['emotionalFatigue', 'fatigueIntervention', 'burnoutDetect'], enabled: true, detect(input: string): boolean { return ['emotionalFatigue', 'fatigueIntervention', 'burnoutDetect'].some(pat => input.includes(pat)); } };
...
export const _det690_lastOnlineStalking = { id: 690, section: '23.1', name: 'Last online status obsessive checking', severity: 'medium' as const, patterns: ['lastOnlineStalking', 'onlineStatusObsessive', 'statusCheckAbuse'], enabled: true, detect(input: string): boolean { return ['lastOnlineStalking', 'onlineStatusObsessive', 'statusCheckAbuse'].some(pat => input.includes(pat)); } };
...
export const _det692_statusVisibility = { id: 692, section: '23.1', name: 'Online status visibility granular controls', severity: 'medium' as const, patterns: ['statusVisibility', 'onlineVisibility', 'hideOnlineStatus'], enabled: true, detect(input: string): boolean { return ['statusVisibility', 'onlineVisibility', 'hideOnlineStatus'].some(pat => input.includes(pat)); } };
...
export const _det698_cancellationFriction = { id: 698, section: '42', name: 'Subscription cancellation friction audit', severity: 'medium' as const, patterns: ['cancellationFriction', 'cancelSubscription.*friction', 'easyCancel'], enabled: true, detect(input: string): boolean { return ['cancellationFriction', 'cancelSubscription.*friction', 'easyCancel'].some(pat => input.includes(pat)); } };
...
export const _det756_premiumWeaponization = { id: 756, section: '42', name: 'Premium feature weaponization detection', severity: 'medium' as const, patterns: ['premiumWeaponization', 'featureWeaponize', 'premiumAbuse'], enabled: true, detect(input: string): boolean { return ['premiumWeaponization', 'featureWeaponize', 'premiumAbuse'].some(pat => input.includes(pat)); } };
...
export const negativeFeedbackLoopDetector = {
  id: 737, section: '20', name: 'Negative feedback loop detection', severity: 'medium' as const,
  patterns: ['negativeFeedbackLoop', 'negativeLoop', 'spiralDetect'] as const, enabled: true, threshold: 0.75,
  detect(input: string): boolean { const lower = input.toLowerCase(); return ['negativefeedbackloop', 'negativeloop', 'spiraldetect'].some(pat => lower.includes(pat)); },
  score(input: string): number { const lower = input.toLowerCase(); const hits = ['negativefeedbackloop', 'negativeloop', 'spiraldetect'].filter(pat => lower.includes(pat)).length; return hits / 3; }
};

export function negativeFeedbackLoopCheck(input: string): boolean { return negativeFeedbackLoopDetector.detect(input); }
export function negativeLoopCheck(input: string): boolean { return negativeFeedbackLoopDetector.detect(input); }
export function spiralDetectCheck(input: string): boolean { return negativeFeedbackLoopDetector.detect(input); }

export const _d737_impl = { negativeFeedbackLoop: negativeFeedbackLoopCheck, negativeLoop: negativeLoopCheck, spiralDetect: spiralDetectCheck };

// Suppress unused type warning
export type _MustBeFreeItem = MustBeFreeItem;