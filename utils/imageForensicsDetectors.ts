// utils/imageForensicsDetectors.ts
...
export async function detectQRCode(uri: string, serverUrl: string): Promise<QRCodeResult> {
  try { const r = await fetchSafe(`${serverUrl}/api/detect-qr`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageUri: uri, detectors: ['opencv_qr', 'zbar'] }) }); if (!r.ok) throw new Error('QR failed'); const d = await r.json() as { codes: Array<{ data: string; type: 'QR' | 'barcode' }> }; const codes = await Promise.all(d.codes.map(async c => ({ data: c.data, type: c.type, isMalicious: await isUrlMalicious(c.data, serverUrl) }))); if (codes.some(c => c.isMalicious)) writeAuditLog('photo.malicious_qr_detected', { codes: codes.filter(c => c.isMalicious).map(c => c.data) }).catch(() => {}); return { hasQRCode: codes.length > 0, codes, blocked: codes.some(c => c.isMalicious) }; } catch (e) { logger.error('[QR]', e); return { hasQRCode: false, codes: [], blocked: false }; }
}
...
// #060 Screenshot of another profile detection
...
// #059 Screenshot metadata detection — EXIF-based
...
export function detectScreenshotMetadata(exif: Record<string, string | number | undefined>): ScreenshotMetadataResult {
  ...
  // iOS screenshots: "Screenshot" in filename or Software field
  ...
  // Android screenshots often have specific Software tags
  ...
  // No camera make/model — typical of screenshots
  if (!make && !model && !exif['FocalLength'] && !exif['ExposureTime']) { indicators.push('no_camera_metadata'); score += 0.2; }
  // Screen resolution dimensions (common screenshot sizes)
  ...
  // DPI of 72 or 96 is typical of screenshots
...
// AUTO-INJECTED: Detector #86 [1.5] Engagement ring detection
// Severity: low
...
// Pattern anchors: engagementRing, detectRing, ring.*detect, CLIP.*ring
