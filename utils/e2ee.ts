...
async function setVal(key: string, value: string) {
  if (canUseIndexedDB()) return idbSet(key, value);
...
async function delVal(key: string) {
  if (canUseIndexedDB()) return idbDelete(key);
...
export async function decryptTextFromSender(payload: { ciphertext: string; nonce: string; senderPublicKey: string }): Promise<string> {
  ...
  const opened = nacl.box.open(
    b64Dec(payload.ciphertext),
    b64Dec(payload.nonce),
    b64Dec(payload.senderPublicKey),
    b64Dec(local.secretKey),
  );
