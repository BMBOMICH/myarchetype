...
export async function preloadSafetyModel(): Promise<boolean> {
  ...
  try { return !!(await loadNsfwModel()); } catch { return false; }
}
...
async function loadNsfwModel(): Promise<NsfwModel | null> {
  ...
  nsfwLoadPromise = (async () => {
    try {
      ...
      console.error('[moderation] NSFW model load failed:', err);
...
async function checkImageSafetyNative(imageUrl: string): Promise<ModerationResult> {
  ...
  try {
    const res = await fetch(`${SERVER_URL}/verify-photo-nsfw`, {
    ...
    return data.isNSFW
      ...
      : { safe: true, reason: 'OK' };
  } catch { return { safe: true, reason: 'Server check error (allowed)' }; }
}
...
export async function checkImageSafety(
  ...
  } catch { return { safe: true, reason: 'Check error (allowed)' }; }
}
...
export async function checkVideoFramesSafety(videoUri: string, frameCount = 5): Promise<ModerationResult> {
  if (!IS_WEB) {
    try {
      const res = await fetch(`${SERVER_URL}/verify-video-nsfw`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
export async function extractTextFromImage(imageUri: string, authToken?: string): Promise<{ text: string; hasContactInfo: boolean; contactTypes: string[] }> {
  ...
  } catch { return { text: '', hasContactInfo: false, contactTypes: [] }; }
}
...
export async function runFullImageScan(imageUri: string, context: 'profile' | 'chat' | 'edit' | 'story' | 'general', authToken?: string): Promise<ModerationResult> {
  const nsfw = await checkImageSafety(imageUri, context);
  if (!nsfw.safe) return nsfw;
  const nude = await checkNudeParts(imageUri, authToken);
  if (!nude.safe) return nude;
  const hate = await detectHateSymbols(imageUri, authToken);
  if (!hate.safe) return hate;
  const weapon = await detectWeapons(imageUri, authToken);
  if (!weapon.safe) return weapon;
  const drug = await detectDrugParaphernalia(imageUri, authToken);
  if (!drug.safe) return drug;
...
export function detectRacialSlurs(text: string): ModerationResult {
  ...
  for (const p of HATE_PATTERNS.filter(h => h.category === 'racial_slur')) {
    if (p.pattern.test(n) || p.pattern.test(text)) return { safe: false, reason: p.reason, flaggedCategories: [p.category, 'IDENTITY_ATTACK'], severity: p.severity };
  }
  return { safe: true, reason: 'OK' };
}
...
export function detectHomophobicSlurs(text: string): ModerationResult {
  const n = preprocessText(text);
  for (const p of HATE_PATTERNS.filter(h => h.category === 'homophobic_slur')) {
    if (p.pattern.test(n) || p.pattern.test(text)) return { safe: false, reason: p.reason, flaggedCategories: [p.category], severity: p.severity };
  }
  return { safe: true, reason: 'OK' };
}

export function detectSexualSolicitation(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectViolenceThreats(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectSelfHarmEncouragement(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectDrugDealingLanguage(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectUnderageReferences(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectSextortion(text: string): ModerationResult {
  const n = preprocessText(text);
  for (const p of SEXTORTION_PATTERNS) {
    if (p.pattern.test(n) || p.pattern.test(text)) return { safe: false, reason: p.reason, flaggedCategories: [p.category], severity: p.severity };
  }
  return { safe: true, reason: 'OK' };
}

export function detectDoxxing(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectCoerciveLanguage(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectGroomingLanguage(text: string): ModerationResult {
  const n = preprocessText(text);
...
export function detectMultilingualHateSpeech(text: string): ModerationResult {
  ...
  for (const p of HATE_PATTERNS.filter(h => h.category === 'hate_multilang')) {
    for (const v of variants) {
      if (p.pattern.test(v)) return { safe: false, reason: p.reason, flaggedCategories: [p.category], severity: p.severity };
    }
  }
  return { safe: true, reason: 'OK' };
}
...
export function checkTextSafety(text: string, field: ContentField = 'general'): ModerationResult {
  ...
  if (detectMixedScripts(text)) console.warn('[moderation] Mixed script detected');
  ...
  for (const { pattern, category, reason, severity } of ALL_HARMFUL_PATTERNS) {
    if (skip.includes(category)) continue;
    if (pattern.test(processed) || pattern.test(text)) return { safe: false, reason, flaggedCategories: [category], severity };
  }
...
export const moderateOccupation = (t: string): ModerationResult => {
  ...
  const processed = preprocessText(t);
  if (containsProfanity(processed)) return { safe: false, reason: 'Profanity is not allowed in occupation.', flaggedCategories: ['profanity'], severity: 'medium' };
