// project-scanner.js
// ULTIMATE PROJECT SCANNER v4.0 — checks EVERYTHING
// Run with: node project-scanner.js
// Outputs: project-audit.json + project-audit-report.txt
...
const IGNORE_DIRS = new Set([
  ...
  'coverage', '.turbo', '__pycache__', 'android', 'ios', '.cache',
  '.parcel-cache', '.svelte-kit', 'out', '.output', '.vercel',
  '.netlify', 'vendor', 'pods', '.yarn', '.pnp',
]);

const IGNORE_FILES = new Set([
  'project-scanner.js', 'project-audit.json',
  'project-audit-report.txt', 'detector-audit.json',
  'audit-detectors.js',
]);
...
function formatBytes(b) { if (b === 0) return '0 B'; const k = 1024; const s = ['B','KB','MB','GB']; const i = Math.floor(Math.log(b)/Math.log(k)); return (b/Math.pow(k,i)).toFixed(1)+' '+s[i]; }
...
function getSourceFiles() {
  return getAllFiles(ROOT, ['.ts', '.tsx', '.js', '.jsx']).filter(f => {
    ...
    return !r.includes('node_modules') && !r.includes('.expo') &&
           !r.includes('dist') && !r.includes('build') &&
           !IGNORE_FILES.has(path.basename(f));
  });
}

// Pre-load source data once for all scanners
let _sourceData = null;
let _allSourceContent = null;
...
function extractActualImports(files) {
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let pkg = null;
      const esMatch = t.match(/^import\s+.*?from\s+['"`]([^'"`]+)['"`]/);
      if (esMatch) pkg = esMatch[1];
      if (!pkg) { const side = t.match(/^import\s+['"`]([^'"`]+)['"`]/); if (side) pkg = side[1]; }
      if (!pkg) { const req = t.match(/require\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/); if (req) pkg = req[1]; }
      if (!pkg) { const dyn = t.match(/import\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/); if (dyn) pkg = dyn[1]; }
      if (pkg) { imports.add(pkg); importLines.push({ file: rel(file), pkg, line: t, lineNum: i + 1 }); }
    }
  }
  return { imports, importLines };
}

function getTopLevelPkg(importPath) {
  if (!importPath || importPath.startsWith('.') || importPath.startsWith('/')) return null;
  if (importPath.startsWith('@')) { const parts = importPath.split('/'); return parts.slice(0, 2).join('/'); }
...
function scanProjectIdentity() {
  ...
  const appConfig = readJSON(path.join(ROOT, 'app.config.js')) ?? readJSON(path.join(ROOT, 'app.config.ts'));
  ...
  const isMonorepo = !!(workspaces || fileExists(path.join(ROOT, 'lerna.json')) ||
  ...
  return {
    ...
    os: pkg?.os ?? [],
    cpu: pkg?.cpu ?? [],
    browserslist: pkg?.browserslist ?? null,
    sideEffects: pkg?.sideEffects ?? null,
    runtime: {
    ...
    expo: expo ? {
      ...
      platforms: expo.platforms ?? ['ios', 'android'],
      scheme: expo.scheme,
      ...
      extra: expo.extra ? Object.keys(expo.extra) : [],
      owner: expo.owner,
      runtimeVersion: expo.runtimeVersion,
      ios: {
        bundleIdentifier: expo.ios?.bundleIdentifier,
        buildNumber: expo.ios?.buildNumber,
        deploymentTarget: expo.ios?.deploymentTarget,
        supportsTablet: expo.ios?.supportsTablet,
        requireFullScreen: expo.ios?.requireFullScreen,
        usesAppleSignIn: expo.ios?.usesAppleSignIn,
      ...
      android: {
        ...
        compileSdkVersion: expo.android?.compileSdkVersion,
        targetSdkVersion: expo.android?.targetSdkVersion,
        minSdkVersion: expo.android?.minSdkVersion,
    ...
    eas: easJson ? {
      cli: easJson.cli,
      build: Object.entries(easJson.build ?? {}).map(([name, config]) => ({
        name, distribution: config.distribution, channel: config.channel,
        env: config.env ? Object.keys(config.env) : [], ios: config.ios ? Object.keys(config.ios) : [],
        android: config.android ? Object.keys(config.android) : [],
      })),
...
function scanPackages(actualImports) {
  ...
  function getInstalledVersion(pkgName) {
    const p = readJSON(path.join(ROOT, 'node_modules', pkgName, 'package.json'));
  ...
  function categorizePkg(name) {
    ...
    if (n.includes('sharp') || n.includes('jimp') || n.includes('canvas')) return '🖼️ Image Processing';
    if (n.includes('pdf') || n.includes('document')) return '📄 Documents';
    if (n.includes('qr') || n.includes('barcode')) return '📷 QR/Barcode';
    if (n.includes('chart') || n.includes('plot') || n.includes('graph') || n.includes('d3') || n.includes('victory')) return '📈 Charts';
    if (n.includes('markdown') || n.includes('mdx')) return '📝 Markdown';
    ...
    if (n.includes('queue') || n.includes('bull') || n.includes('bee-queue')) return '📬 Queues';
    if (n.includes('cache') || n.includes('redis') || n.includes('memcached')) return '⚡ Caching';
    return '📦 Other';
  }

  const FRAMEWORK_INTERNALS = new Set([
    'expo', 'expo-asset', 'expo-constants', 'expo-font', 'expo-system-ui',
    'expo-symbols', 'react-native-screens', 'react-native-gesture-handler',
    'react-native-reanimated', 'react-native-web', 'react-dom', 'react',
    '@react-navigation/bottom-tabs', '@react-navigation/elements', '@react-navigation/native',
    'react-native-worklets', '@babel/core', 'typescript', 'metro',
    '@types/node', '@types/react', '@types/react-native',
    'react-native-safe-area-context', 'react-native-svg',
    'expo-linking', 'expo-splash-screen', 'expo-status-bar',
    'expo-modules-core', 'expo-dev-client',
  ]);
  ...
  function processDeps(depsObj, type) {
    for (const [name, specifiedVersion] of Object.entries(depsObj)) {
      ...
      const isActuallyImported = isFrameworkInternal || actualImports.has(name) ||
        [...actualImports].some(imp => getTopLevelPkg(imp) === name);
      const versionMismatch = installedVersion && specifiedVersion &&
        !specifiedVersion.includes(installedVersion) && installedVersion !== specifiedVersion.replace(/[\^~>=<]/g, '');
      allDeps.push({
        name, specifiedVersion, installedVersion, type,
        category: categorizePkg(name),
        actuallyImported: isActuallyImported,
        isFrameworkInternal,
  ...
  processDeps(deps, 'production');
  processDeps(devDeps, 'development');
  processDeps(peerDeps, 'peer');
  processDeps(optDeps, 'optional');
  ...
  return {
    counts: {
      ...
      actuallyImported: allDeps.filter(d => d.actuallyImported).length,
      notInstalled: notInstalled.length,
      categories: Object.keys(byCategory).length,
    },
    all: allDeps,
    byCategory,
    possiblyUnused: possiblyUnused.map(d => d.name),
    undeclaredImports: undeclared,
    notInstalled: notInstalled.map(d => d.name),
    potentialDuplicates,
...
function scanTechStack(actualImports, importLines) {
  ...
  function getVersion(pkgName) { const p = readJSON(path.join(ROOT, 'node_modules', pkgName, 'package.json')); return p?.version ?? null; }
  ...
  stack.mobileFramework = {
    ...
    reactVersion: getVersion('react'),
    reactNativeVersion: getVersion('react-native'),
    ...
    hermes: (() => {
      const mc = readFile('metro.config.js') + readFile('metro.config.ts');
      const appJson = readJSON(path.join(ROOT, 'app.json'));
      return appJson?.expo?.jsEngine === 'hermes' || mc.includes('hermes') || true; // default since SDK 47
    })(),
  };
  ...
  // Navigation
  stack.navigation = {
    expoRouter: uses('expo-router'),
    expoRouterVersion: getVersion('expo-router'),
    reactNavigation: uses('@react-navigation'),
    reactNavigationVersion: getVersion('@react-navigation/native'),
    ...
    wouter: uses('wouter'),
    reactRouter: uses('react-router') || uses('react-router-native'),
  };
  ...
  stack.backend = {
    firebase: {
      ...
      rnFirebase: uses('@react-native-firebase'),
      services: {
      ...
      securityRules: {
        firestore: fileExists(path.join(ROOT, 'firestore.rules')),
        storage: fileExists(path.join(ROOT, 'storage.rules')),
        database: fileExists(path.join(ROOT, 'database.rules.json')),
      },
      indexes: fileExists(path.join(ROOT, 'firestore.indexes.json')),
      firebaseJson: fileExists(path.join(ROOT, 'firebase.json')),
    },
  ...
  stack.media = {
    cloudinary: {
      used: allContent.includes('cloudinary.com') || allContent.includes('CLOUDINARY'),
      version: getVersion('cloudinary'),
      usedFor: detectCloudinaryUsage(),
    },
    aws: { used: uses('aws-sdk', '@aws-sdk'), version: getVersion('aws-sdk') ?? getVersion('@aws-sdk/client-s3') },
    expo: {
      imageManipulator: uses('expo-image-manipulator'),
      imagePicker: uses('expo-image-picker'),
      mediaLibrary: uses('expo-media-library'),
      camera: uses('expo-camera'),
      video: uses('expo-video', 'expo-av'),
      audio: uses('expo-audio', 'expo-av'),
      fileSystem: uses('expo-file-system'),
      documentPicker: uses('expo-document-picker'),
      print: uses('expo-print'),
      sharing: uses('expo-sharing'),
      brightness: uses('expo-brightness'),
      barCodeScanner: uses('expo-barcode-scanner'),
    },
    expoImage: uses('expo-image'),
    fastImage: uses('react-native-fast-image'),
    sharp: uses('sharp'),
  };

  // Server
  const serverFiles = [
    ...getAllFiles(path.join(ROOT, 'server'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'api'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'functions'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'backend'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'src/server'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'src/api'), ['.ts', '.js']),
  ...
  stack.server = {
    ...
    security: {
      ...
      hmac: serverContent.includes('createHmac'),
      ...
      csrf: serverContent.includes('csrf') || serverContent.includes('csurf'),
      hpp: serverContent.includes('hpp'),
    ...
    hosting: (() => {
      ...
      if (fileExists(path.join(ROOT, 'terraform'))) return 'Terraform (IaC)';
      return 'Unknown';
    })(),
    endpoints: (() => {
      const eps = [];
      for (const file of serverFiles) {
        ...
        lines.forEach((line, idx) => {
          const m = line.match(/app\.(get|post|put|patch|delete|all|use|options|head)\s*\(\s*['"`]([^'"`]+)['"`]/i);
          if (m) eps.push({ method: m[1].toUpperCase(), path: m[2], file: rel(file), line: idx + 1 });
        });
      }
      return eps.length;
    })(),
  ...
  stack.payments = {
    ...
    paypal: uses('@paypal'),
    lemonsqueezy: uses('@lemonsqueezy'),
  };
  ...
  stack.notifications = {
    ...
    version: getVersion('expo-notifications'),
    firebase: allImportPaths.some(i => i.includes('firebase/messaging')),
    oneSignal: uses('react-native-onesignal'),
    webPush: allContent.includes('web-push'),
  ...
  stack.analyticsMonitoring = {
    ...
    amplitude: { used: uses('@amplitude/analytics-react-native') },
    mixpanel: { used: uses('mixpanel-react-native') },
    ...
    datadog: { used: uses('@datadog/mobile-react-native') },
    bugsnag: { used: uses('@bugsnag/react-native') },
    crashlytics: { used: uses('@react-native-firebase/crashlytics') },
    plausible: { used: uses('plausible-tracker') },
    umami: { used: allContent.includes('umami') },
  };
  ...
  // Location
  stack.location = {
    expoLocation: uses('expo-location'),
    version: getVersion('expo-location'),
    mapbox: uses('@rnmapbox/maps'),
    googleMaps: uses('react-native-maps'),
    ...
    ipGeolocation: (() => {
      ...
      if (allContent.includes('geoip')) s.push('GeoIP');
      return s;
    })(),
  };
  ...
  stack.stateManagement = {
    redux: uses('@reduxjs/toolkit', 'react-redux'),
    zustand: uses('zustand'),
    jotai: uses('jotai'),
    recoil: uses('recoil'),
    valtio: uses('valtio'),
    mobx: uses('mobx', 'mobx-react'),
    xstate: uses('xstate', '@xstate/react'),
    legendState: uses('@legendapp/state'),
    asyncStorage: uses('@react-native-async-storage/async-storage'),
    mmkv: uses('react-native-mmkv'),
    secureStore: uses('expo-secure-store'),
    watermelonDB: uses('@nozbe/watermelondb'),
    realm: uses('realm'),
    sqlite: uses('expo-sqlite', 'react-native-sqlite-storage'),
  ...
  // UI
  stack.ui = {
    nativeWind: uses('nativewind'),
    nativeWindVersion: getVersion('nativewind'),
    tailwindcss: uses('tailwindcss'),
    tamagui: uses('tamagui', '@tamagui'),
    gluestack: uses('@gluestack-ui'),
    nativebase: uses('native-base'),
    paper: uses('react-native-paper'),
    elements: uses('@rneui/themed'),
    shopifyFlash: uses('@shopify/flash-list'),
    ...
    skia: uses('@shopify/react-native-skia'),
    svg: uses('react-native-svg'),
    lottie: uses('lottie-react-native'),
    rive: uses('rive-react-native'),
    moti: uses('moti'),
    haptics: uses('expo-haptics'),
    linearGradient: uses('expo-linear-gradient'),
    blurView: uses('expo-blur') || uses('@react-native-community/blur'),
    bottomSheet: uses('@gorhom/bottom-sheet'),
    actionSheet: uses('@expo/react-native-action-sheet'),
    toast: uses('react-native-toast-message') || uses('burnt') || uses('sonner-native'),
    modal: uses('react-native-modal'),
    dropdown: uses('react-native-element-dropdown'),
    calendar: uses('react-native-calendars'),
    webview: uses('react-native-webview'),
    icons: {
      expo: uses('@expo/vector-icons'),
      lucide: uses('lucide-react-native'),
      phosphor: uses('phosphor-react-native'),
      heroicons: uses('@heroicons/react'),
      materialIcons: uses('react-native-vector-icons'),
    },
    carousel: uses('react-native-reanimated-carousel'),
    pager: uses('react-native-pager-view'),
    charts: uses('react-native-chart-kit') || uses('victory-native') || uses('react-native-gifted-charts'),
  };

  // Security libraries
  stack.securityLibraries = {
    tweetnacl: uses('tweetnacl'),
    tweetnaclVersion: getVersion('tweetnacl'),
    sodium: uses('libsodium-wrappers') || uses('react-native-sodium'),
    expoSecureStore: uses('expo-secure-store'),
    expoCrypto: uses('expo-crypto'),
    ...
    hmacSigning: serverContent.includes('createHmac'),
    bcrypt: uses('bcrypt', 'bcryptjs'),
    argon2: uses('argon2'),
    helmet: uses('helmet'),
    csp: allContent.includes('Content-Security-Policy'),
    certificatePinning: allContent.includes('ssl-pinning') || allContent.includes('certificatePinning') || allContent.includes('pinning'),
  ...
  stack.testing = {
    ...
    jestVersion: getVersion('jest'),
    vitest: uses('vitest'),
    testingLibrary: uses('@testing-library/react-native') || uses('@testing-library/react'),
    detox: uses('detox'),
    maestro: fileExists(path.join(ROOT, '.maestro')),
    appium: uses('appium'),
    cypress: uses('cypress'),
    playwright: uses('@playwright/test'),
    msw: uses('msw'),
    nock: uses('nock'),
    supertest: uses('supertest'),
  ...
  stack.buildDeployment = {
    ...
    cicd: {
      ...
      circleci: fileExists(path.join(ROOT, '.circleci')),
      bitrise: fileExists(path.join(ROOT, 'bitrise.yml')),
      fastlane: fileExists(path.join(ROOT, 'fastlane')),
      travisci: fileExists(path.join(ROOT, '.travis.yml')),
      jenkinsfile: fileExists(path.join(ROOT, 'Jenkinsfile')),
      gitlab: fileExists(path.join(ROOT, '.gitlab-ci.yml')),
      codemagic: fileExists(path.join(ROOT, 'codemagic.yaml')),
      appcenter: fileExists(path.join(ROOT, 'appcenter-post-clone.sh')),
    },
    docker: {
      dockerfile: fileExists(path.join(ROOT, 'Dockerfile')),
      dockerCompose: fileExists(path.join(ROOT, 'docker-compose.yml')) || fileExists(path.join(ROOT, 'docker-compose.yaml')) || fileExists(path.join(ROOT, 'compose.yml')),
      dockerIgnore: fileExists(path.join(ROOT, '.dockerignore')),
    },
  ...
  stack.externalAPIs = (() => {
    const checks = [
      ...
      { name: 'OpenStreetMap', url: 'overpass-api.de', category: 'Location' },
      ...
      { name: 'Instagram', url: 'graph.instagram.com', category: 'Social' },
    ...
    return checks.filter(c => allContent.includes(c.url)).map(c => ({
      ...c,
      usedIn: sourceData.filter(f => f.content.includes(c.url)).map(f => f.rel).slice(0, 5),
    }));
  })();

  // Permissions
  stack.permissions = {
    camera: allContent.includes('expo-camera') || allContent.includes('Camera.requestPermission') || allContent.includes('requestCameraPermission'),
    microphone: allContent.includes('Audio.requestPermissionsAsync') || allContent.includes('microphone'),
    location: allContent.includes('requestForegroundPermissionsAsync') || allContent.includes('requestPermissionsAsync') && allContent.includes('Location'),
    backgroundLocation: allContent.includes('requestBackgroundPermissionsAsync'),
    notifications: allContent.includes('Notifications.requestPermissionsAsync') || allContent.includes('requestPermission') && allContent.includes('notification'),
    ...
    speechRecognition: allContent.includes('Speech') && allContent.includes('permission'),
    healthKit: allContent.includes('HealthKit'),
  };
...
function scanEnvironmentVariables() {
  ...
  const envFiles = [
    ...
    '.env.staging', '.env.test', '.env.preview',
    'server/.env', 'server/.env.local', 'server/.env.production',
  ...
  const expoPublicVars = new Set();
  for (const file of sourceFiles) {
    const content = readFileAbs(file);
    let m;
    const envPattern = /process\.env\.([A-Z_0-9]+)/g;
    while ((m = envPattern.exec(content)) !== null) { referencedVars.add(m[1]); }
    const expoPattern = /Constants\.expoConfig\.extra\.([A-Za-z_0-9]+)/g;
    while ((m = expoPattern.exec(content)) !== null) { referencedVars.add(m[1]); }
    const expoPublicPattern = /process\.env\.(EXPO_PUBLIC_[A-Z_0-9]+)/g;
    while ((m = expoPublicPattern.exec(content)) !== null) { expoPublicVars.add(m[1]); }
  }

  const allDefinedVars = new Set(Object.values(result).flat().map(v => v.key));
  const missingFromEnv = [...referencedVars].filter(v => !allDefinedVars.has(v) && v !== 'NODE_ENV' && v !== 'PORT' && v !== 'HOME' && v !== 'PATH').sort();

  // Check if .env.example has vars not in .env
  ...
  return {
    files: result,
    referencedInCode: [...referencedVars].sort(),
    expoPublicVars: [...expoPublicVars].sort(),
    missingFromEnvFiles: missingFromEnv,
    missingFromMainEnv: missingFromMain,
    emptyOrPlaceholder: unique(emptyOrPlaceholder),
    envInGitignore,
    hasExample: !!(result['.env.example'] ?? result['.env.template'] ?? result['.env.sample']),
    summary: {
      totalFiles: Object.keys(result).length,
      totalVars: allVars.length,
      uniqueVars: new Set(allVars.map(v => v.key)).size,
      emptyVars: allVars.filter(v => !v.hasValue).length,
      placeholderVars: allVars.filter(v => v.isPlaceholder).length,
      secretVars: allVars.filter(v => v.isSecret).length,
      expoPublicVars: allVars.filter(v => v.isExpoPublic).length,
    },
  };
}
...
function scanSecurity(sourceFiles) {
  ...
  const dangerousPatterns = [
    ...
    { pattern: 'eval(', severity: 'critical', message: 'eval() — code injection risk' },
    { pattern: 'new Function(', severity: 'critical', message: 'new Function() — code injection risk' },
    ...
    { pattern: 'FIREBASE_API_KEY', severity: 'high', message: 'Firebase API key ref — ensure env var not hardcoded' },
    { pattern: 'disable-next-line', severity: 'high', message: 'Lint rule disabled' },
    ...
    { pattern: 'insecure', severity: 'high', message: 'Reference to insecure operation' },
    { pattern: 'allowHTTP', severity: 'high', message: 'HTTP (non-HTTPS) explicitly allowed' },
    { pattern: 'Math.random()', severity: 'high', message: 'Math.random() — not cryptographically secure' },
    ...
    { pattern: 'exec(', severity: 'medium', message: 'exec() — potential command injection' },
    { pattern: 'spawn(', severity: 'medium', message: 'spawn() — potential command injection' },
    { pattern: '__dirname', severity: 'medium', message: 'Path traversal risk with __dirname' },
    ...
    { pattern: 'console.log(', severity: 'low', message: 'console.log() in code' },
    { pattern: 'console.warn(', severity: 'low', message: 'console.warn() in code' },
    { pattern: 'alert(', severity: 'low', message: 'alert() in code' },
    { pattern: 'confirm(', severity: 'low', message: 'confirm() in code' },
    ...
    { pattern: 'any[]', severity: 'info', message: 'TypeScript "any" array' },
    { pattern: '// @ts-', severity: 'info', message: 'TypeScript directive' },
  ...
  for (const { rel: relPath, content } of sourceData) {
    for (const check of dangerousPatterns) {
      ...
      if (matches) {
        issues.push({ file: relPath, severity: check.severity, message: check.message, count: matches.length, pattern: check.pattern });
        fileIssueCounts[relPath] = (fileIssueCounts[relPath] ?? 0) + matches.length;
      }
    }
  }
  ...
  const gitignoreChecks = {
    envFiles: gitignore.includes('.env'),
    nodeModules: gitignore.includes('node_modules'),
    buildDirs: gitignore.includes('dist') || gitignore.includes('build'),
    osFiles: gitignore.includes('.DS_Store'),
    ideFiles: gitignore.includes('.idea') || gitignore.includes('.vscode'),
    coverage: gitignore.includes('coverage'),
    logs: gitignore.includes('*.log'),
    keystores: gitignore.includes('.keystore') || gitignore.includes('.jks'),
  ...
  return {
    ...
    issues,
    mostProblematicFiles: Object.entries(fileIssueCounts).sort((a, b) => b[1] - a[1]).slice(0, 15).map(([file, count]) => ({ file, issueCount: count })),
    gitignoreChecks,
    securityHeaders: {
      hasHelmet: getAllSourceContent().includes('helmet'),
      hasCsp: getAllSourceContent().includes('Content-Security-Policy'),
      hasHsts: getAllSourceContent().includes('Strict-Transport-Security'),
      hasXFrame: getAllSourceContent().includes('X-Frame-Options'),
      hasXContent: getAllSourceContent().includes('X-Content-Type-Options'),
    },
  };
}
...
function scanFileStructure(sourceFiles) {
  ...
  for (const file of sourceFiles) {
    ...
    if (relPath.includes('component') || relPath.endsWith('.tsx')) {
      if (name[0] && name[0] === name[0].toLowerCase() && !name.startsWith('use') && !name.startsWith('index') && !name.startsWith('_') && !name.startsWith('[') && !name.startsWith('+')) {
        // Component files should typically be PascalCase
        // But only flag .tsx files outside of app/ directory (file-based routing uses lowercase)
  ...
  const configFiles = [
    'tsconfig.json', 'babel.config.js', 'babel.config.ts', 'metro.config.js', 'metro.config.ts',
    ...
    'eas.json', 'firebase.json', 'firestore.rules', 'firestore.indexes.json', 'storage.rules',
    'Dockerfile', 'docker-compose.yml', 'docker-compose.yaml', '.dockerignore',
    '.gitignore', '.gitattributes', '.nvmrc', '.node-version', '.tool-versions',
    'README.md', 'CHANGELOG.md', 'LICENSE', 'LICENSE.md', 'SECURITY.md',
    'CONTRIBUTING.md', 'CODE_OF_CONDUCT.md', '.github/workflows',
    '.editorconfig', '.browserslistrc',
    'tailwind.config.js', 'tailwind.config.ts', 'postcss.config.js',
    'nativewind-env.d.ts', 'global.css',
    'Procfile', 'fly.toml', 'render.yaml', 'railway.json', 'vercel.json', 'netlify.toml',
    '.husky', 'commitlint.config.js', '.commitlintrc', 'lint-staged.config.js', '.lintstagedrc',
    'turbo.json', 'lerna.json', 'nx.json', 'pnpm-workspace.yaml',
    '.env.example', '.env.template',
    'renovate.json', '.renovaterc', 'dependabot.yml', '.github/dependabot.yml',
  ];
...
function scanGit() {
  ...
  const result = {
    ...
    firstCommitDate: run('git log --reverse --format="%ai" | head -1'),
    ...
    stashCount: parseInt(run('git stash list | wc -l') ?? '0', 10) || 0,
    ...
    largeFiles: (run('git ls-files | head -100') ?? '').split('\n').filter(Boolean).length,
    gitHooks: (() => {
      const hooks = {};
      const huskyDir = path.join(ROOT, '.husky');
      if (fileExists(huskyDir)) {
        try {
          const files = fs.readdirSync(huskyDir).filter(f => !f.startsWith('.') && !f.startsWith('_'));
          for (const f of files) hooks[f] = readFileAbs(path.join(huskyDir, f)).trim().split('\n').filter(l => l.trim() && !l.startsWith('#')).join('; ');
        } catch {}
      }
      const gitHooksDir = path.join(ROOT, '.git/hooks');
      if (fileExists(gitHooksDir)) {
        try {
          const files = fs.readdirSync(gitHooksDir).filter(f => !f.endsWith('.sample'));
          for (const f of files) hooks[f] = '(custom git hook)';
        } catch {}
      }
      return hooks;
    })(),
...
function scanCodeQuality(sourceFiles) {
  ...
  let totalLines = 0, commentLines = 0, blankLines = 0;
  let anyCount = 0, tsIgnoreCount = 0, consoleCount = 0, todoCount = 0, fixmeCount = 0;
  let testFiles = 0, componentFiles = 0, utilFiles = 0, hookFiles = 0, typeFiles = 0, constantFiles = 0;
  let longestFunction = { file: '', length: 0, name: '' };
  let maxComplexity = { file: '', complexity: 0 };
  ...
  for (const file of sourceFiles) {
    ...
    let ifCount = 0, elseCount = 0, ternaryCount = 0, switchCount = 0, catchCount = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inFunc) {
        ...
        if (braceDepth <= 0 && i > funcStart) {
          ...
          if (len > longestFunction.length) longestFunction = { file: relPath, length: len, name: funcName };
          inFunc = false;
        }
      }
    }
  ...
  return {
    ...
    hasLinter: eslintConfig.length > 0,
    hasFormatter: prettierConfig.length > 0,
...
function scanAPIEndpoints() {
  ...
  const serverFiles = [
    ...
    ...getAllFiles(path.join(ROOT, 'src/server'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'src/api'), ['.ts', '.js']),
    ...getAllFiles(path.join(ROOT, 'src/routes'), ['.ts', '.js']),
  ].filter(f => !f.includes('node_modules'));
  ...
  for (const file of serverFiles) {
    ...
    lines.forEach((line, idx) => {
      ...
      const ep = {
        method: m[1].toUpperCase(), path: m[2], file: relPath, line: idx + 1,
        hasAuth: context.includes('requireAuth') || context.includes('verifyToken') || context.includes('authenticate') || context.includes('isAuthenticated') || context.includes('passport'),
        hasRateLimit: context.includes('Limiter') || context.includes('rateLimit') || context.includes('rateLimiter'),
        hasValidation: context.includes('validate') || context.includes('express-validator') || context.includes('schema') || context.includes('joi') || context.includes('zod'),
        hasErrorHandler: context.includes('try') && context.includes('catch'),
      };
...
function scanDependencyHealth() {
  ...
  const hasPackageLock = fileExists(path.join(ROOT, 'package-lock.json'));
  const hasYarnLock = fileExists(path.join(ROOT, 'yarn.lock'));
  const hasPnpmLock = fileExists(path.join(ROOT, 'pnpm-lock.yaml'));
  const hasBunLock = fileExists(path.join(ROOT, 'bun.lockb'));
  ...
  try {
    ...
    if (out) {
      outdated = Object.entries(JSON.parse(out)).map(([name, info]) => ({
        name, current: info.current, wanted: info.wanted, latest: info.latest,
        isOutdated: info.current !== info.latest,
        isMajorBehind: (() => {
          try { const c = parseInt(info.current?.split('.')[0]); const l = parseInt(info.latest?.split('.')[0]); return l > c; } catch { return false; }
        })(),
      }));
    }
  } catch {}
  ...
  for (const depName of Object.keys(allPkgDeps)) {
    ...
    if (depPkg) {
      const hasNative = depPkg.nativePackage || depPkg.rnpm || depPkg['react-native'] ||
        fileExists(path.join(ROOT, 'node_modules', depName, 'android')) ||
        fileExists(path.join(ROOT, 'node_modules', depName, 'ios')) ||
        fileExists(path.join(ROOT, 'node_modules', depName, 'cpp'));
      if (hasNative) nativeModules.push(depName);
    }
  }

  // Dependency auto-update config
  ...
  return {
    ...
    outdatedCount: outdated?.filter(p => p.isOutdated).length ?? null,
...
function scanRoutes() {
  ...
  for (const file of routeFiles) {
    ...
    if (bn === '_layout' || bn === 'layout') {
      layouts.push({ file: relPath, hasTabBar: content.includes('Tabs') || content.includes('BottomTab'), hasStack: content.includes('Stack'), hasDrawer: content.includes('Drawer') });
    } else if (bn === '+not-found' || bn === 'not-found' || bn === '404') {
      errorBoundaries.push({ file: relPath, type: 'not-found' });
    } else if (bn === '+html' || bn === '_error' || bn === 'error') {
      errorBoundaries.push({ file: relPath, type: bn });
    } else if (relPath.includes('+api') || relPath.includes('api/')) {
      apiRoutes.push({ file: relPath });
  ...
  for (const { rel: rp, content, lines } of sourceData) {
    for (let i = 0; i < lines.length; i++) {
      const t = lines[i];
      if (t.includes('router.push(') || t.includes('router.replace(') || t.includes('router.navigate(') ||
          t.includes('navigation.navigate(') || t.includes('navigation.push(') || t.includes('navigation.goBack(') ||
          t.includes('router.back()') || t.includes('Redirect') || t.includes('Link ') || t.includes('<Link')) {
  ...
  if (appJson?.expo?.ios?.associatedDomains) {
    for (const d of appJson.expo.ios.associatedDomains) deepLinks.push({ type: 'associated-domain', value: d });
  }
  if (appJson?.expo?.android?.intentFilters) {
    for (const f of appJson.expo.android.intentFilters) deepLinks.push({ type: 'intent-filter', value: JSON.stringify(f) });
  }
  ...
  return {
    ...
    routes, layouts, errorBoundaries, apiRoutes,
    navigations: navigations.slice(0, 50),
    deepLinks,
    hasNotFound: errorBoundaries.some(e => e.type === 'not-found'),
    hasTabNavigation: layouts.some(l => l.hasTabBar),
    hasStackNavigation: layouts.some(l => l.hasStack),
    hasDrawerNavigation: layouts.some(l => l.hasDrawer),
  };
}
...
function scanComponents() {
  ...
  for (const { rel: rp, content, lines, ext } of sourceData) {
    ...
    const isComponent = (ext === '.tsx' || ext === '.jsx') && hasJSX;

    if (isComponent) {
      const compInfo = {
        file: rp, lines: lines.length,
        usesMemo: content.includes('React.memo') || content.includes('memo('),
        usesForwardRef: content.includes('forwardRef'),
        usesErrorBoundary: content.includes('componentDidCatch') || content.includes('ErrorBoundary'),
        ...
        refCount: (content.match(/useRef/g) ?? []).length,
        memoCount: (content.match(/useMemo/g) ?? []).length,
        callbackCount: (content.match(/useCallback/g) ?? []).length,
        hasStyleSheet: content.includes('StyleSheet.create'),
        hasInlineStyles: (content.match(/style=\{\{/g) ?? []).length > 0,
        inlineStyleCount: (content.match(/style=\{\{/g) ?? []).length,
    ...
    if (bn.startsWith('use') && bn[3] && bn[3] === bn[3].toUpperCase()) {
      hooks.push({
        file: rp, name: bn, lines: lines.length,
        dependencies: unique([
          ...(content.match(/use[A-Z]\w+/g) ?? []).filter(h => h !== bn),
        ]),
        hasCleanup: content.includes('return () =>') || content.includes('return function'),
        usesEffect: content.includes('useEffect'),
        usesState: content.includes('useState'),
        usesRef: content.includes('useRef'),
    ...
    if (content.includes('createContext')) {
      ...
      for (const m of ctxMatches) {
        const nameMatch = m.match(/(?:const|let)\s+(\w+)/);
        contexts.push({ file: rp, name: nameMatch?.[1] ?? 'unknown' });
      }
    }

    // Detect providers
    if (content.includes('Provider') && content.includes('value=')) {
      providers.push({ file: rp });
    }

    // Detect HOCs
    if (content.includes('withRouter') || content.includes('connect(') || content.match(/with[A-Z]\w+\s*\(/)) {
      hocs.push({ file: rp });
    }
  }

  // Prop drilling detection (simplified: count how deep props are passed)
  const propDrilling = [];
  for (const { rel: rp, content } of sourceData) {
    const spreadProps = (content.match(/\{\.\.\.props\}/g) ?? []).length;
    const restProps = (content.match(/\.\.\.(?:rest|props|other)/g) ?? []).length;
    if (spreadProps + restProps > 2) {
      propDrilling.push({ file: rp, spreadCount: spreadProps + restProps });
    }
  }

  return {
    totalComponents: components.length,
    classComponents: components.filter(c => c.isClassComponent).length,
    functionalComponents: components.filter(c => !c.isClassComponent).length,
    memoizedComponents: components.filter(c => c.usesMemo).length,
    forwardRefComponents: components.filter(c => c.usesForwardRef).length,
    ...
    hocs: hocs.length,
    propDrilling: propDrilling.slice(0, 10),
    errorBoundaries: components.filter(c => c.usesErrorBoundary).length,
  };
}
...
function scanErrorHandling() {
  ...
  const sourceData = getSourceData(getSourceFiles());
  let tryCatchBlocks = 0, catchBlocks = 0, emptyTryCatch = 0;
  let errorBoundaries = 0, throwStatements = 0;
  let unhandledPromises = 0, promiseWithCatch = 0;
  let consoleErrorCount = 0, sentryCapture = 0;
  ...
  for (const { rel: rp, content, lines } of sourceData) {
    ...
    // Async functions without try-catch (simplified detection)
    const asyncFuncs = content.match(/async\s+(?:function\s+\w+|\(\w*\)|\w+)\s*(?:\([^)]*\))?\s*(?:=>|{)/g) ?? [];
    if (asyncFuncs.length > 0 && tryCount === 0 && catchCount === 0 && content.includes('await')) {
      asyncWithoutTryCatch.push({ file: rp, asyncFunctionCount: asyncFuncs.length });
    }

    // Unhandled promise (promise without .catch or await/try-catch)
    const promiseNew = (content.match(/new Promise/g) ?? []).length;
    const thenCount = (content.match(/\.then\s*\(/g) ?? []).length;
    if (thenCount > 0 && promiseWithCatch === 0 && tryCount === 0) unhandledPromises++;
  }
  ...
  const globalHandlers = {
    ...
    sentryInit: allContent.includes('Sentry.init'),
    crashlytics: allContent.includes('crashlytics'),
  };

  return {
    tryCatchBlocks, catchBlocks, throwStatements, emptyTryCatch, promiseWithCatch,
    errorBoundaries, consoleErrorCount, sentryCapture,
    asyncWithoutTryCatch: asyncWithoutTryCatch.slice(0, 15),
    ...
    score: (() => {
      let s = 0;
      if (tryCatchBlocks > 0) s += 20;
      if (errorBoundaries > 0) s += 20;
      if (emptyTryCatch === 0) s += 15;
      if (sentryCapture > 0) s += 15;
      if (globalHandlers.unhandledRejection || globalHandlers.sentryInit) s += 15;
      if (asyncWithoutTryCatch.length === 0) s += 15;
...
function scanNetworkCalls() {
  ...
  const axiosCalls = [];
  const urls = new Set();
  let hasTimeout = false, hasRetry = false, hasAbortController = false, hasInterceptor = false;
  let hasGraphQL = false, hasWebSocket = false, hasSSE = false;

  for (const { rel: rp, content, lines } of sourceData) {
    // Fetch calls
    for (let i = 0; i < lines.length; i++) {
      const t = lines[i].trim();
      if (t.includes('fetch(') || t.includes('fetch (')) {
        const urlMatch = t.match(/fetch\s*\(\s*['"`]([^'"`]+)['"`]/);
        if (urlMatch) { urls.add(urlMatch[1]); fetchCalls.push({ file: rp, line: i + 1, url: urlMatch[1] }); }
        else fetchCalls.push({ file: rp, line: i + 1, url: '(dynamic)' });
      }
      if (t.includes('axios.') || t.includes('axios(')) {
        const urlMatch = t.match(/['"`](https?:\/\/[^'"`]+)['"`]/) || t.match(/['"`](\/[^'"`]+)['"`]/);
        axiosCalls.push({ file: rp, line: i + 1, url: urlMatch?.[1] ?? '(dynamic)' });
  ...
  // Detect API client/service files
  const apiServiceFiles = getSourceFiles().filter(f => {
    const r = rel(f).toLowerCase();
    return r.includes('api') || r.includes('service') || r.includes('client') || r.includes('http');
  }).map(f => rel(f));

  // Data fetching libraries
  const allContent = getAllSourceContent();
  const dataFetching = {
    tanstackQuery: allContent.includes('@tanstack/react-query') || allContent.includes('useQuery'),
    swr: allContent.includes('useSWR'),
    apollo: allContent.includes('@apollo/client'),
    urql: allContent.includes('urql'),
    rtk: allContent.includes('createApi') && allContent.includes('fetchBaseQuery'),
    axios: allContent.includes('axios'),
    ky: allContent.includes("from 'ky'"),
    got: allContent.includes("from 'got'"),
  };

  return {
    fetchCalls: fetchCalls.length,
    axiosCalls: axiosCalls.length,
    totalNetworkCalls: fetchCalls.length + axiosCalls.length,
    uniqueUrls: [...urls].slice(0, 30),
    hasTimeout, hasRetry, hasAbortController, hasInterceptor,
    hasGraphQL, hasWebSocket, hasSSE,
    dataFetchingLibraries: Object.entries(dataFetching).filter(([, v]) => v).map(([k]) => k),
    apiServiceFiles: apiServiceFiles.slice(0, 20),
    fetchDetails: fetchCalls.slice(0, 30),
    axiosDetails: axiosCalls.slice(0, 30),
    resilience: {
      hasTimeout, hasRetry, hasAbortController,
      hasCircuitBreaker: allContent.includes('circuitBreaker') || allContent.includes('circuit-breaker'),
      hasBackoff: allContent.includes('backoff') || allContent.includes('exponential'),
    },
  };
}
...
function scanAccessibility() {
  ...
  const sourceData = getSourceData(getSourceFiles());
  let labelCount = 0, roleCount = 0, hintCount = 0, stateCount = 0;
  let accessibleProp = 0, importantForAccessibility = 0;
  let touchableWithoutLabel = 0, imageWithoutAlt = 0;
  let ariaCount = 0, tabIndexCount = 0;
  const issues = [];
  ...
  for (const { rel: rp, content } of sourceData) {
    ...
    for (const match of touchableMatches) {
      if (!match.includes('accessibilityLabel') && !match.includes('aria-label')) {
        touchableWithoutLabel++;
        if (touchableWithoutLabel <= 10) issues.push({ file: rp, issue: 'Touchable without accessibilityLabel' });
      }
    }

    // Images without alt/accessibilityLabel
    const imageMatches = content.match(/<Image\b[^>]*>/g) ?? [];
    for (const match of imageMatches) {
      if (!match.includes('accessibilityLabel') && !match.includes('alt=') && !match.includes('aria-label')) {
        imageWithoutAlt++;
        if (imageWithoutAlt <= 10) issues.push({ file: rp, issue: 'Image without accessibilityLabel/alt' });
      }
    }
  }
  ...
  return {
    labels: labelCount, roles: roleCount, hints: hintCount, states: stateCount,
    accessibleProps: accessibleProp, ariaAttributes: ariaCount,
    touchableWithoutLabel, imageWithoutAlt,
    hasScreenReaderSupport, hasDynamicFontSize, hasReducedMotion, hasVoiceOver,
    issues: issues.slice(0, 20),
    score: (() => {
      let s = 0;
      if (labelCount > 0) s += 20;
      if (roleCount > 0) s += 15;
      if (hintCount > 0) s += 10;
      if (touchableWithoutLabel === 0) s += 20;
      if (imageWithoutAlt === 0) s += 15;
      if (hasScreenReaderSupport) s += 10;
      if (hasDynamicFontSize) s += 10;
      return Math.min(s, 100);
    })(),
  };
}
...
function scanPerformance() {
  ...
  for (const { rel: rp, content } of sourceData) {
    // Inline functions in JSX event handlers
    ...
    // ScrollView with many children (should use FlatList)
    if (content.includes('<ScrollView') && (content.match(/<View/g) ?? []).length > 20 && !content.includes('FlatList') && !content.includes('FlashList')) {
      issues.push({ file: rp, issue: 'ScrollView with many children — consider FlatList/FlashList', severity: 'high' });
    }

    // Missing key prop in maps
    const mapWithoutKey = content.match(/\.map\([^)]*\)\s*\.\s*map|\.map\([^]*?(?:<\w+)(?![^>]*key=)/g) ?? [];
    if (content.includes('.map(') && !content.includes('key=') && !content.includes('key:') && content.includes('return (')) {
      // Simplified detection — check if .map returns JSX without key
      const mapBlocks = content.match(/\.map\(\s*(?:\([^)]*\)|[^=]*)\s*=>\s*(?:\(?\s*<)/g) ?? [];
      if (mapBlocks.length > 0) {
        const mapContext = content.split('.map(');
        for (let i = 1; i < mapContext.length; i++) {
          const block = mapContext[i].substring(0, 200);
          if (block.includes('<') && !block.includes('key=') && !block.includes('key:')) {
            missingKeys++;
          }
        }
      }
    ...
    // Heavy imports
    if (content.includes("from 'moment'") || content.includes("from 'lodash'") || content.includes("require('moment')") || content.includes("require('lodash')")) {
      heavyImports++;
      issues.push({ file: rp, issue: 'Heavy import (moment/lodash) — consider lighter alternatives', severity: 'medium' });
    }

    // Large component files
    if (content.split('\n').length > 500 && rp.endsWith('.tsx')) {
      issues.push({ file: rp, issue: `Component file is ${content.split('\n').length} lines — consider splitting`, severity: 'low' });
    }

    // Re-render risk: object/array creation in render
    const objectInRender = (content.match(/style=\{\{/g) ?? []).length;
    if (objectInRender > 10) {
      issues.push({ file: rp, issue: `${objectInRender} inline style objects — potential re-render cause`, severity: 'low' });
    }
  }

  const allContent = getAllSourceContent();
  const optimizations = {
    usesFlashList: allContent.includes('FlashList'),
    usesFlatList: allContent.includes('FlatList'),
    usesSectionList: allContent.includes('SectionList'),
    usesVirtualizedList: allContent.includes('VirtualizedList'),
    usesMemo: (allContent.match(/useMemo/g) ?? []).length,
    usesCallback: (allContent.match(/useCallback/g) ?? []).length,
    usesReactMemo: (allContent.match(/React\.memo|memo\(/g) ?? []).length,
    usesLazy: allContent.includes('React.lazy') || allContent.includes('lazy('),
    usesSuspense: allContent.includes('Suspense'),
    usesInteractionManager: allContent.includes('InteractionManager'),
    usesRequestAnimationFrame: allContent.includes('requestAnimationFrame'),
  ...
  return {
    ...
    inlineFunctionsInJSX: inlineFunctions,
    possiblyMissingKeys: missingKeys,
    heavyImports,
    optimizations,
    score: (() => {
      let s = 50; // Start at 50
      if (optimizations.usesFlashList) s += 10;
      if (optimizations.usesMemo > 0) s += 5;
      if (optimizations.usesCallback > 0) s += 5;
      if (optimizations.usesReactMemo > 0) s += 5;
      if (optimizations.hasImageOptimization) s += 5;
      if (optimizations.hasListOptimization) s += 5;
      if (heavyImports > 0) s -= 10;
      if (inlineFunctions > 20) s -= 10;
      if (issues.filter(i => i.severity === 'high').length > 0) s -= 10;
...
function scanAssets() {
  ...
  for (const dir of assetDirs) {
    allAssetFiles.push(...getAllFiles(path.join(ROOT, dir), []));
  }
  // Also check root-level asset files
  ...
  for (const file of unique(allAssetFiles.map(f => f))) {
    ...
    else if (!jsonExts.includes(ext)) assets.other.push(info);
  }

  // Check for @2x/@3x variants
  const baseImages = assets.images.filter(i => !i.file.includes('@2x') && !i.file.includes('@3x'));
    const has2x = assets.images.filter(i => i.file.includes('@2x')).length;
  const has3x = assets.images.filter(i => i.file.includes('@3x')).length;

  // Check for adaptive icons, splash screens
  const appJson = readJSON(path.join(ROOT, 'app.json'))?.expo;
  const splashImage = appJson?.splash?.image;
  const icon = appJson?.icon;
  const adaptiveIcon = appJson?.android?.adaptiveIcon;
  const favicon = appJson?.web?.favicon;

  // Check for unused assets (images referenced in code)
  const allContent = getAllSourceContent();
  const possiblyUnusedAssets = [];
  for (const img of [...assets.images, ...assets.svgs]) {
    const basename = path.basename(img.file);
    const nameNoExt = path.basename(img.file, path.extname(img.file)).replace(/@[23]x/, '');
    if (!allContent.includes(basename) && !allContent.includes(nameNoExt)) {
      possiblyUnusedAssets.push(img.file);
    }
  }

  // Check for WebP usage (more efficient)
  const webpCount = assets.images.filter(i => i.ext === '.webp').length;
  const pngCount = assets.images.filter(i => i.ext === '.png').length;
  const jpgCount = assets.images.filter(i => ['.jpg', '.jpeg'].includes(i.ext)).length;

  return {
    summary: {
      totalAssets: allAssetFiles.length,
    ...
    retina: { baseImages: baseImages.length, '@2x': has2x, '@3x': has3x },
    largeAssets: largeAssets.sort((a, b) => b.size - a.size).slice(0, 15),
    possiblyUnusedAssets: possiblyUnusedAssets.slice(0, 20),
    appAssets: {
      icon: icon ?? null,
    ...
    recommendations: (() => {
      const recs = [];
      if (pngCount > 10 && webpCount === 0) recs.push('Consider converting PNGs to WebP for smaller bundle size');
      if (largeAssets.length > 0) recs.push(`${largeAssets.length} assets over 500KB — consider compressing`);
      if (has2x === 0 && has3x === 0 && assets.images.length > 5) recs.push('No @2x/@3x image variants detected — may look blurry on high-DPI screens');
      if (possiblyUnusedAssets.length > 5) recs.push(`${possiblyUnusedAssets.length} possibly unused assets — verify and remove to reduce bundle`);
      if (assets.fonts.length > 5) recs.push('Many custom fonts loaded — consider reducing for faster startup');
      return recs;
    })(),
  };
}
...
function scanI18n() {
  ...
  const hasI18n = allContent.includes('i18n') || allContent.includes('intl') || allContent.includes('locale') ||
    allContent.includes('react-intl') || allContent.includes('i18next') || allContent.includes('lingui');

  const library = (() => {
    if (allContent.includes('i18next') || allContent.includes('react-i18next')) return 'i18next';
    if (allContent.includes('react-intl') || allContent.includes('FormatMessage')) return 'react-intl';
    if (allContent.includes('@lingui')) return 'Lingui';
    if (allContent.includes('expo-localization')) return 'expo-localization';
    return null;
  })();

  // Detect translation files
  const translationFiles = getAllFiles(ROOT, ['.json']).filter(f => {
    const r = rel(f).toLowerCase();
    return (r.includes('locale') || r.includes('translation') || r.includes('i18n') || r.includes('/en.') || r.includes('/en/') ||
            r.includes('lang') || r.includes('messages')) && !r.includes('node_modules');
  }).map(f => rel(f));

  // Detect locale files and extract languages
  const detectedLanguages = new Set();
  const langPattern = /\/([a-z]{2}(?:-[A-Z]{2})?)\.(json|ts|js)$/;
  for (const f of translationFiles) {
    const m = f.match(langPattern);
    if (m) detectedLanguages.add(m[1]);
  }

  // Hardcoded strings in components (simplified)
  let hardcodedStrings = 0;
  for (const { rel: rp, content } of sourceData) {
    if (!rp.endsWith('.tsx') && !rp.endsWith('.jsx')) continue;
    // Text components with hardcoded strings
    const hardcoded = content.match(/<Text[^>]*>\s*[A-Z][a-z]+/g) ?? [];
    hardcodedStrings += hardcoded.length;
  }

  // RTL support
  const hasRtl = allContent.includes('I18nManager') || allContent.includes('isRTL') || allContent.includes('forceRTL') ||
    allContent.includes('writingDirection') || allContent.includes('direction:');
  ...
  return {
    ...
    hardcodedStringsEstimate: hardcodedStrings,
...
function scanDocumentation() {
  ...
  let jsdocCount = 0, inlineCommentCount = 0, blockCommentCount = 0;
  let documentedFunctions = 0, undocumentedExports = 0;
  ...
  for (const { content } of sourceData) {
    ...
    const exportConst = content.match(/export\s+(?:default\s+)?const\s+\w+/g) ?? [];
    const totalExports = exports.length + exportConst.length;
    undocumentedExports += totalExports;
    // Check if export is preceded by JSDoc
    const documented = content.match(/\/\*\*[\s\S]*?\*\/\s*\n\s*export/g) ?? [];
    documentedFunctions += documented.length;
    undocumentedExports -= documented.length;
  }

  // Check for additional docs
  const docsDir = fileExists(path.join(ROOT, 'docs')) || fileExists(path.join(ROOT, 'documentation'));
  const hasApiDocs = fileExists(path.join(ROOT, 'docs/api')) || fileExists(path.join(ROOT, 'API.md'));
  const hasChangelog = fileExists(path.join(ROOT, 'CHANGELOG.md'));
  const hasContributing = fileExists(path.join(ROOT, 'CONTRIBUTING.md'));
  const hasCodeOfConduct = fileExists(path.join(ROOT, 'CODE_OF_CONDUCT.md'));
  const hasSecurityPolicy = fileExists(path.join(ROOT, 'SECURITY.md'));
  const hasLicense = fileExists(path.join(ROOT, 'LICENSE')) || fileExists(path.join(ROOT, 'LICENSE.md'));

  // Storybook
  const hasStorybook = fileExists(path.join(ROOT, '.storybook')) || getAllFiles(ROOT, ['.stories.tsx', '.stories.ts', '.stories.jsx', '.stories.js']).length > 0;
  const storyCount = getAllFiles(ROOT, ['.tsx', '.ts', '.jsx', '.js']).filter(f => f.includes('.stories.')).length;

  return {
    readme: {
      exists: readme.length > 0,
      lines: readmeLines,
      ...
      hasBadges,
      hasScreenshots,
      hasTOC: readme.includes('Table of Contents') || readme.includes('## Contents'),
      hasApiReference: readme.toLowerCase().includes('api reference') || readme.toLowerCase().includes('api docs'),
    },
    comments: { jsdoc: jsdocCount, inline: inlineCommentCount, block: blockCommentCount },
    documentedExports: documentedFunctions,
    undocumentedExports: Math.max(0, undocumentedExports),
    documentationRatio: documentedFunctions + undocumentedExports > 0
      ? `${Math.round((documentedFunctions / (documentedFunctions + undocumentedExports)) * 100)}%` : 'N/A',
    ...
    score: (() => {
      let s = 0;
      if (readme.length > 100) s += 15;
      if (readmeSections >= 3) s += 10;
      if (hasInstallInstructions) s += 10;
      if (hasUsageInstructions) s += 10;
      if (jsdocCount > 5) s += 10;
      if (hasChangelog) s += 10;
      if (hasLicense) s += 10;
      if (hasContributing) s += 5;
      if (hasStorybook) s += 10;
      if (documentedFunctions > undocumentedExports) s += 10;
      return Math.min(s, 100);
    })(),
  };
}
...
function scanBundleSize() {
  ...
  // Check node_modules sizes for heavy packages
  const heavyPackages = [
    'moment', 'lodash', 'firebase', 'aws-sdk', '@firebase/firestore',
    '@firebase/auth', 'face-api.js', '@tensorflow/tfjs', 'nsfwjs',
    'react-native-maps', 'lottie-react-native', '@sentry/react-native',
    'date-fns', 'rxjs', 'core-js', 'graphql', 'apollo-client',
  ];
  ...
  for (const pkg of heavyPackages) {
    ...
    if (fileExists(pkgDir)) {
      try {
        ...
        let size = 0;
        for (const f of files) size += getFileSizeBytes(f);
        if (size > 100 * 1024) { // Only report > 100KB
          packageSizes.push({ name: pkg, size, formattedSize: formatBytes(size) });
        }
      } catch {}
    }
  }
  ...
  const allProjectFiles = getAllFiles(ROOT, []).filter(f => !rel(f).includes('node_modules'));
  for (const f of allProjectFiles) {
    const ext = path.extname(f).toLowerCase();
    const size = getFileSizeBytes(f);
    if (imageExts.includes(ext)) imageBytes += size;
    if (fontExts.includes(ext)) fontBytes += size;
  }
  ...
  return {
    ...
    heavyDependencies: packageSizes.sort((a, b) => b.size - a.size).slice(0, 15),
    treeShaking,
    estimatedBundleSize: formatBytes(totalSourceBytes + imageBytes + fontBytes),
    recommendations: (() => {
      const recs = [];
      if (packageSizes.some(p => p.name === 'moment')) recs.push('Replace moment.js with date-fns or dayjs (much smaller)');
      if (packageSizes.some(p => p.name === 'lodash')) recs.push('Use lodash-es or individual lodash imports');
      if (treeShaking.wildcardReExports > 5) recs.push('Many wildcard re-exports — may prevent tree shaking');
      if (treeShaking.barrelFiles > 10) recs.push('Many barrel (index) files — may slow Metro bundler');
      if (imageBytes > 5 * 1024 * 1024) recs.push('Image assets exceed 5MB — consider CDN or compression');
      if (fontBytes > 2 * 1024 * 1024) recs.push('Font assets exceed 2MB — consider subsetting');
      return recs;
    })(),
  };
}
...
function scanTypeSafety() {
  ...
  const sourceData = getSourceData(getSourceFiles());
  let anyCount = 0, unknownCount = 0, neverCount = 0;
  let typeAssertions = 0, nonNullAssertions = 0;
  let genericUsage = 0, typeGuards = 0;
  let interfaceCount = 0, typeAliasCount = 0, enumCount = 0;
  let tsIgnores = 0, tsNoChecks = 0, tsExpectErrors = 0;
  let strictFiles = 0, looseFiles = 0;
  const worstFiles = [];
  ...
  for (const { rel: rp, content } of sourceData) {
    ...
    const fileUnknown = (content.match(/:\s*unknown\b/g) ?? []).length;
    const fileNever = (content.match(/:\s*never\b/g) ?? []).length;
    anyCount += fileAnyCount;
    unknownCount += fileUnknown;
    neverCount += fileNever;
    typeAssertions += (content.match(/\bas\s+(?!any)\w/g) ?? []).length;
    nonNullAssertions += (content.match(/!\./g) ?? []).length + (content.match(/!\[/g) ?? []).length;
    genericUsage += (content.match(/<[A-Z]\w*(?:,\s*[A-Z]\w*)*>/g) ?? []).length;
    typeGuards += (content.match(/is\s+\w+/g) ?? []).length;
    interfaceCount += (content.match(/\binterface\s+\w+/g) ?? []).length;
    typeAliasCount += (content.match(/\btype\s+\w+\s*=/g) ?? []).length;
    enumCount += (content.match(/\benum\s+\w+/g) ?? []).length;
    tsIgnores += (content.match(/@ts-ignore/g) ?? []).length;
    tsNoChecks += (content.match(/@ts-nocheck/g) ?? []).length;
    tsExpectErrors += (content.match(/@ts-expect-error/g) ?? []).length;

    if (fileAnyCount > 5) {
      worstFiles.push({ file: rp, anyCount: fileAnyCount });
      looseFiles++;
    } else {
      strictFiles++;
    }
  ...
  const runtimeValidation = {
    zod: allContent.includes('z.object') || allContent.includes('z.string') || allContent.includes("from 'zod'"),
    yup: allContent.includes('yup.object') || allContent.includes("from 'yup'"),
    joi: allContent.includes('Joi.object') || allContent.includes("from 'joi'"),
    superstruct: allContent.includes("from 'superstruct'"),
    typebox: allContent.includes('@sinclair/typebox'),
    valibot: allContent.includes("from 'valibot'"),
    io_ts: allContent.includes('io-ts'),
  };

  return {
    tsFiles, jsFiles, dtsFiles,
    typescriptCoverage: tsFiles + jsFiles > 0 ? `${Math.round((tsFiles / (tsFiles + jsFiles)) * 100)}%` : 'N/A',
    anyUsage: anyCount,
    unknownUsage: unknownCount,
    neverUsage: neverCount,
    typeAssertions, nonNullAssertions, genericUsage, typeGuards,
    ...
    score: (() => {
      let s = 50;
      if (tsFiles > jsFiles) s += 15;
      if (anyCount < 10) s += 15; else if (anyCount < 50) s += 5;
      if (tsIgnores + tsNoChecks === 0) s += 10;
      if (unknownCount > anyCount * 0.5) s += 5; // Using unknown over any
      if (runtimeValidation.zod || runtimeValidation.yup || runtimeValidation.joi) s += 10;
      if (anyCount > 100) s -= 20;
      if (tsNoChecks > 0) s -= 15;
      return Math.max(0, Math.min(s, 100));
    })(),
  };
}

// ─── 22. Dependency Graph (Circular Deps) ────────────────

function scanDependencyGraph() {
  console.log('🔄 Scanning for circular dependencies...');
  const sourceFiles = getSourceFiles();
  const graph = new Map(); // file -> [imported files]
  const fileMap = new Map(); // normalized path -> full path

  // Build file map
  for (const f of sourceFiles) {
    const r = rel(f);
    fileMap.set(r, f);
    fileMap.set(r.replace(/\.(ts|tsx|js|jsx)$/, ''), f);
  }
  ...
  for (const f of sourceFiles) {
    ...
    for (const m of importMatches) {
      ...
      const candidates = [
        relResolved, relResolved + '.ts', relResolved + '.tsx',
        relResolved + '.js', relResolved + '.jsx',
        path.join(relResolved, 'index.ts'), path.join(relResolved, 'index.tsx'),
        path.join(relResolved, 'index.js'), path.join(relResolved, 'index.jsx'),
      ];
      for (const c of candidates) {
        if (fileMap.has(c)) { deps.push(c); break; }
      }
    }
  ...
  function dfs(node) {
    if (inStack.has(node)) {
      ...
      if (cycleStart !== -1) {
        ...
        if (cycle.length >= 2 && cycle.length <= 10) { // Only report reasonable cycles
  ...
  // Most imported files (high coupling)
  const mostImported = Object.entries(importCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 15)
    .map(([file, count]) => ({ file, importedBy: count }));

  // Orphan files (no imports and not imported by anyone)
  ...
  const orphanFiles = sourceFiles
    .map(f => rel(f))
    .filter(f => !importedFiles.has(f) && !filesWithImports.has(f) && !f.includes('index.') && !f.includes('_layout') && !f.includes('+'))
    .slice(0, 20);

  return {
    totalModules: graph.size,
    totalImports: [...graph.values()].reduce((a, b) => a + b.length, 0),
    circularDependencies: circularDeps.slice(0, 20),
...
async function runFullScan() {
  ...
  const sourceFiles = getSourceFiles();
  getSourceData(sourceFiles); // Pre-load all source data
...
function calculateHealthScore(a) {
  const scores = {
    security: (() => {
      let s = 100;
      s -= a.security.bySeverity.critical * 25;
      s -= a.security.bySeverity.high * 10;
      s -= a.security.bySeverity.medium * 3;
      return Math.max(0, s);
    })(),
    codeQuality: (() => {
      let s = 70;
      if (a.codeQuality.hasLinter) s += 10;
      if (a.codeQuality.hasFormatter) s += 10;
      if (a.codeQuality.hasHusky) s += 5;
      if (a.codeQuality.issues.anyTypes < 10) s += 5;
      else if (a.codeQuality.issues.anyTypes > 50) s -= 15;
      if (a.codeQuality.issues.consoleLogs > 20) s -= 10;
      return Math.max(0, Math.min(s, 100));
    })(),
    testing: (() => {
      const testRatio = a.codeQuality.files.total > 0 ? a.codeQuality.files.test / a.codeQuality.files.total : 0;
      return Math.min(100, Math.round(testRatio * 500));
    })(),
    documentation: a.documentation.score,
    accessibility: a.accessibility.score,
    performance: a.performance.score,
    typeSafety: a.typeSafety.score,
    errorHandling: a.errorHandling.score,
    dependencies: (() => {
      let s = 80;
      if (a.dependencyHealth.lockfile.hasAny) s += 10;
      if (a.dependencyHealth.lockfile.multipleLockfiles) s -= 15;
      if (a.packages.possiblyUnused?.length > 5) s -= 10;
      if (a.dependencyHealth.autoUpdate.hasAny) s += 10;
      if ((a.dependencyHealth.majorUpdatesAvailable ?? 0) > 5) s -= 10;
      return Math.max(0, Math.min(s, 100));
    })(),
    architecture: (() => {
      let s = 60;
      if (a.dependencyGraph.circularCount === 0) s += 20;
      else if (a.dependencyGraph.circularCount < 5) s += 10;
      else s -= 10;
      if (a.routes.hasNotFound) s += 5;
      if (a.components.errorBoundaries > 0) s += 10;
      if (a.components.customHooks.length > 3) s += 5;
      return Math.max(0, Math.min(s, 100));
    })(),
  };

  const weights = {
    security: 0.20, codeQuality: 0.15, testing: 0.10,
    documentation: 0.05, accessibility: 0.10, performance: 0.10,
    typeSafety: 0.10, errorHandling: 0.10, dependencies: 0.05, architecture: 0.05,
  };

  const overall = Math.round(
    Object.entries(scores).reduce((sum, [key, val]) => sum + val * (weights[key] ?? 0.1), 0)
  );
...
function generateReport(a) {
  ...
  const scoreBar = (score, label) => {
    ...
    l(`  ${label.padEnd(20)} [${bar}] ${score}/100`);
  };

  l('╔══════════════════════════════════════════════════════════════════════╗');
  l('║              ULTIMATE PROJECT AUDIT REPORT v4.0                    ║');
  l('╚══════════════════════════════════════════════════════════════════════╝');
  ...
  for (const [key, val] of Object.entries(a.healthScore.breakdown)) {
    const emoji = { security: '🔒', codeQuality: '✨', testing: '🧪', documentation: '📚', accessibility: '♿',
      performance: '⚡', typeSafety: '📘', errorHandling: '🚨', dependencies: '📦', architecture: '🏗️' }[key] ?? '📊';
    scoreBar(val, `${emoji} ${key}`);
  }

  H('1. PROJECT IDENTITY');
  row('Name:', a.project.name);
  row('Version:', a.project.version);
  row('License:', a.project.license);
  row('Package Manager:', a.project.packageManager);
  row('Monorepo:', a.project.isMonorepo ? 'Yes' : 'No');
  row('Module Type:', a.project.type);
  row('Node.js:', a.project.runtime.node);
  if (a.project.expo) {
    S('Expo');
    row('SDK Version:', a.project.expo.sdkVersion);
    row('Bundle ID (iOS):', a.project.expo.ios.bundleIdentifier);
    row('Package (Android):', a.project.expo.android.package);
    row('Scheme:', a.project.expo.scheme);
  ...
  row('React:', ts.mobileFramework.reactVersion);
  row('React Native:', ts.mobileFramework.reactNativeVersion);
  row('Workflow:', ts.mobileFramework.workflow);
  row('Hermes:', ts.mobileFramework.hermes ? 'Yes ✅' : 'No');
  ...
  tick(ts.backend.redis.used, `Redis v${ts.backend.redis.version ?? '?'}`);
  ...
  S('Server');
  row('Framework:', ts.server.framework);
  row('Language:', ts.server.language);
  row('Hosting:', ts.server.hosting);
  row('Endpoints:', ts.server.endpoints);
  row('Server Files:', ts.server.fileCount);
  l('  Security Middleware:');
  tick(ts.server.security.helmet, '  Helmet');
  tick(ts.server.security.cors, '  CORS');
  tick(ts.server.security.rateLimit, '  Rate Limiting');
  tick(ts.server.security.hmac, '  HMAC Signing');
  tick(ts.server.security.validateOrigin, '  Origin Validation');
  tick(ts.server.security.requireAuth, '  Auth Middleware');
  tick(ts.server.security.inputValidation, '  Input Validation');
  tick(ts.server.security.csrf, '  CSRF Protection');

  S('Authentication');
  tick(ts.authentication.firebaseAuth, 'Firebase Auth');
  tick(ts.authentication.googleSignIn, 'Google Sign-In');
  tick(ts.authentication.appleSignIn, 'Apple Sign-In');
  tick(ts.authentication.biometric, 'Biometric / Face ID');
  tick(ts.authentication.e2ee, `E2EE (${ts.authentication.e2eeMethod})`);
  tick(ts.authentication.mfa, 'Multi-Factor Auth');
  tick(ts.authentication.jwt, 'JWT');
  tick(ts.authentication.oauth, 'OAuth');
  ...
  tick(ts.aiMl.langchain.used, `LangChain v${ts.aiMl.langchain.version ?? '?'}`);
  ...
  tick(ts.media.aws.used, 'AWS S3/SDK');
  tick(ts.media.expoImage, 'expo-image');
  tick(ts.media.expo.imagePicker, 'Image Picker');
  tick(ts.media.expo.camera, 'Camera');
  tick(ts.media.expo.video, 'Video');

  S('Security Libraries');
  tick(ts.securityLibraries.tweetnacl, `TweetNaCl v${ts.securityLibraries.tweetnaclVersion ?? '?'}`);
  tick(ts.securityLibraries.expoSecureStore, 'Expo SecureStore');
  tick(ts.securityLibraries.expoCrypto, 'Expo Crypto');
  tick(ts.securityLibraries.webCrypto, 'Web Crypto API');
  tick(ts.securityLibraries.nodeCrypto, 'Node.js crypto');
  tick(ts.securityLibraries.hmacSigning, 'HMAC signing');
  tick(ts.securityLibraries.certificatePinning, 'Certificate Pinning');
  tick(ts.securityLibraries.jailbreakDetection, 'Jailbreak Detection');

  S('State Management');
  tick(ts.stateManagement.redux, 'Redux Toolkit');
  tick(ts.stateManagement.zustand, 'Zustand');
  tick(ts.stateManagement.jotai, 'Jotai');
  tick(ts.stateManagement.mmkv, 'MMKV');
  tick(ts.stateManagement.secureStore, 'SecureStore');
  tick(ts.stateManagement.asyncStorage, 'AsyncStorage');
  row('createContext() usages:', ts.stateManagement.contextCount);
  row('useReducer() usages:', ts.stateManagement.useReducerCount);
  ...
  tick(ts.ui.bottomSheet, 'Bottom Sheet');
  tick(ts.ui.haptics, 'Haptics');
  tick(ts.ui.carousel, 'Carousel');
  tick(ts.ui.charts, 'Charts');
  tick(ts.ui.webview, 'WebView');
  ...
  tick(ts.notifications.oneSignal, 'OneSignal');
  tick(ts.notifications.webPush, 'Web Push');
  ...
  tick(ts.analyticsMonitoring.mixpanel.used, 'Mixpanel');
  tick(ts.analyticsMonitoring.firebaseAnalytics, 'Firebase Analytics');
  tick(ts.analyticsMonitoring.crashlytics.used, 'Crashlytics');

  S('Testing');
  tick(ts.testing.jest, `Jest v${ts.testing.jestVersion ?? '?'}`);
  tick(ts.testing.vitest, 'Vitest');
  tick(ts.testing.testingLibrary, 'Testing Library');
  tick(ts.testing.detox, 'Detox E2E');
  tick(ts.testing.maestro, 'Maestro');
  tick(ts.testing.msw, 'MSW (Mock Service Worker)');
  row('Test files:', ts.testing.testFiles);
  row('Snapshot files:', ts.testing.snapshotFiles);

  S('Build & Deployment');
  tick(ts.buildDeployment.eas, 'EAS Build');
  tick(ts.buildDeployment.cicd.githubActions, 'GitHub Actions');
  tick(ts.buildDeployment.docker.dockerfile, 'Docker');
  tick(ts.buildDeployment.otaUpdates, 'OTA Updates');
  tick(ts.buildDeployment.bundleAnalyzer, 'Bundle Analyzer');
  ...
  for (const [cat, apis] of Object.entries(byCategory)) {
    l(`  [${cat}]`);
    for (const api of apis) { l(`    ✅ ${api.name} (${api.url})`); if (api.usedIn.length) l(`       Used in: ${api.usedIn.join(', ')}`); }
    l('');
  }

  H('4. PACKAGES');
  row('Total:', a.packages.counts.total);
  row('Production:', a.packages.counts.production);
  row('Development:', a.packages.counts.development);
  row('Actually imported:', a.packages.counts.actuallyImported);
  row('Not installed:', a.packages.counts.notInstalled);
  row('Categories:', a.packages.counts.categories);
  ...
  for (const [cat, pkgs] of Object.entries(a.packages.byCategory)) {
    l(`\n  ${cat} (${pkgs.length})`);
    for (const pkg of pkgs) l(`    ${pkg.actuallyImported ? '✅' : '⚪'} ${pkg.name.padEnd(45)} ${pkg.installedVersion ?? pkg.specifiedVersion}`);
  }
  S('NPM Scripts');
  for (const [name, cmd] of Object.entries(a.packages.scripts)) l(`  ${name.padEnd(25)} ${cmd}`);

  H('5. ENVIRONMENT VARIABLES');
  row('Total vars:', a.environmentVariables.summary.totalVars);
  row('Unique vars:', a.environmentVariables.summary.uniqueVars);
  row('Empty:', a.environmentVariables.summary.emptyVars);
  row('Placeholders:', a.environmentVariables.summary.placeholderVars);
  row('Secrets:', a.environmentVariables.summary.secretVars);
  row('EXPO_PUBLIC_ vars:', a.environmentVariables.summary.expoPublicVars);
  tick(a.environmentVariables.envInGitignore, '.env in .gitignore');
  tick(a.environmentVariables.hasExample, '.env.example exists');
  ...
  for (const [file, vars] of Object.entries(a.environmentVariables.files)) {
    S(file);
    for (const v of vars) l(`  ${v.hasValue && !v.isPlaceholder ? '✅' : '❌'} ${v.isSecret ? '🔑' : '  '} ${v.key.padEnd(45)} ${v.maskedValue}`);
  }

  H('6. SECURITY');
  row('Critical:', a.security.bySeverity.critical);
  row('High:', a.security.bySeverity.high);
  row('Medium:', a.security.bySeverity.medium);
  row('Low:', a.security.bySeverity.low);
  row('Info:', a.security.bySeverity.info);
  ...
  H('7. API ENDPOINTS');
  row('Total:', a.apiEndpoints.total);
  row('Without auth:', a.apiEndpoints.withoutAuth);
  row('Without rate limit:', a.apiEndpoints.withoutRateLimit);
  row('Without validation:', a.apiEndpoints.withoutValidation);
  row('Server files:', a.apiEndpoints.serverFileCount);
  if (a.apiEndpoints.endpoints.length) {
    S('All Endpoints');
    for (const ep of a.apiEndpoints.endpoints) {
      const auth = ep.hasAuth ? '🔒' : ep.path === '/health' ? '🟢' : '🔓';
      const rl = ep.hasRateLimit ? '⏱' : ' ';
      const val = ep.hasValidation ? '✓' : ' ';
      l(`  ${auth}${rl}${val} [${ep.method.padEnd(6)}] ${ep.path.padEnd(35)} ${ep.file}:${ep.line}`);
    }
  }

  H('8. ROUTES & NAVIGATION');
  row('Router type:', a.routes.type);
  row('Total routes:', a.routes.totalRoutes);
  row('Dynamic routes:', a.routes.dynamicRoutes);
  row('Static routes:', a.routes.staticRoutes);
  row('Layouts:', a.routes.layouts.length);
  row('Groups:', a.routes.groups.join(', ') || 'None');
  row('Modals:', a.routes.modals);
  row('API routes:', a.routes.apiRoutes);
  tick(a.routes.hasNotFound, '404 / Not Found page');
  tick(a.routes.hasTabNavigation, 'Tab navigation');
  tick(a.routes.hasStackNavigation, 'Stack navigation');
  tick(a.routes.hasDrawerNavigation, 'Drawer navigation');
  if (a.routes.deepLinks.length) { S('Deep Links'); for (const dl of a.routes.deepLinks) l(`  • [${dl.type}] ${dl.value}`); }

  H('9. COMPONENTS & HOOKS');
  row('Total components:', a.components.totalComponents);
  row('Functional:', a.components.functionalComponents);
  row('Class (legacy):', a.components.classComponents);
  row('Memoized:', a.components.memoizedComponents);
  row('Custom hooks:', a.components.hookCount);
  row('Contexts:', a.components.contextCount);
  row('Error boundaries:', a.components.errorBoundaries);
  ...
  H('10. ERROR HANDLING');
  row('Try/catch blocks:', a.errorHandling.tryCatchBlocks);
  row('Empty catch blocks:', a.errorHandling.emptyTryCatch);
  row('Error boundaries:', a.errorHandling.errorBoundaries);
  row('Throw statements:', a.errorHandling.throwStatements);
  row('Sentry captures:', a.errorHandling.sentryCapture);
  ...
  row('Fetch calls:', a.networkCalls.fetchCalls);
  row('Axios calls:', a.networkCalls.axiosCalls);
  row('Data libraries:', a.networkCalls.dataFetchingLibraries.join(', ') || 'None');
  tick(a.networkCalls.hasTimeout, 'Timeout configured');
  tick(a.networkCalls.hasRetry, 'Retry logic');
  tick(a.networkCalls.hasAbortController, 'AbortController');
  tick(a.networkCalls.hasGraphQL, 'GraphQL');
  tick(a.networkCalls.hasWebSocket, 'WebSocket');
  tick(a.networkCalls.resilience.hasCircuitBreaker, 'Circuit breaker');

  H('12. ACCESSIBILITY');
  scoreBar(a.accessibility.score, 'Accessibility');
  row('Labels:', a.accessibility.labels);
  row('Roles:', a.accessibility.roles);
  row('Hints:', a.accessibility.hints);
  row('Touchable without label:', a.accessibility.touchableWithoutLabel);
  row('Image without alt:', a.accessibility.imageWithoutAlt);
  tick(a.accessibility.hasScreenReaderSupport, 'Screen reader support');
  tick(a.accessibility.hasDynamicFontSize, 'Dynamic font sizing');
  tick(a.accessibility.hasReducedMotion, 'Reduced motion support');
  ...
  row('Inline functions in JSX:', a.performance.inlineFunctionsInJSX);
  row('Heavy imports:', a.performance.heavyImports);
  S('Optimizations Used');
  for (const [k, v] of Object.entries(a.performance.optimizations)) {
    if (typeof v === 'boolean') tick(v, k);
    else if (typeof v === 'number' && v > 0) row(`  ${k}:`, v);
  }
  if (a.performance.issues.length) { S('Issues'); for (const i of a.performance.issues.slice(0, 15)) l(`  [${i.severity}] ${i.file}: ${i.issue}`); }

  H('14. ASSETS');
  row('Total assets:', a.assets.summary.totalAssets);
  row('Total size:', a.assets.summary.totalSize);
  row('Images:', a.assets.summary.images);
  row('SVGs:', a.assets.summary.svgs);
  row('Fonts:', a.assets.summary.fonts);
  row('Videos:', a.assets.summary.videos);
  row('Audios:', a.assets.summary.audios);
  ...
  row('Translation files:', a.i18n.translationFiles.length);
  tick(a.i18n.hasRtlSupport, 'RTL support');
  tick(a.i18n.hasLocaleDetection, 'Locale detection');
  tick(a.i18n.hasDateFormatting, 'Date formatting');
  tick(a.i18n.hasNumberFormatting, 'Number formatting');
  row('Hardcoded strings (est):', a.i18n.hardcodedStringsEstimate);

  H('16. DOCUMENTATION');
  scoreBar(a.documentation.score, 'Documentation');
  tick(a.documentation.readme.exists, `README (${a.documentation.readme.lines} lines, ${a.documentation.readme.sections} sections)`);
  tick(a.documentation.readme.hasInstallInstructions, 'Install instructions');
  tick(a.documentation.readme.hasUsageInstructions, 'Usage instructions');
  tick(a.documentation.files.changelog, 'CHANGELOG');
  tick(a.documentation.files.contributing, 'CONTRIBUTING');
  tick(a.documentation.files.license, 'LICENSE');
  tick(a.documentation.files.security, 'SECURITY');
  ...
  row('Documented exports:', a.documentation.documentedExports);
  row('Undocumented exports:', a.documentation.undocumentedExports);
  row('Documentation ratio:', a.documentation.documentationRatio);

  H('17. TYPE SAFETY');
  scoreBar(a.typeSafety.score, 'Type Safety');
  row('TS files:', a.typeSafety.tsFiles);
  row('JS files:', a.typeSafety.jsFiles);
  row('Coverage:', a.typeSafety.typescriptCoverage);
  row('"any" usage:', a.typeSafety.anyUsage);
  row('"unknown" usage:', a.typeSafety.unknownUsage);
  row('Type assertions:', a.typeSafety.typeAssertions);
  row('Non-null assertions (!.):', a.typeSafety.nonNullAssertions);
  row('Interfaces:', a.typeSafety.definitions.interfaces);
  row('Type aliases:', a.typeSafety.definitions.typeAliases);
  row('Enums:', a.typeSafety.definitions.enums);
  row('@ts-ignore:', a.typeSafety.suppressions.tsIgnore);
  row('@ts-nocheck:', a.typeSafety.suppressions.tsNoCheck);
  ...
  row('Source code:', a.bundleSize.sourceCode.totalSize);
  row('Image assets:', a.bundleSize.assets.images);
  row('Font assets:', a.bundleSize.assets.fonts);
  row('Estimated total:', a.bundleSize.estimatedBundleSize);
  row('Barrel (index) files:', a.bundleSize.treeShaking.barrelFiles);
  row('Wildcard re-exports:', a.bundleSize.treeShaking.wildcardReExports);
  ...
  row('Total modules:', a.dependencyGraph.totalModules);
  row('Total imports:', a.dependencyGraph.totalImports);
  row('Avg imports/file:', a.dependencyGraph.averageImportsPerFile);
  row('Circular deps:', a.dependencyGraph.circularCount);
  ...
  if (a.dependencyGraph.mostImportedFiles.length) { S('Most Imported Files (High Coupling)'); for (const f of a.dependencyGraph.mostImportedFiles.slice(0, 10)) l(`  ${String(f.importedBy).padStart(4)}x  ${f.file}`); }
  ...
  row('Major updates:', a.dependencyHealth.majorUpdatesAvailable ?? 'N/A');
  if (a.dependencyHealth.vulnerabilities) {
    S('Vulnerabilities');
    row('Critical:', a.dependencyHealth.vulnerabilities.critical);
    row('High:', a.dependencyHealth.vulnerabilities.high);
    row('Moderate:', a.dependencyHealth.vulnerabilities.moderate);
    row('Low:', a.dependencyHealth.vulnerabilities.low);
    row('Total:', a.dependencyHealth.vulnerabilities.total);
  }

  H('21. FILE STRUCTURE');
  row('Total files:', a.fileStructure.totalFiles);
  row('Code files:', a.fileStructure.totalCodeFiles);
  row('Lines of code:', a.fileStructure.totalLinesOfCode.toLocaleString());
  row('Avg lines/file:', a.fileStructure.averageLinesPerFile);
  row('Median lines/file:', a.fileStructure.medianLinesPerFile);
  row('Files >300 lines:', a.fileStructure.longFiles);
  row('Empty files:', a.fileStructure.emptyFiles);
  row('Max directory depth:', a.fileStructure.maxDirectoryDepth);
  S('Top-Level Directories');
  for (const d of a.fileStructure.topLevelDirs) l(`  📁 ${d}`);
  S('By Directory');
  for (const [dir, count] of Object.entries(a.fileStructure.byDirectory).sort((a, b) => b[1] - a[1])) l(`  ${dir.padEnd(30)} ${count} files`);
  S('Largest Files');
  for (const f of a.fileStructure.largestFiles.slice(0, 15)) l(`  ${String(f.lines).padStart(6)} lines  ${f.file}`);
  S('Config Files Present');
  for (const f of a.fileStructure.configFilesPresent) l(`  ✅ ${f}`);
  S('Config Files Missing');
  for (const f of a.fileStructure.configFilesMissing.slice(0, 20)) l(`  ❌ ${f}`);

  H('22. CODE QUALITY');
  row('Total lines:', a.codeQuality.totalLines.toLocaleString());
  row('Code lines:', a.codeQuality.codeLines.toLocaleString());
  row('Comment ratio:', a.codeQuality.commentRatio);
  row('Blank ratio:', a.codeQuality.blankRatio);
  tick(a.codeQuality.hasLinter, 'ESLint');
  tick(a.codeQuality.hasFormatter, 'Prettier');
  tick(a.codeQuality.hasEditorConfig, '.editorconfig');
  tick(a.codeQuality.hasHusky, 'Husky git hooks');
  tick(a.codeQuality.hasLintStaged, 'lint-staged');
  tick(a.codeQuality.hasCommitLint, 'commitlint');
  S('Issues');
  row('TODOs:', a.codeQuality.issues.todos);
  row('FIXMEs:', a.codeQuality.issues.fixmes);
  row('"any" types:', a.codeQuality.issues.anyTypes);
  row('@ts-ignore:', a.codeQuality.issues.tsIgnores);
  row('console.log():', a.codeQuality.issues.consoleLogs);
  S('File Breakdown');
  row('Components:', a.codeQuality.files.component);
  row('Hooks:', a.codeQuality.files.hook);
  row('Utils:', a.codeQuality.files.util);
  row('Types:', a.codeQuality.files.type);
  row('Tests:', a.codeQuality.files.test);
  row('Config:', a.codeQuality.files.constant);
  if (a.codeQuality.complexity.longestFunction.length > 0) {
    S('Complexity');
    row('Longest function:', `${a.codeQuality.complexity.longestFunction.name} (${a.codeQuality.complexity.longestFunction.length} lines) in ${a.codeQuality.complexity.longestFunction.file}`);
    row('Most complex file:', `${a.codeQuality.complexity.maxComplexityFile.file} (complexity: ${a.codeQuality.complexity.maxComplexityFile.complexity})`);
  }

  H('23. GIT');
  row('Branch:', a.git.currentBranch);
  row('Total commits:', a.git.totalCommits);
  row('Last commit:', `${a.git.lastCommitDate}`);
  row('Last message:', a.git.lastCommitMessage);
  row('Uncommitted:', a.git.uncommittedFiles);
  row('Staged:', a.git.stagedFiles);
  row('Untracked:', a.git.untrackedFiles);
  row('Stash count:', a.git.stashCount);
  row('Remote:', a.git.remoteUrl);
  row('Tags:', a.git.tags.length);
  row('Branches:', a.git.branches.length);
  tick(a.git.hasGitHubActions, 'GitHub Actions');
  tick(a.git.hasGitIgnore, '.gitignore');
  tick(a.git.hasGitAttributes, '.gitattributes');
  tick(a.git.lfs, 'Git LFS');
  tick(a.git.submodules, 'Submodules');
  if (a.git.commitConventions.total > 0) {
    S('Commit Conventions');
    row('Conventional commits:', `${a.git.commitConventions.conventional}/${a.git.commitConventions.total} (${a.git.commitConventions.ratio})`);
  }
  ...
  if (a.git.topContributors?.length) { S('Contributors'); for (const c of a.git.topContributors) l(`  ${c}`); }
...
function printSummary(a) {
  const ts = a.techStack;
  const sec = a.security.bySeverity;
  ...
  if (ts.backend.firebase.used) {
    const svcs = Object.entries(ts.backend.firebase.services).filter(([, v]) => v).map(([k]) => k);
    console.log(`   Firebase:      v${ts.backend.firebase.version} (${svcs.join(', ')})`);
  }
  ...
  const aiList = [ts.aiMl.faceApi.used && 'face-api.js', ts.aiMl.tensorflow.used && 'TensorFlow', ts.aiMl.openai.used && 'OpenAI', ts.aiMl.langchain.used && 'LangChain', ts.aiMl.cloudinaryAI && 'Cloudinary AI'].filter(Boolean);
  console.log(`   AI/ML:         ${aiList.join(', ') || 'None'}`);

  console.log('\n🔒 SECURITY');
  console.log(`   E2EE:          ${ts.authentication.e2ee ? ts.authentication.e2eeMethod : 'No ❌'}`);
  const criticalColor = sec.critical > 0 ? '🔴' : '✅';
  const highColor = sec.high > 0 ? '🟠' : '✅';
  const medColor = sec.medium > 0 ? '🟡' : '✅';
  console.log(`   Issues:        ${criticalColor} ${sec.critical} critical  ${highColor} ${sec.high} high  ${medColor} ${sec.medium} medium`);

  console.log('\n📦 CODE');
  console.log(`   Packages:      ${a.packages.counts.total} (${a.packages.counts.production} prod)`);
  ...
  console.log('\n📊 QUALITY METRICS');
  console.log(`   Type safety:   ${a.typeSafety.typescriptCoverage} TS coverage, ${a.typeSafety.anyUsage} "any" usages`);
  console.log(`   Accessibility: score ${a.accessibility.score}/100`);
  console.log(`   Performance:   score ${a.performance.score}/100`);
  console.log(`   Error handl.:  score ${a.errorHandling.score}/100`);
  console.log(`   Documentation: score ${a.documentation.score}/100`);

  if (a.environmentVariables.emptyOrPlaceholder?.length) {
    console.log('\n⚠️  ENV VARS NEEDING ATTENTION:');
    for (const v of a.environmentVariables.emptyOrPlaceholder.slice(0, 5)) console.log(`   ❌ ${v}`);
    if (a.environmentVariables.emptyOrPlaceholder.length > 5) console.log(`   ... and ${a.environmentVariables.emptyOrPlaceholder.length - 5} more`);
  }

  if (a.packages.possiblyUnused?.length) {
    console.log('\n⚠️  POSSIBLY UNUSED PACKAGES:');
    for (const p of a.packages.possiblyUnused.slice(0, 5)) console.log(`   • ${p}`);
  }

  if (a.dependencyGraph.circularCount > 0) {
    console.log('\n🔄 CIRCULAR DEPENDENCIES:');
    for (const c of a.dependencyGraph.circularDependencies.slice(0, 3)) console.log(`   ${c.join(' → ')}`);
  }
  ...
  if (a.typeSafety.anyUsage > 50) recs.push(`📘 Reduce "any" usage (${a.typeSafety.anyUsage} found)`);
  ...
  if (a.accessibility.score < 50) recs.push('♿ Improve accessibility (labels, roles, hints)');
  ...
  if (a.performance.heavyImports > 0) recs.push('📏 Replace heavy imports (moment/lodash) with lighter alternatives');
  if (!a.dependencyHealth.autoUpdate.hasAny) recs.push('🔄 Add Renovate or Dependabot for auto-updates');

  if (recs.length) {
    console.log('\n💡 TOP RECOMMENDATIONS:');
    for (const r of recs.slice(0, 10)) console.log(`   ${r}`);
  }
