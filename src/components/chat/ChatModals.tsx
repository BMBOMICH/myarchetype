...
export const ChatModals = React.memo(({
  core, matchName, formatTime,
  onCloseOptions, onPinSelected, onOpenReaction, onOpenReport,
  onCloseReaction, onCloseReport, onCancelReport, onSubmitReport, onReportReasonChange,
  onCloseNote, onSaveNote, onNoteTextChange,
  onClosePinned, onCloseVideo, onCloseDateIdeas, onClosePreview,
  onReaction, onUseDateIdea, onShuffleDateIdeas,
}: ChatModalsProps) => {
  return (
    <>
        ...
        <Pressable style={styles.modalOverlay} onPress={onCloseOptions} accessibilityLabel="Close options" accessibilityRole="button">
          <View style={styles.optionsCard}>
            {core.selectedMessageId && (
              <>
                <Pressable style={styles.optionItem} onPress={onPinSelected} accessibilityLabel="Pin message" accessibilityRole="button">
                  <Ionicons name="pin-outline" size={18} color="#9494B8" /><Text style={styles.optionText}>Pin Message</Text>
                </Pressable>
                <Pressable style={styles.optionItem} onPress={onOpenReaction} accessibilityLabel="React to message" accessibilityRole="button">
                  <Ionicons name="happy-outline" size={18} color="#9494B8" /><Text style={styles.optionText}>React</Text>
                </Pressable>
                <Pressable style={styles.optionItem} onPress={onOpenReport} accessibilityLabel="Report message" accessibilityRole="button">
                  <Ionicons name="flag-outline" size={18} color="#FF6B6B" /><Text style={[styles.optionText, styles.optionTextDanger]}>Report</Text>
                </Pressable>
              </>
            )}
          </View>
        ...
        <Pressable style={styles.modalOverlay} onPress={onCloseReaction} accessibilityLabel="Close reaction picker" accessibilityRole="button">
          <View style={styles.reactionPickerCard}>
            {EMOJI_REACTIONS.map((emoji) => (
              <Pressable key={emoji} onPress={() => onReaction(emoji)} style={styles.reactionPickItem} accessibilityLabel={`React with ${emoji}`} accessibilityRole="button">
      ...
      <Modal visible={core.showReport} transparent animationType="slide" onRequestClose={onCloseReport}>
        <View style={styles.reportModal}>
          <View style={styles.reportCard}>
            ...
            <TextInput style={styles.reportInput} value={core.reportReason} onChangeText={onReportReasonChange} placeholder="Describe the issue…" placeholderTextColor="#64648a" multiline maxLength={500} autoFocus accessibilityLabel="Report reason input" />
            <View style={styles.reportBtns}>
              ...
              <Pressable style={[styles.reportSubmit, (!core.reportReason.trim() || core.submittingReport) && styles.reportSubmitDisabled]} onPress={onSubmitReport} disabled={!core.reportReason.trim() || core.submittingReport} accessibilityLabel="Submit report" accessibilityRole="button">
      ...
      <Modal visible={core.showNote} transparent animationType="slide" onRequestClose={onCloseNote}>
        <View style={styles.noteModal}>
          <View style={styles.noteCard}>
            ...
            <TextInput style={styles.noteInput} value={core.noteText} onChangeText={onNoteTextChange} placeholder="Write notes together…" placeholderTextColor="#64648a" multiline maxLength={500} autoFocus accessibilityLabel="Note input" />
            ...
            <View style={styles.noteBtns}>
              ...
              <Pressable style={[styles.noteSave, core.savingNote && styles.noteSaveDisabled]} onPress={onSaveNote} disabled={core.savingNote} accessibilityLabel="Save note" accessibilityRole="button">
        ...
        <Pressable style={styles.modalOverlay} onPress={onCloseVideo} accessibilityLabel="Close call prompt" accessibilityRole="button">
          <View style={styles.videoPromptCard}>
            ...
            <View style={styles.videoPromptBtns}>
              <Pressable style={styles.videoBtn} onPress={() => { onCloseVideo(); Alert.alert('Coming Soon', 'Video calls will be available soon!'); }} accessibilityLabel="Start video call" accessibilityRole="button">
                <Ionicons name="videocam" size={22} color="#6C63FF" /><Text style={styles.videoBtnText}>Video</Text>
              </Pressable>
              <Pressable style={styles.videoBtn} onPress={() => { onCloseVideo(); Alert.alert('Coming Soon', 'Audio calls will be available soon!'); }} accessibilityLabel="Start audio call" accessibilityRole="button">
                <Ionicons name="call" size={22} color="#6C63FF" /><Text style={styles.videoBtnText}>Audio</Text>
              </Pressable>
            </View>
          </View>
        </Pressable>
      ...
      <Modal visible={core.showDateIdeas} transparent animationType="slide" onRequestClose={onCloseDateIdeas}>
        <View style={styles.dateModal}>
          <View style={styles.dateCard}>
            ...
            {core.loadingDateIdeas ? <ActivityIndicator size="large" color="#6C63FF" style={styles.dateLoader} /> : (
              <>
                {core.dateIdeas.map((idea) => (
                  <Pressable key={idea.text} style={styles.dateIdeaItem} onPress={() => onUseDateIdea(idea.text)} accessibilityLabel={`Use date idea: ${idea.text}`} accessibilityRole="button">
                    <Text style={styles.dateIdeaVibe}>{idea.vibe}</Text>
                    <Text style={styles.dateIdeaText}>{idea.text}</Text>
                  </Pressable>
                ))}
                <Pressable style={styles.dateRefreshBtn} onPress={onShuffleDateIdeas} accessibilityLabel="Shuffle date ideas" accessibilityRole="button">
                  <Ionicons name="refresh" size={16} color="#6C63FF" /><Text style={styles.dateRefreshText}>Shuffle</Text>
                </Pressable>
              </>
            )}
          </View>
      ...
      <Modal visible={!!core.previewImage} transparent animationType="fade" onRequestClose={onClosePreview}>
        <View style={styles.previewModal}>
          <Pressable style={styles.previewClose} onPress={onClosePreview} hitSlop={16} accessibilityLabel="Close image preview" accessibilityRole="button">
            <Ionicons name="close" size={28} color="#fff" />
          </Pressable>
          {core.previewImage ? <TurboImage source={{ uri: core.previewImage }} style={styles.previewImage} resizeMode="contain" cachePolicy="dataCache" accessibilityLabel="Full size image preview" /> : null}
        </View>
      </Modal>
    </>
  );
